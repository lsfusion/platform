...
//        return true;
    ...
    void OutChangesTable(DataSession Session) throws SQLException {
        ...
        Join<KeyField,PropertyField> ChangeJoin = new MapJoin<KeyField,PropertyField,T>(ChangeTable,Query,ChangeTableMap);
        ChangeJoin.Joins.put(ChangeTable.Property,new ValueExpr(ID,ChangeTable.Property.Type));
        Query.add(ChangeJoin.InJoin);
    ...
    class Change {
        ...
        void correct(int RequiredType) {
            ...
            if(Type != RequiredType) {
                ...
                UniJoin<T, PropertyField> ChangeJoin = new UniJoin<T, PropertyField>(Source, NewQuery);
                SourceExpr NewExpr = ChangeJoin.Exprs.get(ChangeTable.Value);
                NewQuery.add(ChangeJoin.InJoin);
        ...
        void save(DataSession Session) throws SQLException {
            ...
            Join<KeyField,PropertyField> ReadJoin = new MapJoin<KeyField,PropertyField,T>(ChangeTable,ReadQuery,ChangeTableMap);
            ReadJoin.Joins.put(ChangeTable.Property,new ValueExpr(ID,ChangeTable.Property.Type));
            ReadQuery.add(ReadJoin.InJoin);

            JoinQuery<KeyField,PropertyField> WriteQuery = new JoinQuery<KeyField,PropertyField>(ChangeTable.Keys);
            Join<T,PropertyField> WriteJoin = new MapJoin<T,PropertyField,KeyField>(Source,ChangeTableMap,WriteQuery);
            WriteQuery.putKeyWhere(ValueKeys);
            WriteQuery.add(WriteJoin.InJoin);
        ...
        JoinExpr getExpr(Map<T,? extends SourceExpr> JoinImplement, int Value) {
            ...
            if(Value==1 && Type==2) {
                ...
                Join<T,PropertyField> ChangeJoin = new UniJoin<T,PropertyField>(Source,DiffQuery);
                DiffQuery.add(ChangeTable.Value,SourceExpr.getDiff(ChangeJoin.Exprs.get(ChangeTable.Value),
                                ChangeJoin.Exprs.get(ChangeTable.PrevValue)));
                DiffQuery.add(ChangeJoin.InJoin);
...
class DataProperty<D extends PropertyInterface> extends Property<DataPropertyInterface> {
    ...
    Change incrementChanges(DataSession Session, int ChangeType) {
            ...
            if(!OnDefaultChange) {
                ...
                for(DataPropertyInterface Interface : Interfaces)
                    if(Session.Changes.AddClasses.contains(Interface.Class)) {
                        ...
                        Query.add(ChangeTable.Value, DefaultExpr);
                        Query.add(DefaultExpr.getWhere());
                ...
                if(Session.PropertyChanges.containsKey(DefaultProperty)) {
                    ...
                    // new, не равно prev
                    Query.add(new CompareWhere(NewExpr,DefaultChange.getExpr(JoinImplement,2),CompareWhere.EQUALS).not());
        ...
        if(DataChanged) {
            ...
            // GetChangedFrom
            Join<KeyField,PropertyField> DataJoin = new MapJoin<KeyField,PropertyField,DataPropertyInterface>(DataTable,DataTableMap,DataQuery);
            DataJoin.Joins.put(DataTable.Property,new ValueExpr(ID,DataTable.Property.Type));
            DataQuery.add(DataJoin.InJoin);
        ...
        for(DataPropertyInterface RemoveInterface : Interfaces) {
            if(Session.Changes.RemoveClasses.contains(RemoveInterface.Class)) {
                ...
                Query.add(ChangeTable.Value,new ValueExpr(null,ChangeTable.Value.Type));
                Query.add(getSourceExpr(Query.MapKeys,RemoveClassSet).getWhere());
...
/*        if(caption.equals("Дата 8") || caption.equals("OL 269")) {
...
//            Session = Session;
    ...
    void reCalculateAggregation(DataSession Session) throws SQLException {
        ...
        ReCalculateQuery.add(WriteField, ReCalculateExpr);
        ReCalculateQuery.add(ReCalculateExpr.getWhere());
        ...
        WriteQuery.add(WriteField,(new MapJoin<T,PropertyField,KeyField>(ReCalculateQuery,WriteQuery,MapTable).Exprs.get(WriteField)));
...
class ClassProperty extends AggregateProperty<DataPropertyInterface> {
    ...
    Change incrementChanges(DataSession Session, int ChangeType) {
        ...
        for(DataPropertyInterface ChangedInterface : RemoveInterfaces) {
            ...
            Query.add(ChangeTable.Value,new ValueExpr(null,ChangeTable.Value.Type));
            Query.add(ChangeTable.PrevValue,new ValueExpr(Value,ChangeTable.PrevValue.Type));
        ...
        while(il.hasNext()) {
            ...
            Query.add(ChangeTable.PrevValue,new ValueExpr(null,ChangeTable.PrevValue.Type));
            Query.add(ChangeTable.Value,new ValueExpr(Value,ChangeTable.Value.Type));
    ...
    SourceExpr calculateSourceExpr(Map<DataPropertyInterface, ? extends SourceExpr> JoinImplement, InterfaceClassSet<DataPropertyInterface> JoinClasses) {
        ...
        if(Value==null) // если Value null закинем false по сути в запрос
            ...
            for(DataPropertyInterface ValueInterface : Interfaces)
                joinObjects(Query,ValueInterface);
        Query.add(ValueString,new ValueExpr(Value,ValueClass.getType()));
...
class JoinProperty<T extends PropertyInterface> extends MapProperty<JoinPropertyInterface,T,JoinPropertyInterface,T,PropertyField> {
    ...
    SourceExpr calculateSourceExpr(Map<JoinPropertyInterface,? extends SourceExpr> JoinImplement, InterfaceClassSet<JoinPropertyInterface> JoinClasses) {
        ...
        Query.add(JoinValue, Implements.Property.getSourceExpr(ImplementSources, ImplementClasses));
...
abstract class GroupProperty<T extends PropertyInterface> extends MapProperty<GroupPropertyInterface<T>,T,T,GroupPropertyInterface<T>,Object> {
    ...
    void fillChangedRead(UnionQuery<T, Object> ListQuery, Object Value, MapChangedRead<T> Read, ValueClassSet<GroupPropertyInterface<T>> ReadClasses) {
        ...
        for(GroupPropertyInterface<T> Interface : Interfaces) {
            ...
            Query.add(Interface, ImplementExpr);
    ...
    SourceExpr calculateSourceExpr(Map<GroupPropertyInterface<T>,? extends SourceExpr> JoinImplement, InterfaceClassSet<GroupPropertyInterface<T>> JoinClasses) {
        ...
        JoinQuery<T,Object> Query = new JoinQuery<T,Object>(GroupProperty.Interfaces);
        for(GroupPropertyInterface<T> Interface : Interfaces)
            Query.add(Interface,Interface.Implement.mapSourceExpr(Query.MapKeys,ImplementClasses));
        Query.add(GroupValue, GroupProperty.getSourceExpr(Query.MapKeys, ImplementClasses));
...
class MaxGroupProperty<T extends PropertyInterface> extends GroupProperty<T> {
    ...
    Change incrementChanges(DataSession Session, int ChangeType) throws SQLException {
        ...
        UniJoin<GroupPropertyInterface<T>,PropertyField> ChangeJoin = new UniJoin<GroupPropertyInterface<T>,PropertyField>(ChangeQuery,SuspiciousQuery);
        ...
        Where.or(CompareWhere.get(PrevValue,NewValue,CompareWhere.LESS));
        Where.or(CompareWhere.get(PrevValue,OldValue,CompareWhere.EQUALS));
        Where.or(NewValue.getWhere().and(PrevValue.getWhere().not()));
        SuspiciousQuery.add(Where);
        ...
        Join<GroupPropertyInterface<T>,PropertyField> SourceJoin = new UniJoin<GroupPropertyInterface<T>,PropertyField>(IncrementChange.Source,ReReadQuery);

        NewValue = SourceJoin.Exprs.get(ChangeTable.Value);
        Where = new OrWhere();
        ReReadQuery.add(SourceJoin.InJoin.and(NewValue.getWhere().not())); // новое null и InJoin
        ReReadQuery.add(new CompareWhere(NewValue,SourceJoin.Exprs.get(ChangeTable.PrevValue),CompareWhere.LESS));
        ReReadQuery.add(Where);
        ...
        if(!ReReadQuery.readEmpty(Session)) {
            ...
            UpdateQuery.add(new MapJoin<GroupPropertyInterface<T>,PropertyField,KeyField>(ReReadQuery,ChangeTableMap,UpdateQuery).InJoin);
            ...
            UpdateQuery.add(ChangeTable.Value,(new MapJoin<GroupPropertyInterface<T>,PropertyField,KeyField>(getGroupQuery(NewRead,ChangeTable.Value,NewClass),ChangeTableMap,UpdateQuery)).Exprs.get(ChangeTable.Value));
    ...
    SourceExpr calculateSourceExpr(Map<PropertyInterface,? extends SourceExpr> JoinImplement, InterfaceClassSet<PropertyInterface> JoinClasses) {
        ...
        for(PropertyMapImplement<PropertyInterface,PropertyInterface> Operand : Operands) {
            if(Operand.mapIsInInterface(JoinClasses)) {
                ...
                Query.add(ValueString,Operand.mapSourceExpr(Query.MapKeys,JoinClasses));
    ...
    Change incrementChanges(DataSession Session, int ChangeType) {
        ...
        if(Operator==1 && ChangeType==1) {
            ...
            for(PropertyMapImplement<PropertyInterface,PropertyInterface> Operand : getChangedProperties(Session)) {
                ...
                Query.add(ChangeTable.Value, ChangeExpr);
                Query.add(ChangeExpr.From.InJoin);
                ResultQuery.add(Query,Coeffs.get(Operand));
        ...
        while(il.hasNext()) {
            ...
            for(PropertyMapImplement<PropertyInterface, PropertyInterface> Operand : Operands)
                if(ChangedProps.contains(Operand))
                    ResultOperands.add(SourceExpr.coeff(Operand.mapChangeExpr(Session, Query.MapKeys, MapType),Coeffs.get(Operand)));
                    ...
                    if(!LeftClass.isEmpty()) {
                        ResultOperands.add(SourceExpr.coeff(Operand.mapSourceExpr(Query.MapKeys, LeftClass.getClassSet(ClassSet.universal)),Coeffs.get(Operand)));
                        // значит может изменится Value на другое значение
                        ChangeClass.or(LeftClass);
                    }
                }

            Query.add(Value,UnionQuery.getExpr(ResultOperands,Operator));

            ResultQuery.add(Query,1);
            ResultClass.or(ChangeClass);
        }
        if(ResultQuery.Joins.isEmpty())
            ResultQuery.add(Value,new ValueExpr(null,getType()));
...
class StringFormulaProperty extends ValueFormulaProperty<StringFormulaPropertyInterface> {
    ...
    SourceExpr calculateSourceExpr(Map<StringFormulaPropertyInterface,? extends SourceExpr> JoinImplement, InterfaceClassSet<StringFormulaPropertyInterface> JoinClasses) {
        ...
        return FormulaExpr.get(Formula,Params,Value.getType());
    }
...
abstract class WhereFormulaProperty extends ValueFormulaProperty<FormulaPropertyInterface> {
    ...
    SourceExpr calculateSourceExpr(Map<FormulaPropertyInterface, ? extends SourceExpr> JoinImplement, InterfaceClassSet<FormulaPropertyInterface> JoinClasses) {
        return new CaseExpr(getWhere(JoinImplement),new ValueExpr(1,Type.Bit));
    }
...
class CompareFormulaProperty extends WhereFormulaProperty {
    ...
    Where getWhere(Map<FormulaPropertyInterface, ? extends SourceExpr> JoinImplement) {
        return CompareWhere.get(JoinImplement.get(Operator1),JoinImplement.get(Operator2),Compare);
    }
}
...
class ObjectFormulaProperty extends FormulaProperty<FormulaPropertyInterface> {
    ...
    SourceExpr calculateSourceExpr(Map<FormulaPropertyInterface,? extends SourceExpr> JoinImplement, InterfaceClassSet<FormulaPropertyInterface> JoinClasses) {
        ...
        return FormulaExpr.get("prm"+(ObjectInterface.ID+1),Params,Type.Object);
    }
...
class DataSession  {
    ...
    <T extends PropertyInterface> Object readProperty(Property<T> Property,Map<T,ObjectValue> Keys) throws SQLException {
        ...
        ReadQuery.add(ReadValue,getSourceExpr(Property,ReadQuery.MapKeys,new InterfaceClassSet<T>(new InterfaceClass<T>(Keys))));
    ...
    <P extends PropertyInterface> SourceExpr getSourceExpr(Property<P> Property, Map<P,? extends SourceExpr> JoinImplement, InterfaceClassSet<P> JoinClasses) {
        ...
        if(Change!=null && !Change.Classes.getClassSet(ClassSet.universal).and(JoinClasses).isEmpty()) {
            ...
            if(inInterface) {
                ...
                SourceQuery.add(Value, ValueExpr);
                SourceQuery.add(ValueExpr.getWhere());
            ...
            NewQuery.add(Value, ChangeExpr);
            NewQuery.add(ChangeExpr.From.InJoin);
        ...
        if(inInterface)
            ...
            return new ValueExpr(null,Property.getType());
    }
...
//            }
    ...
    void UpdateInsertRecord(Table Table,Map<KeyField,Integer> KeyFields,Map<PropertyField,Object> PropFields) throws SQLException {
        ...
        IsRecQuery.add(new Join<KeyField,PropertyField>(Table,KeyExprs).InJoin);
        ...
        if(IsRecQuery.executeSelect(this).size()>0) {
            ...
            for(Map.Entry<PropertyField,Object> MapProp : PropFields.entrySet())
                UpdateQuery.add(MapProp.getKey(),new ValueExpr(MapProp.getValue(),MapProp.getKey().Type));
...
class MapChangedRead<P extends PropertyInterface> extends MapRead<P> {
    ...
    <M extends PropertyInterface,OM> void fillMapExpr(JoinQuery<P, OM> Query, OM Value, Property<M> MapProperty, Map<M, ? extends SourceExpr> JoinImplement, Map<PropertyInterfaceImplement<P>, SourceExpr> ImplementExprs, InterfaceClassSet<M> JoinClasses) {
        if(MapChanged) {
            ...
            Query.add(MapExpr.From.InJoin);
            Query.add(Value,(MapType==3?new ValueExpr(null,MapExpr.getType()):MapExpr));
        } else {
            if(MapType==1) {
                // нужно закинуть не Changed'ы на notNull, а Changed'ы на InJoin
                for(Map.Entry<PropertyInterfaceImplement<P>,SourceExpr> ImplementExpr : ImplementExprs.entrySet())
                    if(ImplementChanged.contains(ImplementExpr.getKey()))
                        Query.add(((JoinExpr)ImplementExpr.getValue()).From.InJoin);
                    else
                        Query.add(ImplementExpr.getValue().getWhere());
            } else {
                SourceExpr MapExpr = MapProperty.getSourceExpr(JoinImplement,JoinClasses);
                Query.add(MapExpr.getWhere());
                Query.add(Value,(MapType==2?new ValueExpr(null,MapExpr.getType()):MapExpr));
            }
        }
    }
...
abstract class MapProperty<T extends PropertyInterface,M extends PropertyInterface,IN extends PropertyInterface,IM extends PropertyInterface,OM> extends AggregateProperty<T> {
    ...
    JoinQuery<IN, OM> getMapQuery(List<MapChangedRead<IN>> ReadList, OM Value, ValueClassSet<T> ReadClass, boolean Sum) {
        ...
        if(ListQuery.Joins.isEmpty()) {
            for(Map.Entry<OM,Type> NullProp : getMapNullProps(Value).entrySet())
                ListQuery.add(NullProp.getKey(),new ValueExpr(null,NullProp.getValue()));
            ListQuery.add(new OrWhere());
        }
