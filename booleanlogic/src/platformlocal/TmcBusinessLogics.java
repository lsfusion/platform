...
//        OstArtStore = AddUProp("остаток по складу",1,2,1,PrihArtStore,1,2,-1,RashArtStore,1,2);
        ...
        extIncDate = AddDProp(baseGroup, "Дата 1", Class.date, extIncomeDocument);
        intraDate = AddDProp(baseGroup, "Дата 2", Class.date, intraDocument);
        extOutDate = AddDProp(baseGroup, "Дата 3", Class.date, extOutcomeDocument);
        exchDate = AddDProp(baseGroup, "Дата 4", Class.date, exchangeDocument);
        revalDate = AddDProp(baseGroup, "Дата 5", Class.date, revalDocument);

        primDocDate = AddUProp(paramsGroup, "Дата 6", 2, 1, 1, datePrimDocument, 1, 1, extIncDate, 1, 1, intraDate, 1, 1, revalDate, 1);
        secDocDate = AddUProp("Дата 7", 2, 1, 1, extOutDate, 1, 1, exchDate, 1);

        docDate = AddUProp("Дата 8", 2, 1, 1, dateDocument, 1, 1, secDocDate, 1, 1, primDocDate, 1);
...
/*        Persistents.add((AggregateProperty)storePriceIn.Property);
    ...
    private class TmcNavigatorForm extends NavigatorForm {
        ...
        void addArticleRegularFilterGroup(PropertyObjectImplement documentProp, Object documentValue, PropertyObjectImplement... extraProps) {
            ...
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  new Filter(documentProp, CompareWhere.NOT_EQUALS, new UserValueLink(documentValue)),
                                  "Документ",
            ...
            for (PropertyObjectImplement extraProp : extraProps) {
                filterGroup.addFilter(new RegularFilter(IDShift(1),
                                      new Filter(extraProp, CompareWhere.NOT_EQUALS, new UserValueLink(0)),
    ...
    private class ExtIncDetailNavigatorForm extends ExtIncDocumentNavigatorForm {
        ...
        public ExtIncDetailNavigatorForm(int ID, String caption) {
            ...
            addFixedFilter(new Filter(detDocument, CompareWhere.EQUALS, new ObjectValueLink(objDoc)));
        }
    }
...
//            addPropertyView(Properties, artgrGroup, objArtTo);
            ...
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  ...
                                  KeyStroke.getKeyStroke(KeyEvent.VK_F11, 0)));
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  new Filter(getPropertyView(exchIncQuantity.Property).View, CompareWhere.NOT_EQUALS, new UserValueLink(0)),
                                  "Приход",
                                  KeyStroke.getKeyStroke(KeyEvent.VK_F10, 0)));
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  new Filter(getPropertyView(exchOutQuantity.Property).View, CompareWhere.NOT_EQUALS, new UserValueLink(0)),
                                  "Расход",
                                  KeyStroke.getKeyStroke(KeyEvent.VK_F9, 0)));
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  new Filter(getPropertyView(docOutBalanceQuantity.Property, gobjArtTo).View, CompareWhere.NOT_EQUALS, new UserValueLink(0)),
                                  "Остаток",
                                  KeyStroke.getKeyStroke(KeyEvent.VK_F8, 0)));
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  new Filter(getPropertyView(docOutBalanceQuantity.Property, gobjArtTo).View, CompareWhere.LESS, new UserValueLink(0)),
                                  "Отр. остаток",
            ...
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  new Filter(getPropertyView(exchangeQuantity.Property).View, CompareWhere.NOT_EQUALS, new UserValueLink(0)),
                                  "Документ",
                                  KeyStroke.getKeyStroke(KeyEvent.VK_F10, InputEvent.SHIFT_DOWN_MASK)));
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  new Filter(getPropertyView(docOutBalanceQuantity.Property, gobjArtFrom).View, CompareWhere.NOT_EQUALS, new UserValueLink(0)),
                                  "Остаток",
                                  KeyStroke.getKeyStroke(KeyEvent.VK_F8, InputEvent.SHIFT_DOWN_MASK)));
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  new Filter(getPropertyView(docOutBalanceQuantity.Property, gobjArtFrom).View, CompareWhere.GREATER, new UserValueLink(0)),
                                  "Пол. остаток",
                                  KeyStroke.getKeyStroke(KeyEvent.VK_F7, InputEvent.SHIFT_DOWN_MASK)));
            filterGroup.addFilter(new RegularFilter(IDShift(1),
                                  new Filter(getPropertyView(docOverPriceOut.Property, gobjArtFrom).View, CompareWhere.EQUALS, new PropertyValueLink(getPropertyView(docOverPriceOut.Property, gobjArtTo).View)),
...
//            addPropertyView(Properties, artgrGroup, objArt);
            ...
            if (groupStore) {
                ...
                addFixedFilter(new Filter(addPropertyObjectImplement(revalStore, objDoc), CompareWhere.EQUALS, new ObjectValueLink(objStore)));
            } else {
                addPropertyView(Properties, storeGroup, false, objDoc);
            }
...
//            addPropertyView(Properties, quantity, objPrimDoc, objArt);
            ...
            addFixedFilter(new Filter(getPropertyView(changesParams.Property).View, CompareWhere.NOT_EQUALS, new UserValueLink(false)));
            addFixedFilter(new Filter(getPropertyView(primDocStore.Property).View, CompareWhere.EQUALS, new ObjectValueLink(objStore)));
...
//            addPropertyView(dltDocStoreArticleSumAccount, objDoc, objStore, objArt);
            ...
            addFixedFilter(new Filter(addPropertyObjectImplement(isDocStoreArtInclude, objDoc, objStore, objArt), CompareWhere.EQUALS, new UserValueLink(true)));
    ...
    private class SupplierStoreArticleNavigatorForm extends TmcNavigatorForm {
        ...
        SupplierStoreArticleNavigatorForm(int ID, String caption) {
            ...
            addFixedFilter(new Filter(addPropertyObjectImplement(storeSupplier, objStore, objArt), CompareWhere.EQUALS, new ObjectValueLink(objSupplier)));
    ...
    private class MainAccountNavigatorForm extends DateIntervalNavigatorForm {
        ...
        public MainAccountNavigatorForm(int ID, String caption) {
            ...
            addFixedFilter(new Filter(getPropertyView(dltDocStoreSumAccount.Property).View, CompareWhere.NOT_EQUALS, new UserValueLink(0)));
            addFixedFilter(new Filter(getPropertyView(docDate.Property).View, CompareWhere.GREATER_EQUALS, new ObjectValueLink(objDateFrom)));
            addFixedFilter(new Filter(getPropertyView(docDate.Property).View, CompareWhere.LESS_EQUALS, new ObjectValueLink(objDateTo)));
