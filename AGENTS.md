...
- **server behaviour** — an action named `test*` with a caption, at the top level of a module under `tests/src/main/lsfusion/<theme>/` (`LsfIntegrationIT`, failsafe). Nothing is registered: modules come off the classpath, tests are found by name. Each is called through the external HTTP `/exec`, so it runs in a session of its own and its changes are dropped afterwards — a test that needs them kept has to `APPLY`. Assertions live in `TestUtils.lsf` and end in `fail(message)`, which is what the report shows.

```
mvn -pl tests test                        # the compile corpus only
mvn -pl tests verify                      # and the behaviour tests, which need postgres
mvn -pl tests verify -Dlsf.tests=forms/*  # one directory of them
mvn -pl tests verify -DskipITs            # skip them, e.g. in a full build without a database
```
...
- **Migration notes** (root `MIGRATION.md`): when a change carries a non-obvious regression risk — typically a non-local change near the core (query compilation and planning, apply/materialization semantics, locking) whose effects may surface only on production data volumes or in rare query/data shapes — add a section under the target version describing the symptoms an upgrading application may see (errors, performance degradations) and the quickest way back: the backward-compatibility setting when one exists (also document it in `docs/paradigm/{en,ru}/Working_parameters.md`), or the revert/workaround otherwise. Rules of thumb: if the change warranted a `...BackwardCompatibility` setting, or the commit body says something like "changes plans globally", `MIGRATION.md` needs a note; a local fix with an obvious blast radius doesn't.
