...
public class HOrderMap<K, V> extends AMWrapOrderMap<K, V, HMap<K, V>> {
    ...
    public ImOrderMap<K, V> immutableOrder() {
        ...
        if (!wrapMap.isStored()) {
            ...
            if (wrapMap.getIndexes().length > wrapMap.size() * SetFact.factorNotResize) {
                int[] newIndexes = new int[wrapMap.size()];
                System.arraycopy(wrapMap.getIndexes(), 0, newIndexes, 0, wrapMap.size());
                wrapMap.setIndexes(newIndexes);
            }
        }
        return this;
    }
    ...
    public ImOrderSet<K> keyOrderSet() {
        if (!wrapMap.isStored()) {
            return new HOrderSet<>(new HSet<>(wrapMap.size(), wrapMap.getTable(), wrapMap.getIndexes()));
        } else {
            return new HOrderSet<>(new HSet<>(wrapMap.size(), wrapMap.stored().getStoredKeys(), wrapMap.getIndexes()));
        }
    }
