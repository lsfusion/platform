...
public class RomanBusinessLogics extends BusinessLogics<RomanBusinessLogics> {
    ...
    protected void initProperties() {
        ...
        lessCmpDate = addJProp(and(false, true, false), object(DateClass.instance), 3, rateExchange, 1, 2, 3, addJProp(greater2, 3, date, 4), 1, 2, 3, 4, is(document), 4);
...
//        quantityListArticle.setDerivedForcedChange(orderedInvoiceArticle, 1, 2, numberListArticle, 1, 2);
        ...
        priceRateDocumentSku = addJProp(baseGroup, "priceRateDocumentSku", true, "Цена (конверт.)", round2, addJProp(multiplyDouble2, priceDocumentSku, 1, 2, addJProp(nearestRateExchange, typeExchangeSTX, currencyDocument, 1, 1), 1), 1, 2);
        ...
        diffListShipSku = addJProp(diff2, zeroQuantityListSku, 1, 3, zeroQuantityShipDimensionShipmentSku, 1, 2, 3);
        ...
        diffShipmentSku = addJProp(diff2, zeroQuantityShipmentSku, 1, 2, zeroInvoicedShipmentSku, 1, 2);
        ...
        oneShipmentArticleColor = addJProp(baseGroup, "oneShipmentArticleColor", "Первый артикул-цвет", equals2, quantityShipmentArticleColor, 1, 2, 3, addCProp(DoubleClass.instance, 1));
        oneShipmentArticleSize = addJProp(baseGroup, "oneShipmentArticleSize", "Первый артикул-размер", equals2, quantityShipmentArticleSize, 1, 2, 3, addCProp(DoubleClass.instance, 1));
        ...
        diffShipmentRouteSku = addJProp(greater2, zeroQuantityShipmentRouteSku, 1, 2, 3, zeroInvoicedShipmentRouteSku, 1, 2, 3);

        sumShipmentRouteSku = addJProp(baseGroup, "sumShipmentRouteSku", "Сумма", multiplyDouble2, invoicedShipmentRouteSku, 1, 2, 3, priceShipmentSku, 1, 3);
...
//                routeToFillShipmentBarcode, 1, 2);
        ...
        diffPalletFreight = addJProp(greater2, palletNumberFreight, 1, palletCountFreight, 1);
...
//        priceFreightImporterFreightSku = addJProp(baseGroup, "priceFreightImporterFreightSku", "Цена за фрахт", divideDouble2, sumFreightImporterFreightSku, 1, 2, 3, quantityImporterFreightSku, 1, 2, 3);
        ...
        sumImporterFreightSku = addJProp(baseGroup, "sumImporterFreightSku", "Сумма", multiplyDouble2, quantityImporterFreightSku, 1, 2, 3, priceInOutImporterFreightSku, 1, 2, 3);
        ...
        currentPalletRouteUser = addDProp("currentPalletRouteUser", "Тек. паллета (ИД)", pallet, route, user);

        currentPalletRoute = addJProp(true, "currentPalletRoute", "Тек. паллета (ИД)", currentPalletRouteUser, 1, currentUser);
        ...
        currentFreightBoxRouteUser = addDProp("currentFreightBoxRouteUser", "Тек. короб (ИД)", freightBox, route, user);

        currentFreightBoxRoute = addJProp(true, "currentFreightBoxRoute", "Тек. короб (ИД)", currentFreightBoxRouteUser, 1, currentUser);
...
//            addActionsOnObjectChange(objBarcode, addPropertyObject(barcodeNotFoundMessage, objBarcode));
    ...
    private class FreightShipmentFormEntity extends FormEntity<RomanBusinessLogics> {
        ...
        private FreightShipmentFormEntity(NavigatorElement parent, String sID, String caption) {
            ...
            objFreight = addSingleGroupObject(freight, "Фрахт", date, objectClassName, nameRouteFreight, nameFreightTypeFreight, tonnageFreight, grossWeightFreight, volumeFreight, palletCountFreight, palletNumberFreight);
    ...
    private class CustomCategoryFormEntity extends FormEntity<RomanBusinessLogics> {
        ...
        private CustomCategoryFormEntity(NavigatorElement parent, String sID, String caption, boolean tree) {
            ...
            if (tree) {
                addPropertyDraw(dumb1, objCustomCategory4Origin);
            ...
            if (tree) {
                addPropertyDraw(dumb1, objCustomCategory6Origin);
    ...
    private class FreightChangeFormEntity extends FormEntity<RomanBusinessLogics> {
        ...
        private FreightChangeFormEntity(NavigatorElement parent, String sID, String caption, boolean edit) {
            ...
            setForceViewType(itemAttributeGroup, ClassViewType.GRID, objSku.groupTo);
            addPropertyDraw(addGCAProp(actionGroup, "translationAllMainComposition", "Перевод составов", objSku.groupTo, translationMainCompositionSku, actionTrue), objSku).forceViewType = ClassViewType.PANEL;
            addPropertyDraw(addGCAProp(actionGroup, "translationAllAdditionalComposition", "Перевод доп. составов", objSku.groupTo, translationAdditionalCompositionSku, actionTrue), objSku).forceViewType = ClassViewType.PANEL;
    ...
    public class AnnexInvoiceFormEntity extends FormEntity<RomanBusinessLogics> {
        ...
        private AnnexInvoiceFormEntity(NavigatorElement parent, String sID, String caption, boolean translate) {
            ...
            addPropertyDraw(objFreight, date, objectClassName, nameCurrencyFreight);
            ...
            addPropertyDraw(objCountry, sidCountry);
            if (!translate)
