    ...
    public Collection<InnerGroupJoin<? extends GroupJoinSet>> getInnerJoins(boolean noJoins, Set<KeyExpr> keys) {
        ...
        for(int i=0;i<size;i++) {
            ...
            if(noJoins) { // группируем по enoughKeys
                MapWhere<Set<KeyExpr>> enoughWheres = objectJoinSets.compileEnough(keys);
                for(int j=0;j<enoughWheres.size;j++)
                    result.add(new InnerGroupJoin<EnoughKeys>(keyEqual, new EnoughKeys(enoughWheres.getKey(j)), where));
