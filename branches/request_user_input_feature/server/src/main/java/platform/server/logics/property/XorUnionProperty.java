...
public class XorUnionProperty extends IncrementUnionProperty {
    ...
    public XorUnionProperty(String sID, String caption, List<Interface> interfaces, List<CalcPropertyMapImplement<?, Interface>> operands) {
    ...
    private final List<CalcPropertyMapImplement<?,Interface>> operands; // list нужен чтобы порядок редактирования был
    ...
    protected DataChanges calculateDataChanges(PropertyChange<Interface> change, WhereBuilder changedWhere, PropertyChanges propChanges) {
        ...
        for(CalcPropertyMapImplement<?, Interface> operand : BaseUtils.reverse(operands)) {
            ...
            for(CalcPropertyMapImplement<?, Interface> siblingOperand : operands) // считаем where сиблингов и потом ими xor'им change
