...
public class ClientContainer extends ClientComponent {
    ...
    public boolean isAlignCaptions() {
        ...
        for(ClientComponent child : children) {
            if(!(child instanceof ClientPropertyDraw) || ((ClientPropertyDraw) child).hasColumnGroupObjects() || (((ClientPropertyDraw)child).valueHeight == -1 && ((ClientPropertyDraw) child).isAutoDynamicHeight()) || child.flex > 0 || ((ClientPropertyDraw) child).panelCaptionVertical)
                return false;
