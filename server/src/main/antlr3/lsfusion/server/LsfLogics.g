...
@after {
	if (inPropParseState()) {
		if($type.format == ImportSourceFormat.XLS)
        	...
        	$property = self.addScriptedImportXMLActionProperty($expr.property, $plist.ids, $plist.propUsages, root, list, attr);
	...
	:	'IMPORT' 
		type = importSourceFormat [context, dynamic] { format = $type.format; sheet = $type.sheet; memo = $type.memo; separator = $type.separator;
		        noHeader = $type.noHeader; root = $type.root; list = $type.list; attr = $type.attr; charset = $type.charset; }
...
importSourceFormat [List<TypedParameter> context, boolean dynamic] returns [ImportSourceFormat format, LPWithParams sheet, LPWithParams memo, String separator, boolean noHeader, String charset, boolean list, LPWithParams root, boolean attr]
	...
	|	'XML'	{ $format = ImportSourceFormat.XML; } ('ROOT' rootProperty = propertyExpression[context, dynamic] {$root = $rootProperty.property; })? ('LIST' { $list = true; }|'TABLE')? ('ATTR' { $attr = true; })?
...
@after {
	if (inPropParseState()) {
		...
		$property = self.addScriptedFileAProp(actionType, fileProp, isAbsolutPath ? filePathProp : fileNameProp, isAbsolutPath, noDialog);
	}
}
