...
//            }
    ...
    public void setLazy(LAP property, ActionOrProperty.Lazy lazy) throws ScriptingErrorLog.SemanticErrorException {
        ...
        if (property instanceof LP && property.getActionOrProperty().isLazyStrong()) {
            addScriptedWhen(new LPWithParams(addCHProp((LP<?>) property, IncrementType.CHANGED, PrevScope.EVENT), property.listInterfaces.size()),
                    new LAWithParams(new LA<>(new SystemAction(LocalizedString.NONAME, SetFact.EMPTYORDER()) {
                        @Override
                        protected FlowResult aspectExecute(ExecutionContext<PropertyInterface> context) {
                            context.getDbManager().flushStrong(((LP<?>) property).property);
                            return FlowResult.FINISH;
                        }
                    }), new ArrayList<>()), new ArrayList<>(), false, Event.APPLY, new ArrayList<>(), false, null, null);
        }
    }
