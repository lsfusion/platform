...
public class AsyncMapInputListAction<T extends PropertyInterface> {
    ...
    public AsyncMapInputListAction(SerializableImageIconHolder action, String id, AsyncMapEventExec<T> asyncExec, String keyStroke, Map<String, BindingMode> bindingModesMap, Integer priority, ImList<QuickAccess> quickAccessList) {
        this.action = action;
        this.id = id;
    ...
    public InputListAction map() {
        return new InputListAction(action, id, Action.getAsyncExec(asyncExec), keyStroke, bindingModesMap, priority, quickAccessList);
    }
    ...
    public InputListAction map(ImRevMap<T, ObjectEntity> mapObjects, FormEntity form, GroupObjectEntity toDraw) {
        ...
        return new InputListAction(action, id, mappedAsyncExec, keyStroke, bindingModesMap, priority, quickAccessList);
    }

    public <P extends PropertyInterface> AsyncMapInputListAction<P> map(ImRevMap<T, P> mapping) {
        return new AsyncMapInputListAction<>(action, id, asyncExec != null ? asyncExec.map(mapping) : null, keyStroke, bindingModesMap, priority, quickAccessList);
    }

    public <P extends PropertyInterface> AsyncMapInputListAction<P> mapInner(ImRevMap<T, P> mapping) {
        return new AsyncMapInputListAction<>(action, id, asyncExec != null ? asyncExec.mapInner(mapping) : null, keyStroke, bindingModesMap, priority, quickAccessList);
    }

    public <P extends PropertyInterface> AsyncMapInputListAction<P> mapJoin(ImMap<T, PropertyInterfaceImplement<P>> mapping) {
        return new AsyncMapInputListAction<>(action, id, asyncExec != null ? asyncExec.mapJoin(mapping) : null, keyStroke, bindingModesMap, priority, quickAccessList);
    }
    ...
    public AsyncMapInputListAction<T> replace(String replaceAction, AsyncMapEventExec<T> asyncExec) {
        if(id != null && id.equals(replaceAction))
            return new AsyncMapInputListAction<>(action, id, asyncExec, keyStroke, bindingModesMap, priority, quickAccessList);
        return this;
    }
}
