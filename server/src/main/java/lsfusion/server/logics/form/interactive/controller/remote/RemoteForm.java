...
public class RemoteForm<F extends FormInstance> extends RemoteRequestObject implements RemoteFormInterface {
    ...
    public Object getTreeGroupReportData(long requestIndex, long lastReceivedRequestIndex, final int groupId, final FormUserPreferences userPreferences) throws RemoteException {
        return processRMIRequest(requestIndex, lastReceivedRequestIndex, stack -> {
            ...
            RawFileData file = TreeReportGenerator.generate(groupEntity.treeGroup, exportData, form);
...
//            form.moveGroupObject(parentGroup, deserializeGroupObjectKeys(parentGroup, parentKey));
    ...
    public ServerResponse setUserHidden(long requestIndex, long lastReceivedRequestIndex, int componentID, boolean hidden) throws RemoteException {
        return processPausableRMIRequest(requestIndex, lastReceivedRequestIndex, stack -> {
            ...
            if (component instanceof ContainerView) // a hidden container — collapsed OR react-placed lsf view — fires COLLAPSE, shown fires EXPAND
...
//            throw Throwables.propagate(e);
    ...
    // here we supply only the form-specific execution context. See GFORM-CONTROLLER-EXEC-EVAL-PLAN §5/§12.
    @Override
    ...
    protected ClientAction runControllerRequest(ExecutionStack stack, ControllerBody body) throws SQLException, SQLHandledException, ParseException, IOException {
        return body.run((ExecutionEnvironment) form, getRemoteContext());
    }
