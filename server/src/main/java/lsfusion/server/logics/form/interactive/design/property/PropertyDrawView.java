...
public class PropertyDrawView extends BaseComponentView {
    ...
    public String getDrawCaption() {
        ...
        if(hasNoCaption(caption, entity.getPropertyExtra(CAPTION), elementClass, elementAttr))
            return null;

        return ThreadLocalContext.localize(caption);
    }

    public static boolean hasNoCaption(LocalizedString caption, PropertyObjectEntity<?> propertyCaption, String elementClass, String elementAttr) {
        return ((caption == null || (caption.isEmpty() && elementClass == null && elementAttr == null)) && propertyCaption == null) || (propertyCaption != null && propertyCaption.property.isExplicitNull()); // isEmpty can be better, but we just want to emulate NULL to be like NULL caption
    }
