...
import lsfusion.server.logics.form.stat.integration.export.hierarchy.json.ExportJSONAction;
import lsfusion.server.logics.form.stat.integration.export.hierarchy.xml.ExportXMLAction;
import lsfusion.server.logics.form.stat.integration.export.plain.csv.ExportCSVAction;
import lsfusion.server.logics.form.stat.integration.export.plain.dbf.ExportDBFAction;
import lsfusion.server.logics.form.stat.integration.export.plain.table.ExportTableAction;
import lsfusion.server.logics.form.stat.integration.export.plain.xls.ExportXLSAction;
import lsfusion.server.logics.form.stat.integration.imports.hierarchy.json.ImportJSONAction;
import lsfusion.server.logics.form.stat.integration.imports.hierarchy.xml.ImportXMLAction;
import lsfusion.server.logics.form.stat.integration.imports.plain.csv.ImportCSVAction;
import lsfusion.server.logics.form.stat.integration.imports.plain.dbf.ImportDBFAction;
import lsfusion.server.logics.form.stat.integration.imports.plain.table.ImportTableAction;
import lsfusion.server.logics.form.stat.integration.imports.plain.xls.ImportXLSAction;
...
//        addProperty(null, lcp);
    ...
    protected <D extends PropertyInterface> LP addDCProp(AbstractGroup group, LocalizedString caption, int whereNum, LP<D> derivedProp, Object... params) {
        ...
        LP derDataProp = addProperty(group, new LP<>(dataProperty));

        derDataProp.setEventChange(derivedProp, whereNum, params);
        return derDataProp;
    }

    protected <D extends PropertyInterface> LP addLogProp(AbstractGroup group, LocalizedString caption, int whereNum, LP<D> derivedProp, Object... params) {
        Pair<ValueClass[], ValueClass> signature = getSignature(derivedProp, whereNum, params);

        // выполняем само создание свойства
        StoredDataProperty dataProperty = new StoredDataProperty(caption, signature.first, LogicalClass.instance);
        return addProperty(group, new LP<>(dataProperty));
    }
    ...
    protected LP addTProp(LocalizedString caption, Time time) {
        return addProperty(null, new LP<>(new TimeFormulaProperty(caption, time)));
    }
...
//    protected static <T extends PropertyInterface<T>> AggregateGroupProperty create(String sID, LocalizedString caption, Property<T> property, T aggrInterface, Collection<PropertyMapImplement<?, T>> groupProps) {
    ...
    public LP addLDropProp(SystemEventsLogicsModule systemEventsLM, LP lp) {
        ...
        LP logDropProperty = addLogProp(baseLM.privateGroup, LocalizedString.create("{logics.log}" + " " + lp.property + " {drop}"), 1, lp, add(new Object[]{equalsProperty, lp.listInterfaces.size() + 1}, directLI(lp)));
