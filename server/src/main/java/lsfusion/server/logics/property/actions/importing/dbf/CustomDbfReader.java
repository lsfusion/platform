...
public class CustomDbfReader implements Closeable {
    ...
    public CustomDbfReader(File dbfFile) throws IOException {
        this(new FileInputStream(dbfFile));
    }

    public CustomDbfReader(File dbfFile, File memoFile) throws IOException {
        this.dbfInputStream = new BufferedInputStream(new FileInputStream(dbfFile), BUFFER_SIZE);
        this.memoReader = new CustomMemoReader(new FileInputStream(memoFile), memoFile);
        readMetadata();
    }

    public CustomDbfReader(InputStream dbfInputStream, File memoFile) throws IOException {
    ...
    public CustomDbfReader(InputStream dbfInputStream) throws IOException {
        this.dbfInputStream = new BufferedInputStream(dbfInputStream, BUFFER_SIZE);
        readMetadata();
    }
