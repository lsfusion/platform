...
public abstract class RemoteConnection extends RemoteRequestObject implements RemoteConnectionInterface {
            ...
            return logFromExternalSystemRequest(() -> {
                if(actionName != null) {
                    ...
                    while (true) { // we're doing greedy search for all subpathes to find appropriate "endpoint" action
                        if ((action = businessLogics.findActionByName(findActionName)) != null)
                            break;
    ...
    /** Aspect-wrapped entry point for {@code RemoteLogics.access(...)}: the call triggers
     *  {@code RemoteContextAspect}, which binds the caller's role-clone Settings before
     *  {@code checkAPIAccess} reads {@code enableAPI}. Returns null — only the side effect
