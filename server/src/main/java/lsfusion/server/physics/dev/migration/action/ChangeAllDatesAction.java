...
public class ChangeAllDatesAction extends InternalAction {
    ...
    public ChangeAllDatesAction(ScriptingLogicsModule LM) {
        super(LM);
    }
    ...
    private void changeAllDates(ExecutionContext<ClassPropertyInterface> context) throws SQLException, SQLHandledException {

        SQLSession sql = context.getSession().sql;
        try {
            sql.pushNoReadOnly();
            
            context.newSession(false, newContext -> { // deferInTransaction = false : raw DDL updates under pushNoReadOnly, whose scope ends right after (finally below)
                try {
