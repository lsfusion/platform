...
public interface WhereJoin<K, T extends WhereJoin<K, T>> extends BaseJoin<K>, OuterContext<T> {
    ...
    // rebuilds this join with its expressions translated (see WhereJoins.removeJoin); returns null if this join type cannot be rebuilt (the caller falls back)
    // NOT translateExpr : that name is already taken on the implementations (see Table.Join.translateExpr - the query build variant with a different contract)
    default T translateExprJoin(ExprTranslator translator) {
        return null;
    }
}
