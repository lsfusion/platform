...
public class SubQueryContext extends TwinImmutableObject {
    ...
    public final static SubQueryContext EMPTY = new SubQueryContext(0, 0, 0, 0);
    ...
    private SubQueryContext(int alias, int recursion, int siblingSubQuery, int subQuery) {
        this.alias = alias;
    ...
    public SubQueryContext pushAlias(int shift) {
        ...
        return new SubQueryContext(alias * MAX_PUSH + shift, recursion, siblingSubQuery, subQuery);
    }

    public SubQueryContext pushRecursion() {
        return new SubQueryContext(alias, recursion + 1, siblingSubQuery, subQuery);
    }

    public SubQueryContext pushSiblingSubQuery() {
        return new SubQueryContext(alias, recursion, siblingSubQuery + 1, subQuery);
    }

    public SubQueryContext pushSubQuery() {
        return new SubQueryContext(alias, recursion, siblingSubQuery, subQuery + 1);
    }

    public boolean calcTwins(TwinImmutableObject o) {
        return alias == ((SubQueryContext)o).alias && recursion == ((SubQueryContext)o).recursion  && siblingSubQuery == ((SubQueryContext)o).siblingSubQuery && getSubQueryDepth() == ((SubQueryContext)o).getSubQueryDepth();
    }

    public int immutableHashCode() {
        return 31 * ((alias * 31 + recursion) * 31 + siblingSubQuery) + getSubQueryDepth().hashCode();
    }
}
