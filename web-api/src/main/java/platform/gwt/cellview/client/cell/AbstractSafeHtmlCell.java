...
public abstract class AbstractSafeHtmlCell<C> extends AbstractCell<C> {
    ...
    @Override
    public final void renderDom(Context context, DivElement cellElement, C value) {
        SafeHtmlBuilder sb = new SafeHtmlBuilder();
        render(context, value, sb);
        cellElement.setInnerSafeHtml(sb.toSafeHtml());
    }

    @Override
    public final void updateDom(Context context, DivElement cellElement, C value) {
        renderDom(context, cellElement, value);
    }

    public final void render(Context context, C data, SafeHtmlBuilder sb) {
        if (data == null) {
