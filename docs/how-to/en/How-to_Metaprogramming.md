...
You may often need to write "similar" code for certain cases. The [`META` statement](../language/META_statement.md) exists for this purpose, and makes it possible to create a code template or *metacode*. It can contain parameters that will be replaced by certain values when this metacode is used. Such an approach is called [metaprogramming](../paradigm/Metaprogramming.md).
