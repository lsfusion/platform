...
The [`INPUT` operator](../language/INPUT_operator.md) which requests a file will display a dialog where the user will be able to choose an .xls or .xlsx file. Once the file is selected successfully, the system will call the [action](../paradigm/Actions.md) specified after `DO`.
...
Each file is processed in a separate new [change session](../paradigm/Change_sessions.md) and then is saved using the [`APPLY` operator](../language/APPLY_operator.md). This operator writes `TRUE` to the `canceled` property when a certain [constraint](../paradigm/Constraints.md) has been violated. Then, the `MOVE` statement of the `READ` operator moves the file either to `success` folder or `error` folder. This allows us to call the action again without processing the same orders several times.
...
To implement the import process, we need to declare the form of the [structure](../paradigm/Structured_view.md) matching the structure of the JSON file.
...
To import values from tags nested in the `item` tag, we create a new [group](../paradigm/Groups_of_properties_and_actions.md) called `item` and then place the properties and objects into it. In particular, the local property `idItem` is created and then added to the form in this group. Since the property name does not match the tag name, we specify the corresponding name for the property on the form using the `EXTID` keyword.
