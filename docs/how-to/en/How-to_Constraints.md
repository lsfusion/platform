...
Both options are identical from the execution perspective. If the platform does not find any [change operator](../paradigm/Change_operators_SET_CHANGED_etc.md) in a constraint, the entire expression is automatically "wrapped" into a `SET` operator.
