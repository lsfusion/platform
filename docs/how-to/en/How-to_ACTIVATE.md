...
After closing the form, we need to call the [`ACTIVATE` operator](../language/ACTIVATE_operator.md) which will make the added object active.
...
The property with the default customer is added to the `'Settings'` form on the `'General'` tab. The current object will change once the user opens the form, since the [`ON INIT` event](../language/Event_block.md) will be triggered.
...
Date properties can be found in the `Time` [system module](../paradigm/Modules.md) which is loaded at the very beginning using the `REQUIRE` statement.
...
On form open, the [`ON INIT`](../language/Event_block.md) event fires and the [`ACTIVATE` operator](../language/ACTIVATE_operator.md) moves focus to the `dFrom` field. `ACTIVATE PROPERTY` works for any property displayed on the form — panel, grid, and tree alike.
