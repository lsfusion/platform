...
The solution uses the [`FILTER` operator](../paradigm/Filter_FILTER.md), which returns `TRUE` if the object is included in the current filter on the form.
...
The property with the current order, defined using the [`ORDER` operator](../paradigm/Order_ORDER.md), is not declared as a separate named property but is used directly in the expression.
