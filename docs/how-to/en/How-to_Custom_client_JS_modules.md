...
Like plain bundling, this pass needs no Node and no other external toolchain: the compiler (Babel with `babel-plugin-react-compiler`) is vendored with the build plugin and runs in-process on the JVM. It is a build-time step only — a runtime / deploy box is unaffected. A module with JSX does require the build JVM to be Java 11 or newer; on an older JVM it fails with that guidance — a _.js_/_.ts_-only module builds regardless, or set `-Dlsfusion.web.skip=true` to skip the web compile entirely. An offline build (`mvn -o`) works as usual.
