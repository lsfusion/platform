...
Wherever a member takes a row it takes the row object, its `objects` handle, or the **key** string the projection gave it: the key is looked up in `byKey`, this projection's own index. [Row identity](How-to_Custom_view_controller.md#row-identity-contract) says what each of the three means and where a bare key is not enough.
