...
A property grouped in columns (`COLUMNS`) cannot be projected: its values are addressed by a row-and-column key, and everything here — a row of `list`, an entry of `byKey`, a name on the controller — means one row. So a group a React container projects may not draw one, and neither may a form-level property inside one: such a form is rejected when it is built, naming the property. Elsewhere on the form it stays as it is, and the controller, which names the whole form, refuses it by name — it has no `controller.<group>.<property>` member, and naming it in `properties.change` is an error rather than a write to whichever column happens to be current. The same holds whenever a name does not single out one draw — two properties drawn on one group under one integration name are neither of them addressable, because the projection would show one and a lookup would find the other; give one of them an explicit `EXTID`. Declare an ordinary property if the view has to read those values.
