...
To solve the task, create an action using the [`INTERNAL` operator](../language/INTERNAL_operator.md) which will generate an object of the `GetIP` class (if the class has a package, then you must also specify "package" in the class name) and will call the `executeInternal` method. The source code for this class will be as follows:
    ...
    private class ListenPortServer extends MonitorServer {
        ...
        private class ListenPortHandler extends IoHandlerAdapter {
            ...
            public void messageReceived(IoSession session, Object message) throws Exception {
                ...
                // monitor context is set up and torn down inside the callback:
                // MINA may dispatch messageReceived on any IoProcessor-pool thread,
                // so before/after must be in the same callback block.
                try {
