...
Форме, открываемой окном (`SHOW ... FLOAT`; у `DIALOG` окно — расположение по умолчанию), размер вычисляется по содержимому в момент открытия, когда компонент ещё ничего не нарисовал, поэтому контейнеру задают базовый размер: `size = (900, 600)` либо отдельные атрибуты `width` и `height`. Без него окно с одним таким контейнером схлопывается до заголовка и системных кнопок, а нарисованное позже содержимое вытесняет кнопки OK / Закрыть за край окна. Закладке (`DOCKED`) размер задаёт окно форм, поэтому там базовый размер не нужен.
