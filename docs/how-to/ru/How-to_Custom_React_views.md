...
Поэтому `MOVE` панельного свойства решает, кто его рисует: перенесённое в кастомный контейнер, оно проецируется туда и рисуется React — даже если строки группы рисует платформа в другом контейнере; вынесенное из него, оно возвращается платформе, которая рисует его там, где оно теперь стоит, и уходит из `data`. Каждое значение рисует ровно один из двух движков, и какой именно — говорит дизайн.
...
Везде, где член принимает строку, — `controller.<группа>.change` и явный объект у `controller.<группа>.<свойство>.change` — принимается строка, её `objects` или её **ключ**: член называет свою группу, поэтому ключ ищется среди её текущих строк, а не отвергается. Ищется он в индексе самой этой проекции — поэтому здесь ключ является строкой, а для классического представления на форме без React-контейнера не является. Именно это позволяет виду вернуть то, что он получил: `byKey` — JS-объект, и `Object.keys()` по нему даёт строки, так что прошедший через него ключ больше не несёт того типа, которым был записан `row.key`. Одно место, где ключа всё же недостаточно: `.change(X)` с одним аргументом решает по одному только `X`, строка это или значение, а голый ключ неотличим от значения — и читается как значение; сказать, что это строка, можно передав оба: `.change(ключ, значение)` (у действия слота значения нет, поэтому оно принимает ключ в любой форме).
...
| `data.<g>` | `options` — пользовательские опции группы; `properties` — имена того, что она рисует |
