...
`window.lsfusion.List` добавляет недостающую отмену перерисовки по строкам: каждая строка отрисовывается через стабильный компонент, обёрнутый в `React.memo`, и ей передаётся повторно используемая ссылка на строку, поэтому строка перерисовывается, когда меняется её ссылка (или другой переданный ей проп). Чтобы сделать то же вручную без `window.lsfusion.List`, объявите мемоизированный компонент строки один раз на уровне модуля и задавайте ключ `row.key`:
