...
Доступен более простой вариант `window.lsfusion.List` — `<List simple/>` или, глобально, `window.lsfusion.listSimple`. Он вместо этого отображает список и мемоизирует компонент строки, полагаясь на то, что проекция повторно использует ссылку на не изменившуюся строку; контракт компонента тот же.
