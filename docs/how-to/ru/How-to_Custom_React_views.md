...
Свойство, сгруппированное по колонкам (`COLUMNS`), спроецировать нельзя: его значения адресуются ключом «строка и колонка», а всё здесь — строка `list`, запись `byKey`, имя на контроллере — означает одну строку. Поэтому его не может рисовать ни группа, которую проецирует React-контейнер, ни форменное свойство внутри такого контейнера: такая форма отклоняется при построении с указанием свойства. В остальной части формы оно остаётся как есть, а контроллер, который называет всю форму, отказывает по имени: члена `controller.<группа>.<свойство>` у него нет, а назвать его в `properties.change` — ошибка, а не запись в ту колонку, которая случайно оказалась текущей. То же верно всегда, когда имя не выделяет один драв: два свойства, нарисованных на одной группе под одним интеграционным именем, недоступны оба — проекция показала бы одно, а поиск нашёл бы другое; задайте одному из них явный `EXTID`. Если представлению нужны эти значения, объявите обычное свойство.
