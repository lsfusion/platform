...
Везде, где член принимает строку, он принимает объект строки, её дескриптор `objects` либо строковый **ключ**, который дала ей проекция: ключ ищется в `byKey`, собственном индексе этой проекции. Что означает каждый из трёх и где голого ключа недостаточно, сказано в разделе [Идентификация строки](How-to_Custom_view_controller.md#row-identity-contract).
