...
`props.data` — проекция формы, своя у каждого контейнера `CUSTOM REACT`: в ней только то, чем владеет этот контейнер, поэтому второй React-контейнер формы получает отдельную проекцию. Каждое проецируемое свойство, группа и контейнер — это объект в `data`: значение читается как `.value`, его атрибуты — как соседние поля, а атрибуты колонки списочного свойства — из `data.<group>.<prop>`. Она содержит группу, когда представление рисует ЧАСТЬ этой группы, и содержит ровно те части, которые рисует. Каждая часть проецируется туда, где она находится: строки группы — туда, где её грид (или её дерево), а каждое её панельное свойство — туда, где находится само это свойство. Представление, которое рисует строки — грид лежит внутри кастомного контейнера и без `lsf`, так что строки рисует React из `data`, а не платформа, — получает `props.data.<g>` равное `{ list, byKey, keys, options, properties }` для этого SID группового объекта `g`, где `list` — массив строк в порядке отображения, `byKey` сопоставляет строковому ключу строки тот же объект строки, а `keys` — массив этих строковых ключей в том же порядке. Ключи `byKey` — это данные приложения, поэтому он создаётся БЕЗ прототипа: строка с ключом `__proto__` лежит в нём обычной записью, а `byKey.hasOwnProperty` и прочие методы `Object.prototype` вызвать не на чем — читайте через `byKey[k]`, а проверяйте через `k in byKey` или `Object.keys(byKey)`. Там, где представление рисует строки, `list`, `byKey` и `keys` присутствуют все: когда у группы нет строк — панельная группа или группа до прихода её первых строк — они пусты, поэтому представление читает `props.data.<g>.list` напрямую, без проверки на отсутствующее поле. Там, где не рисует, их нет вовсе, как нет и `options`: здесь нет строк, о которых они могли бы говорить. Группа, ни одной части которой представление не рисует, отсутствует в `props.data` целиком (`props.data.<g>` — `undefined`): контейнер, просто стоящий рядом с платформенным гридом, не получает ничего, как и тот, который лишь обрамляет его (`MOVE GRID(g) { lsf = TRUE; }` — там строки рисует платформа, поэтому контейнер получает [запись, называющую то, что он размещает](#lsf-child), и никаких строк). Запрос строк у `<List>`, когда их здесь нет, сообщается в консоль, а не оборачивается молчаливой отрисовкой пустоты. Панельные свойства группы являются полями узла группы в том контейнере, в котором находится само свойство, по интеграционному SID, а каждое свойство уровня формы (без группы) — полем `props.data` по своему интеграционному SID. Атрибуты колонки списочного свойства тоже являются полем группы, в `data.<g>.<prop>` — по одной записи на колонку, — а его построчное значение и атрибуты ячейки лежат в каждой строке. Действия проецируются так же, как свойства: действие, добавленное на группу, является полем каждой строки (списочное действие) или узла группы (панельное действие) — объектом своих атрибутов, как свойство, — поэтому `controller.<group>.<action>.change(row)` его выполняет; поле `value` у него тоже есть, но ничего не несёт. `options` — собственные [опции отображения](#display-options) группы, `properties` называет записи СВОЙСТВ, которые несёт этот узел, в порядке формы, — а не всё, что на узле лежит: там есть и то, что определяет сама проекция (`list`, `byKey`, `keys`, `options`), и это известно по имени, а не обнаруживается; число строк — это `list.length`. Каждая строка содержит:
...
Поэтому `MOVE` панельного свойства и решает, какой движок его рисует: внутри кастомного контейнера его рисует React по `data`, вне его — платформа, там, где оно теперь стоит, и из `data` оно уходит.

Свойство, сгруппированное по колонкам (`COLUMNS`), спроецировать нельзя: его значения адресуются ключом «строка и колонка», а всё здесь — строка `list`, запись `byKey`, имя на контроллере — означает одну строку. Поэтому его не может рисовать ни группа, которую проецирует React-контейнер, ни форменное свойство внутри такого контейнера: такая форма отклоняется при построении с указанием свойства. В остальной части формы оно остаётся как есть, а у React-представления просто нет для него имени: [контроллер](How-to_Custom_view_controller.md) несёт то же, что и проекция, поэтому у неспроецированного нет ни члена, ни возможности назвать его в `properties.change`. То же верно всегда, когда имя не выделяет один драв: спроецированная группа не может рисовать два свойства под одним интеграционным именем — проекция показала бы одно, а поиск нашёл бы другое; задайте одному из них явный `EXTID`. Если представлению нужны эти значения, объявите обычное свойство.

Везде, где член принимает строку, он принимает объект строки, её дескриптор `objects` либо — там, где это представление рисует строки, — строковый **ключ**, который дала ей проекция: ключ ищется в `byKey`, собственном индексе грида, поэтому он называет строку только там, где эти строки есть. Что означает каждый из трёх и где голого ключа недостаточно, сказано в разделе [Идентификация строки](How-to_Custom_view_controller.md#row-identity-contract).

`key`, `isCurrent`, `objects`, `background`, `foreground` и `selected` — зарезервированные имена полей строки. В узле группы резервирование следует за тем, что этот узел несёт: `properties`, `change`, `__groupSID` и `__member` зарезервированы в каждом из них, а `list`, `byKey`, `keys` и `options` — только в том контейнере, который рисует строки группы; в контейнере, несущем лишь панельное свойство группы, их нет, поэтому свойство с именем `list` там ничем не отклоняется. `__proto__` зарезервировано везде: присваивание по этому имени не добавляет поле, а заменяет прототип объекта. Объекта `meta` нет нигде. Форма, в которой проецируемый интеграционный SID занимает зарезервированное имя или два проецируемых элемента занимают одно имя на одном уровне данных, отклоняется с явной ошибкой при её построении. Задайте ей явный `EXTID` или переименуйте.
...
| `data.<g>` | `options` — пользовательские опции группы, там, где представление рисует её строки; `properties` — записи СВОЙСТВ, которые несёт этот узел |
| `data.<integrationSID>` | Свойство уровня формы (без группы) — его `value` и атрибуты |
| `data.<containerSID>` | `caption` и `image` контейнера, объявленного в дизайне (`NEW <имя>`) или помеченного `lsf` (например, `BOX(o)`), по идентификатору компонента дизайна этого контейнера — всегда на верхнем уровне, во что бы контейнер ни был вложен. Такой контейнер присутствует всегда — `{}` при отсутствии подписи и картинки. Остальные контейнеры, которые платформа создаёт для формы и её групп (`TOOLBAR(g)`, `PANEL(g)`, …), не проецируются |
...
| --- | --- | --- |
| `type` | Вид, которым было преобразовано значение, — `number`, `boolean`, `string`, `date` либо `json` (разобранный, поэтому значением будет то, что лежало в JSON) | строка |
...
// ...
<List data={props.data.o} component={Row} />   // сам узел группы, а не его копия
// либо, без псевдонима:
...
FORM orders 'Orders'
    OBJECTS o = Order
    PROPERTIES(o) READONLY number, date, sum
    PROPERTIES() comment = orderComment      // ниже помечено `lsf`, поэтому его запись — data['PROPERTY(comment)']
;
...
| --- | --- | --- |
| Свойство в **таблице** группы объектов | `data.<g>.<integrationSID>`, рядом с остальными атрибутами колонок группы | Интеграционный SID свойства, `qty` |
| Свойство в **панели** или свойство уровня формы (без группы) | `data.<componentSID>`, на верхнем уровне | Идентификатор компонента дизайна свойства, `PROPERTY(note)` |
| Контейнер | `data.<componentSID>`, всегда на верхнем уровне | Идентификатор компонента дизайна контейнера, `BOX(o)` |

Интеграционным SID адресуется только свойство таблицы — и по той же причине, по которой так адресуется вся таблица: эту группу рисует React, поэтому запись колонки лежит в узле группы рядом с колонками, которые React рисует сам, и названа так же, как они. Всё остальное здесь платформа рисует целиком, и единственное имя, которое у проекции для этого есть, — идентификатор дизайна, то самое имя, по которому его размещает `<Lsf>`. Поэтому имя одно на оба действия: `<Lsf name="PROPERTY(note)"/>` его размещает, а `data['PROPERTY(note)']` читает. Панельное свойство стоит в этой строке, а не в первой, потому что платформа рисует его целиком — один рендерер, размещаемый через `<Lsf>`, — кто бы ни рисовал строки его группы, поэтому его запись — дескриптор верхнего уровня, адресуемый SID компонента.
...
`LSF` говорит, что свойство является компонентом, а не значением. Где оно размещается, не говорится — и не может: у свойства таблицы нет собственного места в дизайне, а его отрисовщики попадают в строки, поэтому размещает их тот компонент, который эти строки рисует. Здесь это компонент, которому группу отдал `MOVE BOX(o)`. `MOVE` свойства таблицы отклоняется при построении формы — с `LSF` или без. Компонент называет свойство и строку:
...
Объявление отклоняется с указанием свойства, когда работать оно не может: на свойстве таблицы, группа которого не отрисовывается контейнером `CUSTOM REACT` (построчные редакторы было бы некому размещать), и на свойстве таблицы, сгруппированном в колонки (построчный редактор не может адресовать ячейку на пересечении строки и колонки).
