...
По умолчанию компонент рисует всё поддерево контейнера по `props.data`. Дочерний компонент может вместо этого сохранить своё представление lsFusion: на нём задаётся `lsf = TRUE`, и компонент не рисует его, а размещает через `<Lsf sid/>`.
...
`props.data.components` сопоставляет `sid` каждого дочернего компонента с `lsf` его дескриптору `{ caption, image }`, в порядке `DESIGN`. `sid` (ключ) — идентификатор компонента дизайна, например `BOX(o)` или `PROPERTY(comment)`; дескриптор несёт заголовок и изображение, которые делегированный компонент больше не рисует в GWT. Он входит в проецируемые `data`, поэтому динамический заголовок или изображение перерисовываются как любое другое изменение данных.

`Lsf`, `Lsfs` и `useLsf` — глобальные переменные времени выполнения, как и `List`, поэтому перед использованием в примерах ниже их нужно связать с локальными именами: `const { Lsf, Lsfs, useLsf } = window.lsfusion;`.
...
`<Lsfs/>` выполняет тот же перебор, поэтому контейнер, использующий его, размещает все дочерние компоненты с `lsf`, не называя ни одного:
...
`<Lsf>` короче, и именно его размещает `<Lsfs/>`. `useLsf(sid)` возвращает ref-колбэк — для элемента, который компонент отрисовывает и так: панели, карточки, ячейки сетки, — и представление попадает прямо в него.
...
`<Lsfs/>` оборачивает каждый дочерний компонент в `<div class="lsf-slot">`, а когда в дескрипторе есть заголовок или изображение, рисует их над узлом размещения в `<div class="lsf-slot-caption">` (изображение — в `<span class="lsf-slot-image">`). Его узлы размещения адресуются из CSS как `.lsf-slot > .lsf-view[data-lsf-sid="..."]`. `props.data.components` перебирается вручную, когда слоту нужна собственная разметка, а не собственный стиль:
...
- Свойство, отрисовываемое на группе объектов, которую рисует компонент, пометить `lsf` нельзя: у такой группы нет представления lsFusion, которое можно разместить. Вместо этого `lsf` ставится на `BOX` этой группы. Сервер отвергает такой дизайн.
...
Компонент React оправдан тогда, когда расположение вычисляется — дочерние компоненты размещаются по условию, сетка выводится из данных, прочитанных через `useFormData`, или разметка берётся из библиотеки компонентов, — либо когда используется `<Lsfs/>`, так как он размещает дочерний компонент, добавленный более поздним модулем, без правки самого компонента. У HTML-шаблона нет ни того, ни другого: чтобы добавить в него дочерний компонент, нужно изменить строку шаблона, а она принадлежит объявившему её модулю.
