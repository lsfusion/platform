---
slug: "/How-to_Filtering_and_ordering"
...
Нужно, чтобы на форме заказов пользователь мог фильтровать по покупателю без отдельного свойства в панели: строка фильтра должна показываться над таблицей сразу при открытии формы.

### Решение

```lsf
FORM ordersFiltered 'Заказы'
    OBJECTS o = Order
...
Опция `FILTER` объявляет [пользовательский фильтр](../paradigm/Interactive_view.md#userfilters) для свойства покупателя. При открытии формы строка его условия уже показана с пустым значением и ничего не отфильтровывает; как только пользователь вводит покупателя, остаются только его заказы. Удалить строку пользователь не может - сброс фильтров лишь очищает её значение, - но, в отличие от локального свойства из [**Примера 3**](#пример-3), при показанных элементах управления фильтрами пользователь может изменить сравнение, а свойство вне формы не нужно.
