...
`FIELDS … DO` стоит выбирать, когда значения нужны ровно один раз и валидация не требует нескольких проходов. Если потребуется сначала проверить ссылки, потом пакетно создать объекты, и наконец заполнить их свойства — переключиться на форму или на промежуточные `LOCAL` (см. [пример 5](#пример-5); для массива на корневом уровне, как здесь, — [пример 6](#пример-6)).
...
`FOR importBookName(INTEGER i)` обходит каждую строку, для которой импортированное название не `NULL`, и для каждой создаёт объект класса `Book`. Системное свойство `imported[INTEGER]` для итерации здесь не годится: импорт формы записывает данные только в свойства и фильтры формы, а фильтра с `imported[INTEGER]` на этой форме нет (после `IMPORT … TO` оно заполнено потому, что такой фильтр есть на форме, которую платформа создаёт для этого оператора сама). Роль признака «строка пришла из файла» здесь играет непустое staging-свойство; как получить такой признак, когда ни одно поле записи не обязательно, показано в [примере 8](#пример-8).
...
Аналогично [**примеру 5**](#пример-5), но файл представляет собой JSON-массив на корневом уровне — в таком виде списки обычно возвращают внешние сервисы:
...
importBooksFromJSONArray 'Импорт из JSON-массива' () {
    INPUT f = FILE DO NEWSESSION {
        IMPORT importBooksArray JSON FROM f;

        IF NOT (GROUP SUM 1 IF importBookName(INTEGER r)) THEN {
            MESSAGE 'Из файла не прочитано ни одной книги';
            RETURN;
        }
...
Форма отличается от формы из [примера 5](#пример-5) только опцией `EXTID 'value'` у группы объектов. Группа объектов читает свои записи из массива под ключом, равным её [имени экспорта / импорта](../paradigm/Structured_view.md#extid), а у массива на корневом уровне ключа нет: такой файл платформа читает как `{ "value" : [ ... ] }` (см. [преобразование предопределённого значения `value`](../paradigm/Structured_view.md#value)), поэтому читающая его группа должна иметь имя экспорта / импорта `value`. Собственное имя объекта `books` при этом сохраняется: `EXTID` меняет только имя, под которым группа ищется в файле.

С любым другим именем импорт завершается без ошибки и не читает ни одной строки: группа объектов, имени которой не соответствует ни один ключ, пропускается вместе со всем, что под ней. То же происходит в [примере 5](#пример-5), если группа названа не так, как ключ, и тогда, когда `ROOT` указывает на ключ самого массива, — `IMPORT importBooks JSON ROOT 'books' FROM f` делает корнем этот массив, и он снова читается только группой с именем `value`. О таком несовпадении ничто не сообщает, поэтому действие, прежде чем продолжить, проверяет, что прочитана хотя бы одна строка. Отличить несовпавшее имя от файла с пустым массивом эта проверка не может, поэтому она уместна там, где пустой файл не ожидается.

Варианту с `FIELDS` из [примера 3](#пример-3) такое имя не нужно: группа объектов формы, которую он создаёт автоматически, уже называется `value`.
...
Аналогично [**Примеру 7**](#пример-7), но ни одно поле записи не обязательно: документооборот может прийти без `number`, а событие — пустым объектом.
...
Нужно обработать каждый элемент обоих массивов независимо от того, какие поля в нём есть, — здесь посчитать события каждого документооборота.
...
countEvents (FILE f) {
    ...
    FOR imported(INTEGER d) DO
        MESSAGE (OVERRIDE docflowNumber(d), 'без номера') + ': ' +
            (OVERRIDE (GROUP SUM 1 IF eventImported(INTEGER e) AND eventDocflow(e) = d), 0) + ' событий';
}
```

Промежуточное свойство признаком строки здесь служить не может: `FOR docflowNumber(INTEGER d)` пропустил бы документооборот без номера. Вместо этого признак задаётся фильтром: [для каждого прочитанного из файла объекта платформа записывает значение по умолчанию во все фильтры его группы объектов](../paradigm/In_a_structured_view_EXPORT_IMPORT.md#importForm), а для `BOOLEAN`-свойства это `TRUE`. Поэтому после импорта `imported[INTEGER]` равно `TRUE` для обоих документооборотов, а `eventImported[INTEGER]` — для обоих событий, включая пустое.

Системное свойство `imported[INTEGER]` подходит только для одной группы объектов. Каждая группа объектов нумерует свои строки с 0, поэтому с `imported[INTEGER]` в фильтрах обеих групп признаки документооборотов и событий попали бы в одни и те же строки одного свойства. Поэтому вложенная группа получает собственное свойство-признак `eventImported[INTEGER]`.
