...
Платформа сама генерирует нужный класс, добавляя туда заданный код, а затем компилирует его при помощи компилятора Eclipse, поставляемого вместе с платформой. Преимущество такого подхода в том, что при сборке проекта не потребуется отдельный шаг с компиляцией Java кода. Однако, такой подход имеет ряд существенных ограничений и может использоваться только в самых простых случаях.
...
Компилировать класс при сборке проекта не обязательно: если скомпилированный `CalculateGCD` не найден в classpath, достаточно положить `CalculateGCD.java` в ресурсы приложения — платформа сама найдет исходный файл и скомпилирует его при загрузке модуля.
