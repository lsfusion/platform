...
Если не "оборачивать" действие по установлению свойства `isPosted` в [оператор `NEWSESSION`](../language/NEWSESSION_operator.md), то во время выполнения [оператора `APPLY`](../language/APPLY_operator.md) в базу данных будут записаны также другие изменения, возможно сделанные на форме `orders`.
...
Если не использовать оператор `NEWSESSION`, то объект нового заказа будет создан в [сессии изменений](../paradigm/Change_sessions.md) формы `orders`. При этом если пользователь закроет форму не сохранив, то изменения "останутся" в сессии изменений формы, и созданный заказ будет отображен в форме со списком заказов.
...
По умолчанию, в новой сессии не учитываются изменения, сделанные в "верхней" сессии. Для того, чтобы свойство `selected[Order]` было доступно в новой сессии, нужно указать его в блоке `NESTED (...)` соответствующего оператора `NEWSESSION` (`NESTED (selected[Order])`). Без этого в свойстве `selected[Order]` всегда будет `NULL`. Кроме того, вместо указания конкретных свойств можно использовать `NESTED LOCAL`. В этом случае будут доступны изменения всех локальных свойств верхней сессии.
...
При использовании [оператора `NESTEDSESSION`](../language/NESTEDSESSION_operator.md) все изменения, сделанные в "верхней" сессии изменений, будут доступны во вложенной сессии. Если пользователь закроет форму, не нажав ОК, то все изменения, сделанные непосредственно в форме, пропадут. Если пользователь нажмет ОК, то изменения будут записаны не в базу данных, а в "верхнюю" сессию изменений. В базу данных они запишутся вместе с изменениями сделанными в основной форме `orders`.

Использовать `NEWSESSION` в данном случае нельзя хотя бы потому, что при создании нового заказа, которого еще нет в базе данных, форма `orderPayments` не сможет увидеть этого заказа (так как в новой сессии, создаваемой `NEWSESSION`, не видны изменения "верхней"), и поведение будет непредсказуемым.
