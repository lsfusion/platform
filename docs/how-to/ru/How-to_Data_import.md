...
[Оператор `INPUT`](../language/INPUT_operator.md), который запрашивает файл, вызовет пользователю диалог с выбором файлов с расширениями xls и xlsx. При успешном выборе будет вызвано [действие](../paradigm/Actions.md), которое следует после слова `DO`.
...
Действие `listFiles` объявлено в системном [модуле](../paradigm/Modules.md) `Utils`. Оно сканирует указанную в параметре папку и считывает все файлы из нее в свойства `fileName` (имя файла) и `fileIsDirectory` (логическое свойство - является ли файл директорием).

[Оператор `READ`](../language/READ_operator.md) читает указанный файл в локальное свойство с типом `FILE`, которое затем обрабатывает оператор `IMPORT`. В его параметрах указывается, что форматом файла является CSV без заголовка в первой строке, с вертикальной чертой в качестве разделителем и кодировкой CP1251.
...
Каждый файл обрабатывается в отдельной новой [сессии изменений](../paradigm/Change_sessions.md) с последующим сохранением путем вызова [оператора `APPLY`](../language/APPLY_operator.md). Этот оператор записывает в свойство `canceled` `TRUE`, если при сохранении было нарушено некоторое [ограничение](../paradigm/Constraints.md). Дальше при помощи конструкции `MOVE` оператора `READ` файл перемещается либо в папку `success`, либо в папку `error`. Это нужно, чтобы действие можно было вызывать повторно, не обрабатывая при этом одни и те же заказы повторно.
...
Для реализации импорта нужно объявить форму [структуры](../paradigm/Structured_view.md), соответствующей структуре JSON-файла.

Тэг `version`, который находится на самом высоком уровне, объявляем без входов и добавляем на форму.

Значением тэга `order` является массив, поэтому на форме объявляем объект с именем этого тэга. Платформа создаст по новом объекту для каждого элемента массива в JSON. Свойства `date` и `number` для заказа будут автоматически импортированы из соответствующий тэгов в JSON. Отдельные значения из такого JSON можно прочитать и без формы — [свойствами доступа к JSON](../paradigm/System_Utils.md#json-access) модуля `Utils`, такими как `field[JSON, STRING]` и `fieldText[JSON, STRING]`.
