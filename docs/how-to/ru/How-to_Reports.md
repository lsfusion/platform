...
Нужно сделать [печатную форму](../paradigm/Print_view.md), в которой будут отображаться все книги с разбивкой по категориям. Также нужно сделать экспорт этой формы в формат XLSX.

### Решение

Сначала объявим [форму](../paradigm/Forms.md), которая описывает структуру печатной формы.
...
Затем добавим два действия, которые при помощи [оператора `PRINT`](../language/PRINT_operator.md) формируют отчет и выводят его на предпросмотр и в XLSX соответственно.
...
Затем запускаем сервер из IDE, запускаем десктоп-клиент, открываем форму с книгами и жмем созданную кнопку печати. Система создаст автоматическую печатную форму и откроет ее в режиме [предпросмотра](../paradigm/In_a_print_view_PRINT.md#interactive). Далее нужно нажать указанную ниже кнопку :
...
Поскольку группы объектов `i` и `d` зависят друг от друга, формируется один плоский отчет, поэтому шаблон будет единственным и называется по [каноническому имени формы](../paradigm/Report_design.md#template) без постфикса (как и в [**Примере 1**](#пример-1)).
