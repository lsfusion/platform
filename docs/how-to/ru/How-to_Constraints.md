...
Аналогично [**Примеру 2**](#пример-2). Для заказа задан признак разрешения редактирования.

```lsf
editable 'Разрешено редактирование' = DATA BOOLEAN (Order);
```

Нужно запретить изменение даты заказа, для которого редактирование не разрешено, не запрещая при этом удаление заказов.

### Решение

```lsf
CONSTRAINT CHANGED(date(Order o)) AND NOT editable(o) AND o IS Order
    MESSAGE 'Запрещено менять дату заказа, закрытого для редактирования';
```

[Условие ограничения проверяется и на объектах, удаляемых в сессии](../paradigm/Constraints.md): при удалении заказа его первичные свойства сбрасываются в `NULL`, поэтому `CHANGED(date(o))` истинно, а `NOT editable(o)` тоже истинно — свойство удалённого объекта даёт `NULL`, и отрицание `NULL` истинно. Без `o IS Order` такое ограничение неожиданно блокировало бы удаление любого заказа с заданной датой. Для удалённого заказа `o IS Order` даёт `NULL`, и ограничение не срабатывает. В [**Примере 2**](#пример-2) защита не нужна: там условие `posted(o)` положительное, и для удалённого заказа оно само даёт `NULL`. Намеренный запрет удаления, наоборот, строится через `DROPPED`, как в [**Примере 3**](#пример-3).
