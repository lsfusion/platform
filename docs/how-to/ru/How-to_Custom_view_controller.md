...
Любое пользовательское представление получает контроллер, привязанный к тому, что оно отрисовывает, — собственный контроллер таблицы у представления объекта/таблицы либо контроллер рендера/редактора ячейки у представления свойства — с геттерами значения, стиля, `diff` и `isCurrent` для этой группы или ячейки. Этот контроллер также выставляет `form`: контроллер формы, описанный на этой странице. Поэтому из любого пользовательского представления — объектной группы **или** свойства — вызовы сервера и изменения в пределах формы идут через `controller.form` (`controller.form.exec(...)`, `controller.form.changeObject(...)`, `controller.form.getPropertyValues(...)`, …). Исключение — представление [на React](How-to_Custom_React_views.md) для всей формы: оно владеет всей формой, поэтому его `props.controller` сразу является контроллером формы.
