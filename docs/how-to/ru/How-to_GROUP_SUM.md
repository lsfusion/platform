...
Такая композиция — базовый агрегат по разрезам записей движения (`currentBalance` из [**Примера 3**](#пример-3)) и второй агрегат поверх него — предпочтительна и для других разрезов остатка. Большая группировка, у которой в источнике или условии вместо этого стоит другой вычисляемый агрегат, может скомпилироваться в подзапрос по всем данным движения — внешнее ограничение не всегда проталкивается внутрь, — поэтому даже чтение одного значения резко деградирует. Переписывание в виде базового агрегата по исходным разрезам и второго агрегата поверх возвращает чтение по индексу.

## Пример 6

### Условие

Аналогично [**Примеру 3**](#пример-3), только нужно посчитать суммарную стоимость остатка на складе — сумму по всем книгам произведения остатка книги на её цену.
...
Книги помечаются тегами (связь многие-ко-многим).

```lsf
CLASS Tag 'Тег';
in 'Вхождение' = DATA BOOLEAN (Book, Tag);
```

Нужно посчитать количество тегов каждой книги.

### Решение

```lsf
tagCount 'Количество тегов' (Book b) = GROUP SUM 1 IF in(b, Tag t);
```

Это стандартный способ посчитать объекты, связанные через отношение с двумя параметрами: как и в [**Примере 6**](#пример-6), контекстный параметр `b` становится параметром создаваемого свойства, а объявленный внутри условия параметр `t` подсчитывается. Блок `BY` нужен, когда параметр создаваемого свойства задаётся выражением от параметров, объявленных внутри группировки (как в [**Примере 1**](#пример-1)), а не внешним параметром.
