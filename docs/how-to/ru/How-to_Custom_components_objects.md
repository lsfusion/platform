...
Названия полей объектов соответствуют названию свойств на форме. Значения свойств преобразуются в значения JS так же, как в строках [React-представления](How-to_Custom_React_views.md): например, значения классов даты и времени передаются как `Date`, а `JSON` — как разобранный объект. Свойство, значением которого является объект, приходит числовым id этого объекта — тем же, которым объект передаётся в [запись FK и вызовы сервера](How-to_Custom_view_controller.md). При помощи метода _isCurrent_ определяется, какой объект из списка является текущим.
...
Определение «значение или строка» в `changeProperty`, формат значений и правила уточнения имени — те же, что у `<group>.<property>.change` [контроллера формы](How-to_Custom_view_controller.md); свойство, не являющееся колонкой этой группы, `changeProperty` передает контроллеру формы, который разрешает его в пределах всей формы. `changeProperties` применяет несколько изменений одним запросом — например, представление диаграммы Ганта при перетаскивании полосы задачи изменяет обе даты сразу:

```js
controller.changeProperties(['start', 'end'], [task, task], [newStart, newEnd]);
```

`getPropertyValues` использует те же режимы `mode` и формат результата, что и `<group>.<property>.getValues` контроллера формы, но свойство разрешается среди колонок этой группы, а запрос выполняется для ее текущего объекта:
...
Геттеры отображения позволяют компоненту использовать атрибуты дизайна, задаваемые на форме: например, представление диаграмм строит наборы данных, беря заголовок и цвета колонок из `getCaption`, `getBackground` и `getForeground`.
