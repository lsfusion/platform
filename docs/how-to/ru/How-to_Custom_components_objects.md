...
Помимо рассмотренных выше методов отрисовки, _controller_ позволяет клиентскому JS обращаться к серверу и получать результат в виде `Promise`. Тот же объект передается последним аргументом в JavaScript-функцию, заданную в действии [`INTERNAL CLIENT`](../language/INTERNAL_operator.md), после преобразованных параметров вызова.
