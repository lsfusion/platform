...
Помимо рассмотренных выше методов отрисовки, _controller_ позволяет клиентскому JS обращаться к серверу и получать результат в виде `Promise`. Тот же объект передается первым аргументом в JavaScript-функцию, заданную в действии [`INTERNAL CLIENT`](../language/INTERNAL_operator.md).
...
Теперь `controller.exec("recalc", id)`, `controller.exec("setQty", id, 5)` и `controller.change("note", id, "ok")` работают на этой форме без `@@api` и `enableAPI`. Каждую запись можно переименовать псевдонимом (`setQty = setQuantity`), снабдить префиксом `ACTION`, чтобы выбрать действие, и указать полностью с сигнатурой (`Sales.post[Order]`) для выбора перегрузки; `exec` требует действие, `change` — свойство. Параметры по-прежнему передаются вызывающей стороной позиционно, поэтому блок меняет то, какие вызовы разрешены, а не то, как связываются параметры, и не ограничивает ключ или объект, к которому обращается вызывающая сторона. `eval`/`evalAction` выполняют произвольный скрипт и остаются под ограничением.