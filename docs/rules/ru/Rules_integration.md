...
6. Для импорта через форму ассистент ОБЯЗАН объявить
   ...
   - `EXTID`, `FORMEXTID`, группы и `ATTR`
     ...
     массив на корневом уровне `JSON` как раз такое место (правило 7)

   Ассистент ОБЯЗАН помнить, что импорт в форму
   отменяет ожидающие изменения импортируемых свойств формы
   в текущей сессии.

7. Для массива на корневом уровне `JSON` ассистент ОБЯЗАН
...
8. После импорта формы ассистент НЕ ДОЛЖЕН итерировать
...
9. Ассистент ОБЯЗАН явно выбирать опции формата,
...
10. Ассистент ОБЯЗАН валидировать ссылочные бизнес-ключи
...
11. Ассистенту СЛЕДУЕТ разделять сырой импорт
...
12. Для запускаемых пользователем пакетных импортов и внешних интеграций
...
13. Ассистент НЕ ДОЛЖЕН частично сохранять
...
14. Для импортов-синхронизаций «создать-или-обновить»
...
15. Если `LOCAL`-свойства промежуточного хранения
