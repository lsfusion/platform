    ...
    [Строковый литерал](Literals.md#strliteral), описывающий комбинацию клавиш. Синтаксис: 
		...
		- `(modeKey=modeValue;)*`
			...
			- `priority = priorityValue` 
				...
				Если приоритет не задан, он равняется порядковому номеру свойства на форме. Кроме того, в любом случае к значению приоритета добавляется 100, если совпадает группа объектов.
			...
			- `cell = cellValue`
				Проверка, находится ли свойство в таблице. Поддерживаемые значения `panelCell`:
				- `AUTO` -> !isMouse || isCell
				- `ALL` -> true
				- `ONLY` -> isCell
				- `NO` -> !isCell
