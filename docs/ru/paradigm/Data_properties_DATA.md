...
- при переходе в [новую сессию](../language/NEWSESSION_operator.md) и обратно;
- после [применения изменений](Apply_changes_APPLY.md);
- после [отмены изменений](Cancel_changes_CANCEL.md).

Если сохранение требуется только для части этих операций, используется ограниченный режим вложенности: либо только после [применения изменений](Apply_changes_APPLY.md) и [отмены изменений](Cancel_changes_CANCEL.md), либо только при переходе в [новую сессию](../language/NEWSESSION_operator.md) и обратно.

### Язык

Для объявления первичного свойства используется [оператор `DATA`](../language/DATA_operator.md).
