...
Оператор *группировки* создает [свойство](Properties.md), которое разбивает все наборы объектов в системе на группы, и с учетом заданного порядка вычисляет для каждой такой группы [агрегирующую функцию](Set_operations.md#func). Соответственно, множество, на котором вычисляется эта агрегирующая функция, определяется как все наборы объектов, принадлежащие данной группе. Если набор группировок пуст и оператор не использует верхних параметров, все подходящие наборы объектов образуют одну общую группу. Используемые оператором верхние параметры неявно расширяют набор группировок, так что каждая комбинация их значений образует отдельную группу. 
...
- при необходимости из упорядоченной группы выбирается только ее фрагмент — например, первые `n` элементов, все элементы после пропуска первых `m`, либо следующие `n` элементов после пропуска первых `m`
...
-   `EQUAL` является частным случаем агрегирующей функции `MAX` (или `MIN`), добавляя [ограничение](Constraints.md) на одинаковость значения операнда агрегирующей функции внутри каждой группы. Классом результата является класс выражения-операнда.
-   `AGGR` и `NAGGR` являются частным случаем `EQUAL`, но с еще более строгим ограничением: для каждой группы существует не более одного набора объектов, операнд агрегирующей функции является одним из объектов, а группировки включают в себя все остальные объекты. Агрегирующая функция `NAGGR` отличается от `AGGR` только тем, что в случае ее использования не создается ограничение (предполагается, что это ограничение следует из семантики самих свойств операндов и/или группировок). Поскольку операнд сам является одним из объектов группы, классом результата является класс этого объекта.
...
Класс результата пользовательского агрегата может быть задан явно. Иначе он выводится из значений, по которым ведётся агрегирование: из агрегируемых значений операндов для обычного пользовательского агрегата, либо из упорядоченной выборки для ordered-set агрегата.

### Язык

Для объявления свойства, реализующего группировку, используется [оператор `GROUP`](../language/GROUP_operator.md).
