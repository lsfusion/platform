...
Также в платформе существует возможность задавать условие и соответствующий ему результат одним свойством. В этом случае условием может являться либо [принадлежность сигнатуре](../language/Property_signature_ISCLASS.md) этого свойства, либо само это свойство. Такую форму оператора выбора будем называть *полиморфной*.
...
Для создания свойства, реализующего выбор в общем случае, используется оператор [`CASE`](../language/CASE_operator.md). Полиморфная форма оператора выбора реализуется при помощи операторов [`MULTI`](../language/MULTI_operator.md), [`OVERRIDE`](../language/OVERRIDE_operator.md) и [`EXCLUSIVE`](../language/EXCLUSIVE_operator.md), одиночная - при помощи операторов [`IF`](../language/IF_operator.md) и [`IF ... THEN`](../language/IF_..._THEN_operator.md) (только в этом операторе можно указывать альтернативный результат).
