...
Так же как и для [оператора ветвления](Branching_CASE_IF_MULTI.md#poly), для абстрактного действия существует *полиморфная форма*, когда можно не задавать условие явно, а использовать в качестве условия [принадлежность сигнатуре](../language/Property_signature_ISCLASS.md) соответствующего этому условию действия.
...
В этой технике используются две языковые конструкции: [оператор `ABSTRACT`](../language/ABSTRACT_action_operator.md) для объявления абстрактного действия и [инструкция `ACTION+`](../language/ACTION_plus_statement.md) для добавления к нему реализации.
