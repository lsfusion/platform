...
-   `CONTEXTMENU [caption]` - пользователь выбрал в контекстном меню свойства (действия) на форме заданный пункт. Также при необходимости можно задать заголовок этого пункта меню ([строковый литерал](../language/Literals.md#strliteral)). Если этого не сделать, то, по умолчанию, он будет равен заголовку действия.
-   `KEYPRESS key` - пользователь нажал кнопку `key` на клавиатуре, задаваемую ([строковым литералом](../language/Literals.md#strliteral)).
