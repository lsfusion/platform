...
Форма `ACTIVE FORM` создаёт действие, выполняющее проверку активности указанной формы у пользователя и записывающее результат (`TRUE` или `FALSE`) во встроенное [локальное](../paradigm/Data_properties_DATA.md#local) свойство `System.isActiveForm[]`. Результат доступен для чтения через свойство `isActiveForm()` в том же сеансе / блоке действия.
