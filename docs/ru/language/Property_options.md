...
При объявлении [свойства](../paradigm/Properties.md) в [инструкции `=`](=_statement.md) в конце объявления может указываться некоторый набор *опций свойства*. 
...
- `IN groupName`
    ...
    Указание [группы свойств и действий](../paradigm/Groups_of_properties_and_actions.md), которой принадлежит свойство. Если опция не указывается, то свойство по умолчанию будет принадлежать группе `System.private`.
...
- `INDEXED`
    ...
    Ключевое слово, указание которого создает [индекс](../paradigm/Indexes.md) по этому свойству. Может использоваться только для [материализованного](../paradigm/Materializations.md) свойства. Аналогично использованию [инструкции `INDEX`](INDEX_statement.md).
    ...
    - `indexType`
        ...
        - `LIKE`
            ...
            Ключевое слово, указание которого для строковых свойств сохраняет обычный индекс и дополнительно пытается создать специализированный GIN индекс для операций `LIKE`.

        - `MATCH`

            Ключевое слово, указание которого создает индекс для операций `MATCH`.

            - для одного поля типа `TSVECTOR` создается только специализированный GIN индекс по самому полю
            - для строковых свойств сохраняется обычный индекс и дополнительно пытаются создаваться специализированные индексы для `MATCH` и `LIKE`
            - строковый `MATCH` индекс строится по `to_tsvector` с текущим языком полнотекстового поиска

            Специализированные `LIKE` / `MATCH` индексы создаются только если для текущего адаптера БД доступна соответствующая trigram/full-text поддержка. Обычный индекс при этом создается независимо от нее.


- `NONULL [DELETE] eventClause`

    Добавление ограничения [определенности](../paradigm/Simple_constraints.md). Если это ограничение в результате каких-то изменений для некоторых объектов нарушается, то будет либо показано соответствующее сообщение, либо при указании `DELETE` такие объекты будут удалены.
...
- `viewType`
    ...
    Указание типа [представления свойства](../paradigm/Interactive_view.md#property) при добавлении на форму.

    - `GRID` - колонка таблицы
    - `TOOLBAR` - тулбар
    - `PANEL` - панель

  По действию аналогично указанию опции `viewType` в [блоке свойств](Properties_and_actions_block.md) [инструкции `FORM`](FORM_statement.md). Соответственно если эта опция не указана ни в опциях свойств, ни в блоке свойств непосредственно на форме, используется тип [представления по умолчанию](../paradigm/Interactive_view.md#property) для группы отображения, добавляемого на форму свойства.

- `ON eventType { actionOperator }`

    Указание действия, которое будет обработкой по умолчанию некоторого [события формы](../paradigm/Form_events.md) всех интерактивных представлений этого свойства. Могут быть переопределены в блоке свойств инструкции `FORM`.
...
- `imageSetting`
    ...
    - `IMAGE [imageLiteral]`
        ...
        [Указание иконки](../paradigm/Icons.md#manual), которая будет отображаться в качестве иконки свойства. Если `imageLiteral` не указывается, то происходит переключение в режим [автоматической установки](../paradigm/Icons.md#auto) иконки.
...
- `CHARWIDTH width [FLEX | NOFLEX]`
    ...
    Указание [количества символов](../paradigm/Form_design.md#valueWidth) значения свойства, которые должны быть видны пользователю. Устанавливает значение для дизайна по умолчанию, может быть переопределено в инструкции `DESIGN`.
...
- `CHANGEKEY key [SHOW | HIDE]`
    ...
    Указание [комбинации клавиш](../paradigm/Form_events.md#keyboard), при нажатии которой будет начато редактирование свойства. Устанавливает значение для дизайна по умолчанию, может быть переопределено в инструкции `DESIGN`.
...
- `DEFAULTCOMPARE [compare]`
    ...
    Указание для свойства типа [фильтра по умолчанию](../paradigm/Interactive_view.md#userfilters).

    - `compare`

        Тип фильтра по умолчанию. [Строковый литерал](Literals.md#strliteral). Может принимать следующие значения: `=`, `>`, `<`, `>=`, `<=`, `!=`, `=*` (содержит), `=@` (нечёткий поиск). По умолчанию принимает значение `=` для всех типов данных кроме строковых регистронезависимых, для которых принимает значение `=@` или `=*` (а при включенной настройке `System.defaultCompareForStringContains` - кроме всех строковых, независимо от регистронезависимости). Выбор значения `=@` или `=*` зависит от настройки `System.defaultCompareSearchInsteadOfContains` (true - `=@`, false - `=*`, по умолчанию true). Может быть переопределено в инструкции `DESIGN`.
