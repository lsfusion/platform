---
slug: "/SCREENSHOT_operator"
title: 'Оператор SCREENSHOT'
---

Оператор `SCREENSHOT` создаёт [действие](../paradigm/Actions.md), которое [делает снимок](../paradigm/Capture_SCREENSHOT.md) текущего отрисованного пользовательского интерфейса и записывает результат в свойство либо, если целевое свойство не задано, отправляет файл на клиент.
...
Оператор `SCREENSHOT` создаёт действие, которое захватывает текущее отрисованное состояние пользовательского интерфейса в [веб-клиенте](../paradigm/Capture_SCREENSHOT.md#client). Область захвата определяется выбранной [целью снимка](../paradigm/Capture_SCREENSHOT.md#target); выходной [формат](../paradigm/Capture_SCREENSHOT.md#format) выбирается ключевым словом `HTML`; [адресат](../paradigm/Capture_SCREENSHOT.md#destination) задаётся выражением `TO`.
...
- `captureTarget`
    ...
    - `containerId`
        ...
        [Простой идентификатор](IDs.md#id) контейнера в [дизайне](../paradigm/Form_design.md) формы, в контексте которой выполняется действие. Действие должно выполняться в контексте формы, и форма должна содержать контейнер с указанным SID.
