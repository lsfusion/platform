...
Оператор `PRINT` - создание [действия](../paradigm/Actions.md), [открывающего форму](../paradigm/In_a_print_view_PRINT.md) в печатном представлении. 
...
Оператор `PRINT` создает действие, которое печатает указанную форму. При печати формы в блоке `OBJECTS` можно добавлять объектам формы [дополнительные фильтры](../paradigm/Open_form.md#params) на равенство этих объектов переданным значениям.
...
- `printFormat`
    ...
    [Формат печати](../paradigm/In_a_print_view_PRINT.md#format), задается одним из ключевых слов:
...
- `PREVIEW`
    ...
    Ключевое слово. Если указывается, то форма показывается в режиме [предварительного просмотра](../paradigm/In_a_print_view_PRINT.md#interactive). Этот режим используется по умолчанию, если другие режимы / форматы не заданы.
...
- `MESSAGE`
    ...
    Ключевое слово. Если указывается, то форма выдает данные пользователю в режиме [сообщения](../paradigm/In_a_print_view_PRINT.md#interactive).
