---
slug: "/ABSTRACT_action_operator"
title: 'Оператор ABSTRACT'
---

Оператор `ABSTRACT` создает [абстрактное действие](../paradigm/Action_extension.md).
...
Оператор `ABSTRACT` создает абстрактное действие. Его реализации позже добавляются инструкциями [`ACTION+`](ACTION_plus_statement.md). В зависимости от выбранного типа платформа формирует из них поведение [оператора ветвления](../paradigm/Branching_CASE_IF_MULTI.md) или [оператора последовательности](../paradigm/Sequence.md).
...
- `type`
    ...
    - `CASE` - явная условная форма абстрактного действия. Условие выбора каждой реализации задается в соответствующей инструкции [`ACTION+`](ACTION_plus_statement.md) с помощью блока `WHEN`.
    - `MULTI` - [полиморфная форма](../paradigm/Branching_CASE_IF_MULTI.md#poly) абстрактного действия. Условием выбора реализации является совместимость текущих аргументов с ее [сигнатурой](ISCLASS_operator.md).
...
- `exclusionType`
    ...
    Опция. Задает [тип взаимоисключения](../paradigm/Branching_CASE_IF_MULTI.md#exclusive). Возможные значения:
...
- `FULL`
    ...
    Ключевое слово. Если задано, платформа автоматически проверит [полноту набора реализаций](../paradigm/Action_extension.md#full): для всех потомков классов аргументов должна существовать хотя бы одна подходящая реализация (или ровно одна, если условия взаимоисключающие).
