...
Оператор `CANCEL` - создание [действия](../paradigm/Actions.md), [отменяющего изменения](../paradigm/Cancel_changes_CANCEL.md) в текущей сессии.
...
Оператор `CANCEL` создает действие, которое отменяет изменения в текущей сессии. С помощью указания ключевого слова `NESTED` можно указать [локальные свойства](../paradigm/Data_properties_DATA.md#local), изменения которых не сбросятся при отмене изменений.
