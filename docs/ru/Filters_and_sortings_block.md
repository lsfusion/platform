...
Блоки фильтров и сортировок [инструкции `FORM`](FORM_statement.md) - добавление [фильтров](Form_structure.md#filters) и [сортировок](Form_structure.md#sort) в структуру формы, а также [групп фильтров](Interactive_view.md#filtergroup) и [пользовательских фильтров](Interactive_view.md#userfilters) в интерактивное представление формы.
...
## Блок пользовательских фильтров {#userfilters}
...
Каждый фильтр задается [свойством на форме](Properties_and_actions_block.md#name), которое уже должно быть добавлено на форму ранее.
...
Опции `orderOptions` должны указываться в следующем порядке: сначала `USER | FIXED`, затем `DESC`. Поддерживается следующий набор опций:
...
- `USER | FIXED`
    ...
    `USER` — пользовательская сортировка (по умолчанию), которую можно переопределять из интерфейса;
    `FIXED` — фиксированная сортировка, которая задаётся только в коде формы.
