...
- `messageType`
    ...
    Тип сообщения. Определяет, как и где покажется сообщение. Задается одним из ключевых слов:

    - `LOG` - сообщение показывается в окне сообщений `System.log`.
  
    - `INFO` - сообщение, стилизованное как информационное,  показывается в виде уведомления в браузере (если не включена опция `enableShowingRecentlyLogMessages`) и в окне сообщений `System.log`.
	
	- `SUCCESS` - сообщение, стилизованное как успешное,  показывается в виде диалогового окна с текстовым сообщением и в виде уведомления в браузере (если не включена опция `enableShowingRecentlyLogMessages`).
	
	- `WARN` - сообщение, стилизованное как предупреждение,  показывается в виде диалогового окна с текстовым сообщением и в виде уведомления в браузере (если не включена опция `enableShowingRecentlyLogMessages`).
	
	- `ERROR` - сообщение, стилизованное как ошибка, показывается в виде диалогового окна с текстовым сообщением и в виде уведомления в браузере (если не включена опция `enableShowingRecentlyLogMessages`) и в окне сообщений `System.log`.
	
	- `DEFAULT` - сообщение без иконки и без цвета фона показывается в виде диалогового окна с текстовым сообщением и в виде уведомления в браузере (если не включена опция `enableShowingRecentlyLogMessages`). Это значение используется по умолчанию.
