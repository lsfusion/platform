...
### Синтаксис
    ...
    MESSAGE expression [syncType] [LOG]

### Описание

Оператор `MESSAGE` создает действие, которое выдает пользователю окно с текстовым сообщением либо записывает текстовое сообщение в лог. Текстовое сообщение может быть представлено как строковой константой, так и другим более сложным [выражением](Expression.md), значением которого является строка, либо значение которого преобразовывается в строку.
...
- `LOG` - ключевое слово, которое определяет, что сообщение не показывается клиенту, а записывается в клиентский лог
...
// В этом случае пользователю будет выдано пять текстовых сообщений, а третье будет дополнительно записано в лог
testMessage()  {                    
    LOCAL i = INTEGER();
    i() <- 0;
    WHILE i() < 5 DO {
