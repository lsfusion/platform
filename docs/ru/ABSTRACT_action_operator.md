...
Оператор `ABSTRACT` создает абстрактное действие. Его реализации позже добавляются инструкциями [`ACTION+`](ACTION_plus_statement.md). В зависимости от выбранного типа платформа формирует из них поведение [оператора ветвления](Branching_CASE_IF_MULTI.md) или [оператора последовательности](Sequence.md).
