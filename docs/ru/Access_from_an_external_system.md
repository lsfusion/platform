...
Возможность выполнения запроса также зависит от настройки [`enableAPI`](Working_parameters.md), которая позволяет полностью отключить API, ограничить его авторизованными запросами или дополнительно разрешить анонимные запросы. Отдельное действие можно пометить [`@@noauth`](Action_options.md), чтобы обойти и проверку аутентификации, и `enableAPI`, или [`@@api`](Action_options.md), чтобы разрешить его при `enableAPI=0` (всё ещё требуя авторизованного пользователя).
