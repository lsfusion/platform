...
```
NEWSESSION [NEWSQL] [FORMS formName1, ..., formNameN] [nestedBlock] action 
```
...
- `formName1, ..., formNameN`
  ...
  Список имен [форм](Forms.md), для которых дополнительно действует новая сессия . Каждый элемент списка является [составным идентификатором](IDs.md#cid). Если список не задается, то новая сессия действует только для всех активных форм.
