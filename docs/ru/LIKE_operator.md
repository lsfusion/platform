...
Оператор `LIKE` создает [свойство](Properties.md), реализующее [сравнение строки с шаблоном](Comparison_operators_=_etc.md). Для полнотекстового поиска используется отдельный [оператор `MATCH`](MATCH_operator.md).
...
- `searchExpr`
    ...
    Значение выражения должно принадлежать одному из [строковых классов](Built-in_classes.md).
