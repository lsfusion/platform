...
Оператор `RETURN` создает действие, которое выходит из наиболее вложенного [вызова действия](Call_EXEC.md). Если задано `resultExpr`, значение этого выражения становится результатом этого вызова. Иначе вызов просто завершается, не возвращая значения.

Класс результата окружающего действия определяется одним из двух способов:

- Для [абстрактных действий](Action_extension.md) класс результата объявляется в [операторе `ABSTRACT`](ABSTRACT_action_operator.md); класс значения выражения `resultExpr` должен соответствовать объявленному классу.
