...
Внутри тела действия в выражении в качестве свойства может также выступать вызов [действия](Actions.md), возвращающего результат. В этом случае в скобках сначала указываются значения входных параметров действия, а затем — новые локальные параметры, которые становятся именами параметров результата и могут использоваться дальше в выражении.

Такой вызов эквивалентен последовательному выполнению:

- создание [локального свойства](Data_properties_DATA.md#local) с сигнатурой результата действия;
