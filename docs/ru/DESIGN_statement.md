...
- `insertPos`
    ...
    - `FIRST` | `LAST` | `DEFAULT`
        ...
        Ключевое слово, которое указывает на то, что компонент должен быть добавлен или перенесен на первое место (`FIRST`), на последнее место (`LAST`) или в порядке объявления (`DEFAULT`, используется по умолчанию) в списке потомков текущего контейнера.
