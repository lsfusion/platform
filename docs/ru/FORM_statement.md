...
- `name`
    ...
    Имя формы. [Простой идентификатор](IDs.md#id). Имя должно быть уникально в пределах текущего [пространства имен](http://documentation.lsfusion.org/pages/viewpage.action?pageId=1146884).
...
- `AUTOREFRESH period`
    ...
    Указание периода [автоматического обновления](Interactive_view.md#extra) формы через интервал времени. Если опция не задана, то форма автоматически обновляться не будет. **deprecated since version 5, используйте `SCHEDULE refresh()`**
...
- `SCHEDULE ...`
  ...
  Устанавливает планировщик для формы. [Синтаксис блока планировщика](Scheduler_block.md).
