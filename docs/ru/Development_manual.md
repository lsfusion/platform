...
-   После старта сервера в логе старта одной из последних строк будет строка со ссылкой на jnlp-файл (например, https://download.lsfusion.org/java/lsfusion-client-6.1.jnlp), запуск которого приведёт к автоматической установке клиента при помощи технологии Java Web Start.
...
-   Скачать [веб-клиента](https://download.lsfusion.org/java/lsfusion-client-6.1.war) платформы lsFusion.
...
-   Скачать файл `lsfusion-server-<version>.jar` нужной версии (например `lsfusion-server-6.1.jar`) с [центрального сервера](https://download.lsfusion.org/java/) в папку нужного модуля проекта (далее будем называть эту папку `$FUSION_DIR$`).
