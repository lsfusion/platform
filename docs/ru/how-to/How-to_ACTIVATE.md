...
После закрытия формы вызывается [оператор `ACTIVATE`](../language/ACTIVATE_operator.md), который делает добавленный объект активным.
...
Свойство с покупателем по умолчанию добавляется на форму `'Настройка'` во вкладку `'Общие'`. Текущий объект изменится при входе на форму, так как сработает [событие `ON INIT`](../language/Event_block.md).
...
Свойства по работе с датами находятся в [системном модуле](../paradigm/Modules.md) `Time`, который подключается в самом начале через инструкцию `REQUIRE`.
...
При открытии формы срабатывает событие [`ON INIT`](../language/Event_block.md) и [оператор `ACTIVATE`](../language/ACTIVATE_operator.md) переводит фокус на поле `dFrom`. `ACTIVATE PROPERTY` работает с любым свойством, отображаемым на форме — как с панельным, так и с гридовым или древовидным.
