...
Для решения задачи необходимо создать действие при помощи [оператора `INTERNAL`](../language/INTERNAL_operator.md), которое создаст объект класса `GetIP` (если у класса есть package, то в названии класса нужно также указывать package) и вызовет у него метод `executeInternal`. Исходный код этого класса:
...
:::info
Канонический паттерн долгоживущего серверного компонента — bean, регистрируемый в Spring и подключаемый к жизненному циклу платформы; см. [свой Spring bean (`EventServer`)](../paradigm/Custom_Spring_bean_EventServer.md). Решение ниже создаёт `MonitorServer` изнутри `InternalAction` без регистрации в Spring — рабочая, но менее каноническая альтернатива.
:::
