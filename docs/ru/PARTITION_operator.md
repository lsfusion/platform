...
Существует три разных вида оператора `PARTITION`. Первый из них реализует разбиение / упорядочивание:
...
Третий вариант позволяет использовать произвольную SQL-агрегирующую функцию:
...
Список выражений `expr1, ..., exprN` для `CUSTOM` может быть опущен, но в этом случае обязательно должен присутствовать блок `ORDER`.
...
- `type`
    ...
    Тип агрегирующей функции. Встроенными типами агрегирующих функций являются `SUM`, `PREV` и `LAST`. Также поддерживается ключевое слово `CUSTOM`, которое позволяет использовать произвольную SQL-агрегирующую функцию.

- `CUSTOM [NULL] [returnClass] 'sqlAggrFunc'`

    Использование произвольной SQL-агрегирующей функции, не встроенной в платформу. Имя SQL-функции передается строковым литералом `sqlAggrFunc` (например, `'STRING_AGG'`, `'BOOL_OR'`). Аргументы, передаваемые SQL-функции, задаются списком выражений `expr1, ..., exprN` после имени функции (без круглых скобок, через запятую); согласно грамматике, при использовании `CUSTOM` допускается также указывать только блок `ORDER`, без списка выражений.

    В `PARTITION` указанная функция компилируется в SQL как оконная агрегирующая функция вида `sqlAggrFunc(...) OVER (...)`, поэтому подходят только те агрегирующие функции, которые СУБД допускает использовать в качестве оконных. Ordered-set агрегаты (форма `... WITHIN GROUP ORDER BY ...`) здесь не поддерживаются — для них предусмотрен оператор [`GROUP`](GROUP_operator.md).

    - `NULL`

        Необязательное ключевое слово. Указывает, что SQL-функция может возвращать `NULL` даже при ненулевых входных значениях.

    - `returnClass`

        Необязательное имя [встроенного класса](Built-in_classes.md) (например, `INTEGER`, `NUMERIC`, `STRING` и т. п.), явно задающее класс результата. Если не указан, класс выводится из выражений-операндов; если список выражений-операндов отсутствует (форма `CUSTOM ... ORDER ...`), класс выводится из первого выражения блока `ORDER`.
...
- `OFFSET offsetExpr`
    ...
    В группировке будут участвовать только записи со смещением `m`, где `m` - значение выражения `offsetExpr`. Может использоваться как вместе с `TOP`, так и самостоятельно.
...
// пример с CUSTOM: для каждой команды возвращает строку со списком всех команд её конференции
name = DATA STRING[30] (Team);
teamNames 'Команды конференции' (Team team) =
