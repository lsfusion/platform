...
- `viewType`
    ...
    - `PIVOT` - вид отображения *PIVOT*.
    - `MAP` - вид отображения *карта*.
    - `CUSTOM` - пользовательский вид отображения.
    - `CALENDAR` - вид отображения *календарь*.
...
- `FIRST` | `LAST` | `DEFAULT`
  ...
  Указание того, что группа объектов должна быть добавлена в структуру формы на первое место (`FIRST`), на последнее место (`LAST`) или в порядке объявления (`DEFAULT`, применяется по умолчанию) в списке потомков текущего контейнера.
