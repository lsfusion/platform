...
В этой технике используются две языковые конструкции: [оператор `ABSTRACT`](ABSTRACT_action_operator.md) для объявления абстрактного действия и [инструкция `ACTION+`](ACTION_plus_statement.md) для добавления к нему реализации.
