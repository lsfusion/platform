...
Оператор `ABSTRACT` создает абстрактное свойство. Его реализации позже добавляются инструкциями [`+=`](plus_equals_statement.md). В зависимости от выбранного типа платформа формирует из них поведение [оператора выбора](Selection_CASE_IF_MULTI_OVERRIDE_EXCLUSIVE.md).
