...
Для объявления действия, обращающегося к внешней системе, используется [оператор `EXTERNAL`](EXTERNAL_operator.md). Для переиспользования внешнего подключения между несколькими вызовами в одном блоке используется [оператор `NEWCONNECTION`](NEWCONNECTION_operator.md).
