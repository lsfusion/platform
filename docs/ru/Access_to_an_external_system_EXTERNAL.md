...
Платформа предоставляет возможность обращаться к внешним системам с использованием различных типов взаимодействия / протоколов. Интерфейсом такого обращения является выполнение кода на языке / в парадигме внешней системы с заданными параметрами и, при необходимости, возврат некоторых значений в качестве *результатов*, записываемых в заданные свойства (без параметров). Предполагается, что все объекты параметров и результатов являются объектами [встроенных классов](Built-in_classes.md).
...
:::info
При `CLIENT` desktop-клиент полностью выполняет вызов локально, а обычный веб-клиент (браузер) передаёт в `XMLHttpRequest` только HTTP-метод, URL, BODY и `HEADERS`; опции `COOKIES`, `COOKIESTO`, свойства `System.timeoutHttp[]` и `System.insecureSSL[]` там игнорируются — cookies управляются собственным cookie jar браузера, а таймауты и проверка TLS - самим браузером.
:::
...
Собственные [заголовки](https://en.wikipedia.org/wiki/List_of_HTTP_header_fields) и [cookies](https://en.wikipedia.org/wiki/HTTP_cookie) запроса можно задать для вызова опциями `HEADERS` и `COOKIES`: каждая опция принимает свойство, отображающее имя (читается из единственного параметра) в значение (читается из значения свойства).

Литеральный текст строки подключения и строки `BODYURL` URL-кодируется перед отправкой запроса (можно отключить опцией `NOENCODE`); значения параметров, подставляемых через `$N`, URL-кодируются независимо.
...
[Заголовки](https://en.wikipedia.org/wiki/List_of_HTTP_header_fields) ответа записываются в свойство, заданное опцией `HEADERSTO`. Так, в единственный параметр этого свойства записывается название заголовка, а в значение свойства - значение этого заголовка.

[Cookies](https://en.wikipedia.org/wiki/HTTP_cookie) ответа аналогично захватываются опцией `COOKIESTO`: в возвращаемых парах `name -> value` объединяются cookies, отправленные через `COOKIES`, и cookies, полученные в заголовках `Set-Cookie` ответа; атрибуты `path`, `domain` и прочие теряются.

HTTP-[код ответа](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes) записывается в свойство `System.statusHttp[]`. Статус, не попадающий в `2xx`, также приводит к runtime-исключению с кодом и телом ответа; его можно перехватить оператором [`TRY`](TRY_operator.md), чтобы прочитать `System.statusHttp[]`.
...
По умолчанию запрос выполняется на сервере приложений; ключевое слово `CLIENT` выполняет его с клиента пользователя.


:::info
При `CLIENT` прямая работа с сокетами доступна локально в desktop-клиенте и через Flutter-мост во Flutter-клиенте (веб/мобильном); обычный веб-клиент (браузер) прямого доступа к сокетам не имеет и падает с `UnsupportedOperationException`.
:::
