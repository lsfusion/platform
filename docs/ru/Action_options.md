...
- `annotationSetting`
  ...
  Аннотация действия. Начинается с @@. Поддерживаются следующие аннотации:
    ...
    - `@@ui`
      ...
      Принудительно помечает действие как [интерактивное](Access_from_an_external_system.md#interactive) (требующее UI) при вызове через внешний API.

    - `@@noui`

      Отключает автоматическое определение [интерактивности](Access_from_an_external_system.md#interactive), так что действие выполняется синхронно при вызове через внешний API (если только клиент явно не запросил идентификатор уведомления через заголовок `Need-Notification-Id`).
