---
slug: "/How-to_Using_objects_as_templates"
...
When the button is pressed, a new [change session](../paradigm/Change_sessions.md) will be created within which the book will be created and the new form will work with. If the user closes the form without saving, the new book will not be saved to the database and will be lost. The word `TOOLBAR` specifies that this button should be displayed in the toolbar of the table with the list of orders.
...
We need to implement an [action](../paradigm/Actions.md) that will create an invoice based on an order and open a form for editing it.
...
We need to implement an [action](../paradigm/Actions.md) that will open a dialog for the invoice with a list of orders and add lines from the selected one.
