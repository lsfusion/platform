...
We need to create a [print form](../paradigm/Print_view.md) to display all the books by category. We also need to export this form to the XLSX format.

### Solution

First, we need to declare a [form](../paradigm/Forms.md) to define the print form structure.
...
Then, we add two actions that use the [`PRINT` operator](../language/PRINT_operator.md) for creating a report and for previewing it and exporting to XLSX respectively.
...
Then, we start the server from the IDE, launch the desktop client, open the form with books and click the created "Print" button. The system will automatically create a print-ready form and open it in the [preview](../paradigm/In_a_print_view_PRINT.md#interactive) mode. Then, click the button below:
