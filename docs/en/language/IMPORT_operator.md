...
The `IMPORT` operator creates an [action](../paradigm/Actions.md) that imports data from a specified file into [specified properties (parameters)](../paradigm/Data_import_IMPORT.md) or, in general, into a [specified form](../paradigm/In_a_structured_view_EXPORT_IMPORT.md#importForm).
...
- `objClassId1, ..., objClassIdK`
    ...
    Classes of the [imported](../paradigm/Data_import_IMPORT.md) objects. Specified by [class IDs](IDs.md#classid). `K <= 1`. By default, it is assumed that what is being imported is one object of class `INTEGER`.
...
- `propClassId1, ..., propClassIdN`
    ...
    List of names of [builtin classes](../paradigm/Built-in_classes.md) of the imported columns.
