...
The `NEWEXECUTOR` operator creates an [action](../paradigm/Actions.md) that runs other actions in a [new execution service](../paradigm/New_threads_NEWTHREAD_NEWEXECUTOR.md) — a server-side thread pool or a client-side dispatcher.
...
- `connectionExpr`
    ...
    An expression whose value is the [connection](../paradigm/User_IS_interaction.md) whose interactive context is used to dispatch the nested `NEWTHREAD`s. The return class is `SystemEvents.Connection`. If the value is `NULL` (or does not resolve to a `SystemEvents.Connection` object), the operator exits without executing the inner action and without raising an error.
