...
Each [module](../paradigm/Modules.md) begins with a *header*.
...
The `MODULE`  statement defines the module name. It is required. Each module within one [project](../paradigm/Projects.md) must have a unique name.

The `REQUIRE` statement defines the list of modules on which the current module [depends](../paradigm/Modules.md#depends). If the `REQUIRE` statement is absent, that is equivalent to depending only on the `System` module.

The `PRIORITY` statement defines the list of additional [namespaces](../paradigm/Naming.md#namespace) that will have priority in [finding](../paradigm/Search_.md) [system elements](../paradigm/Element_identification.md).
