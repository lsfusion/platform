...
The `NEW` operator creates an action which creates objects of the specified class and writes them as the returned values of certain [properties](../paradigm/Properties.md). This operator can add its local [parameters](../paradigm/Actions.md) when defining a condition in the `WHERE` block. These parameters correspond to the objects being iterated for which the object will be created and are not parameters of the created action. 

The `TO` block is used to define the [data property](../paradigm/Data_properties_DATA.md) to which value the created object is written. 
...
- `className`
    ...
    Name of the [custom](../paradigm/User_classes.md) class for the created objects. [Composite ID](IDs.md#cid).
