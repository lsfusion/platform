...
1.  Recursively builds an intermediate property (result) with an additional first parameter (operation number) as follows:
    ...
    2.  `result(i+1, o1, o2, ..., oN) = step(o1, ..., oN, $o1, $o2, ..., $oN) IF result(i, $o1, $o2, ..., $oN)`, where `step` is a *step* property and `$o1, ..., $oN` denote the parameter values at the previous iteration (in source code these are written as `$name` — see the [`RECURSION` operator](RECURSION_operator.md)).
