...
Какая ветка: `language` — синтаксис инструкций и операторов; `paradigm` — концепции и библиотеки системных модулей; `how-to` — рецепты задач. Если не уверены — не указывайте фильтр: по умолчанию поиск идёт по всем трём.
...
- **Параметры результата**: `BY`-выражения плюс использованные верхние параметры; использованный верхний параметр в `BY` не указывается — при явном списке параметров слева `BY`-выражения отображаются по порядку на неиспользованные параметры.
...
- **Описание**: инструкция `@` генерирует код из описаний; IDE может автогенерировать. Тело `META` — инструкции уровня модуля (не операторы действий); инструкция `@` — тоже уровня модуля.
- **Синтаксис (поиск)**: `META name(param1, ...) ... END`, `@name(arg1, ...);`

---

## Идентификация и эргономика
- **Пространства имён**: управляют видимыми именами. **Синтаксис (поиск)**: `NAMESPACE MyNamespace;`
- **Явная типизация**: параметры/локальные свойства типизированы. **Синтаксис (поиск)**: `LOCAL var = Type ();` (только внутри тела действия; на уровне модуля — `var = DATA LOCAL Type ();`)
