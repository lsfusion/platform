...
| --- | --- | --- | --- |
| `EVENTS ON SCHEDULE PERIOD n` [формы](../paradigm/Form_events.md) | на клиенте, у каждой открытой формы | на сервере, в сессии формы; асинхронный запрос, не блокирующий пользователя | к форме: выполняется, только пока она показана на экране, скрытая форма пропускает запуски |
| `SCHEDULE PERIOD n` в [`NAVIGATOR`](../language/NAVIGATOR_statement.md) | на клиенте, у соединения | на сервере, каждый раз в новой сессии; синхронный запрос: ввод заблокирован на время выполнения, индикатор ожидания — после его задержки | к соединению: выполняется, пока пользователь в системе, какие бы формы ни были открыты |
| `NEWTHREAD ... SCHEDULE PERIOD` ([потоки](../paradigm/New_threads_NEWTHREAD_NEWEXECUTOR.md)) | в серверном пуле либо на клиенте при `NEWEXECUTOR ... CLIENT` | на сервере, в сессии вызывающего кода (пул) либо в новой сессии соединения (`CLIENT`) | к объемлющему `NEWEXECUTOR` (при `CLIENT` — к соединению) |
| [планировщик](../paradigm/Scheduler.md) | на сервере | на сервере, без участия клиента | к серверу: задача, настроенная администратором |

Обновление формы, на которую смотрит пользователь, — `SCHEDULE` формы с обработчиком `System.formRefresh[]`; фоновая работа с данными — задача планировщика; работа одного соединения — навигатор; отложенный или повторяемый шаг действия — `NEWTHREAD`. Период у формы и навигатора отсчитывается от окончания предыдущего запуска, с `FIXED` — от его начала.
