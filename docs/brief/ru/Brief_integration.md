...
Импортируемая форма ограничена: объекты только числовых или конкретных пользовательских классов, в группе ровно один объект, свойства и фильтры изменяемые (как правило, [первичные свойства](Brief_logic.md#свойства)). Для каждого прочитанного объекта во все фильтры его группы пишется значение по умолчанию: `FILTERS order(od) = o` записывает связь с верхним объектом, `FILTERS imported(o)` — признак прочитанного объекта; без такого фильтра `System.imported[INTEGER]` не заполняется. Плоский импорт — его частный случай: форму платформа строит сама. Механизм — [в структурированном представлении](../paradigm/In_a_structured_view_EXPORT_IMPORT.md), представления формы — [Brief: формы](Brief_view.md#формы).
