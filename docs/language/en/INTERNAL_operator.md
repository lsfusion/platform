...
The `INTERNAL` operator creates an [action](../paradigm/Actions.md) or an action statement that executes an [internal call](../paradigm/Internal_call_INTERNAL.md).
...
- `DB`
    ...
    Keyword. Runs SQL against the platform's own database. Semantics match [`EXTERNAL SQL`](../paradigm/Access_to_an_external_system_EXTERNAL.md#sql) — `$N` parameter substitution, `TABLE` parameters uploaded as temporary tables, an `.sql` expression treated as a classpath resource, and DML/`SELECT` results written to `TO` properties — except that no connection string is supplied and the call is performed inside the current change [session](../paradigm/Change_sessions.md). Mutually exclusive with `CLIENT`.
