...
The `ABSTRACT` operator creates an [abstract action](../paradigm/Action_extension.md).
...
The `ABSTRACT` operator creates an abstract action. Its implementations are added later by [`ACTION+` statements](ACTION_plus_statement.md). Depending on the selected type, the platform builds from them the behavior of a [branch operator](../paradigm/Branching_CASE_IF_MULTI.md) or a [sequence operator](../paradigm/Sequence.md).
...
- `exclusionType`
    ...
    Option. It specifies the [type of mutual exclusion](../paradigm/Branching_CASE_IF_MULTI.md#exclusive). Possible values:
