---
slug: "/ACTIVE_operator"
title: 'ACTIVE operator'
---

The `ACTIVE` operator creates a [property](../paradigm/Properties.md) or an [action](../paradigm/Actions.md) that returns information about an [active](../paradigm/Activity_ACTIVE.md) form element: a tab, a property (or action), a form, or the current object of an object group.
...
The `ACTIVE FORM` form creates an action that performs the activity check for the specified form for the current user and writes the result (`TRUE` or `FALSE`) into the built-in [local](../paradigm/Data_properties_DATA.md#local) `System.isActiveForm[]` property. The result is available for reading via the `isActiveForm()` property within the same action block / session.
