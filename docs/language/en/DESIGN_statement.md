...
|`lsf`|Keeps the standard view of a child of a React `custom` container, which the component then places with `<Lsf name/>` instead of drawing it (web client only). Valid only on a direct child of such a container; anywhere else the form is rejected when it is built|[Logical literal](Literals.md#booleanliteral)|`FALSE`|`TRUE`<br/>`FALSE`|
...
|`lines`|Number of lines the children are laid out in: columns in a vertical container, rows in a horizontal one. The children are distributed over the lines in turn, in their order: the first goes to the first line, the second to the second, …, child number `lines + 1` to the first line again. With `lines = 1` all the children form a single column (in a vertical container) or a single row (in a horizontal one). The `GROUP(...)` containers of the [default design](../paradigm/Form_design.md#defaultDesign) are created with `lines = 3`, so the default `1` does not apply to them: a single column in such a container requires an explicit `lines = 1`|Integer literal|`1` (`3` for the `GROUP(...)` containers of the default design)|`3`|
...
|`reversed`|Lays the children out in a single line in the direction opposite to the one set by the `horizontal` option (the `lines` value is not applied then). The default is `TRUE` if there are several lines, no more children than lines, and the container is not a `grid`: each child would take a line of its own, so instead of lines the children are simply placed in one row (in a vertical container) or one column (in a horizontal one); otherwise `FALSE`|Logical literal|depends on the container|`TRUE`<br/>`FALSE`|
