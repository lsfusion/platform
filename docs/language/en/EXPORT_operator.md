...
The `EXPORT` operator: creates an [action](../paradigm/Actions.md) that exports [specified properties](../paradigm/Data_export_EXPORT.md) to a file, or, in common case, that [opens a form](../paradigm/In_a_structured_view_EXPORT_IMPORT.md) in a structured view. 
...
- `whereExpr`
    ...
    An expression whose value is a condition for the export. If not specified, it is considered equal to the [disjunction](../paradigm/Logical_operators_AND_OR_NOT_XOR.md) of all exported properties (that is, at least one of the properties must be non-`NULL`).
