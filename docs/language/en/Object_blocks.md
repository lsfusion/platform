...
Object blocks of the [`FORM` statement](FORM_statement.md) - adding [object groups](../paradigm/Form_structure.md#objects) to the form structure, and [object trees](../paradigm/Interactive_view.md#tree) to the interactive form view.
...
A single `OBJECTS` block can contain several comma-delimited declarations of [object groups](../paradigm/Interactive_view.md#objects). An object group can contain just one object or several ones. In case of a single object, you can use simplified syntax without specifying the name of an object group and parentheses.
...
- `ON CHANGE actionId(param1, ..., paramM)`
    ...
    Specifying an [action](../paradigm/Actions.md) that will be called when the current value of the object changes.
...
- `ON CHANGE actionOperator`
    ...
    Creating an [action](../paradigm/Actions.md) that will be called when the current value of the object changes.
...
- `EXTID objectExtID`
    ...
    Specifying the name that will be used for [export/import](../paradigm/Structured_view.md#extid) of this form object. Used only in the [structured](../paradigm/Structured_view.md) view.
...
- `viewType`
    ...
    The [default view](../paradigm/Interactive_view.md#property) for properties of this object group. Specified by one of the following ways:
...
- `insertPosition`
    ...
    Specifying the insertion position of the object group within the list of object groups. Most often used together with the [form extension mechanism](../paradigm/Form_extension.md). It can be specified in one of the following ways:
...
- `defaultObjectsType`
    ...
    - `FIRST`– specifies that the first object collection (according to the current order) will be the [default objects](../paradigm/Interactive_view.md#defaultobject)
...
- `IN propertyGroup`
    ...
    Specifying the [property and action group](../paradigm/Groups_of_properties_and_actions.md) that the object group belongs to. Used only in the [hierarchical](../paradigm/Structured_view.md#hierarchy) view.
...
- `EXTID extID`
    ...
    Specifying the name to be used for [export/import](../paradigm/Structured_view.md#extid) of this object group. Used only in the structured view.
...
- `SUBREPORT [subReportExpression]`
    ...
    Specifies that you need to generate a separate [report](../paradigm/Print_view.md) file for this object group while [building the report hierarchy](../paradigm/Print_view.md#buildhierarchy).

    - `subReportExpression`

        The [expression](Expression.md) whose value will be used as the name of the  [report](../paradigm/Print_view.md) file for the created object group. You can use the names of already declared objects on the form as parameters. It is assumed that the values of these objects will be [passed](../paradigm/Open_form.md#params) when the form is opened [in the print view](../paradigm/In_a_print_view_PRINT.md) (if it's not done, they will be considered equal `NULL`).
...
*Object tree block* lets you create an [object tree](../paradigm/Interactive_view.md#tree). The first specified object group will form a list of top-level objects, each of which will have a child list of objects of the second specified object group and so  on.

Use the `PARENT` option to create [hierarchical object groups](../paradigm/Interactive_view.md#treegroup). To do that, specify a property that will define the parent element for an object (or several objects if an object group contains several ones). If an object group consists of several objects, the `PARENT` option is specified for each object inside the parentheses of the group declaration.
