...
|---|---|---|---|---|
|<a className="lsdoc-anchor" id="strliteral"/>String literal|`'text'`<br/>`'{form.title}'`<br/>`'a${x}b'`|`STRING[N]`, `TEXT`|A sequence of characters in single quotes. See [String literal](String_literal.md) for escaping, multi-line, [localization](../paradigm/Internationalization.md), interpolation, and inline sequences.||
|<a className="lsdoc-anchor" id="rawstrliteral"/>Raw string literal|`r'text'`<br/>`r'C:\dir\file'`<br/>`r!'don't stop'!`|`STRING[N]`|A literal with the `r`/`R` prefix whose content is taken as is (no escaping, localization, or reverse translation). See [String literal](String_literal.md).||
...
The `YEAR` class has no literal of its own: a year value is written with the [type conversion operator](Type_conversion_operator.md) applied to an integer literal, `YEAR(2024)`.
