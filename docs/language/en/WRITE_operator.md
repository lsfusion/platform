...
The `WRITE` operator creates an [action](../paradigm/Actions.md) that [writes a file](../paradigm/Write_file_WRITE.md) from a property to an external resource. 
...
- `APPEND`
    ...
    With `CLIENT`, merging into an existing `xls`, `xlsx`, `docx` or `pdf` file is done only by the desktop client.
