...
- `$I{resourceName}` — includes the content of a **text** file `resourceName` in the string literal. The file is read at code parse time, and its content becomes part of the literal. Interpolation blocks `${...}` in it are processed as ordinary interpolation with the parameters of the current construction. Because of this, the literal sequence `${` cannot be used in the file (for example, in JavaScript or CSS). It cannot be escaped either, so a file containing it (a shell script, for example) cannot be included; to show the content of such a resource as text, read it at run time with the `readResource` and `fileToString` actions of the [`Utils`](../paradigm/System_Utils.md) module (the result lands in the local property `resultString[]`). All other characters, such as quotes, backslashes, and curly braces, do not need to be escaped in the file — the platform does it automatically. Can be used to keep large parameterizable templates (HTML emails, pages, fragments, etc.) in separate files.
- `$R{resourceName}` — embeds the content of the file `resourceName` into the string not at code parse time, but at the moment the **value is sent to the client**. The file may be **arbitrary**, including binary (an image, a PDF); what is substituted into the string is not the file text but a reference to it (a URL in the web client), so its content is not processed as code. Can be used inside HTML.
