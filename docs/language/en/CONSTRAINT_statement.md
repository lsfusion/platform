...
The `CONSTRAINT` statement creates a [constraint](../paradigm/Constraints.md).
...
but it also has [a number of advantages](../paradigm/Constraints.md).
:::
...
- `constraintExpr`
    ...
    An [expression](Expression.md) whose value is a condition for the constraint being created. If the obtained property does not contain the [`PREV` operator](PREV_operator.md), the platform automatically wraps it into the [`SET` operator](../paradigm/Change_operators_SET_CHANGED_etc.md).
