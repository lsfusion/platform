---
slug: "/NEWTHREAD_operator"
title: 'NEWTHREAD operator'
---

The `NEWTHREAD` operator creates an [action](../paradigm/Actions.md) that executes another action in a [new thread](../paradigm/New_threads_NEWTHREAD_NEWEXECUTOR.md).

### Syntax

```
NEWTHREAD action [dispatchClause]
```

where `dispatchClause` has one of the following forms:

```
SCHEDULE [PERIOD periodExpr] [DELAY delayExpr] [TO toPropertyId]
CLIENT notificationPropertyId [TO toPropertyId]
TO toPropertyId
```
