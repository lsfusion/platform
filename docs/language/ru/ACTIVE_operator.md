...
Оператор `ACTIVE` - создание [свойства](../paradigm/Properties.md) или [действия](../paradigm/Actions.md), возвращающего информацию об [активном](../paradigm/Activity_ACTIVE.md) элементе формы: закладке, свойстве (или действии), форме либо текущем объекте в группе объектов.
...
Форма `ACTIVE FORM` создаёт действие, выполняющее проверку активности указанной формы у пользователя и записывающее результат (`TRUE` или `FALSE`) во встроенное [локальное](../paradigm/Data_properties_DATA.md#local) свойство `System.isActiveForm[]`. Результат после этого доступен для чтения через `isActiveForm[]` в том же сеансе / блоке действия.
