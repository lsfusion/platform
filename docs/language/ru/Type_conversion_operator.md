...
Объект пользовательского класса преобразуется к числовым и строковым классам (результат — внутренний идентификатор объекта) и к логическому (результат — `TRUE`); значение, суженное до пользовательского класса, возвращает оператор [`AS`](IS_AS_operators.md).
