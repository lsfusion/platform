...
Блок фиксированных фильтров добавляет на форму фильтры, которые будут автоматически применяться при чтении любых данных на форме (фильтр типа `USER` вместо этого показывается пользователю строкой условия и применяется, когда введено его значение). В одном блоке можно перечислить через запятую произвольное количество фильтров.
...
- `USER | FIXED`
    ...
    `USER` - [пользовательский фильтр](../paradigm/Interactive_view.md#userfilters): строка условия свойства показывается сразу при открытии формы. Выражением должно быть [имя свойства на форме](Properties_and_actions_block.md#name), то есть свойство должно быть предварительно добавлено на форму.
...
- `nullType`
    ...
    Поскольку фильтр `(Все)` присутствует в группе по умолчанию, не следует добавлять в группу отдельный фильтр с всегда истинным условием — он лишь дублирует фильтр `(Все)`.
