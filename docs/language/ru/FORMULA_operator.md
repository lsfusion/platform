...
Оператор `FORMULA` - создание [свойства](../paradigm/Properties.md), реализующего [пользовательскую формулу](../paradigm/Custom_formula_FORMULA.md).
...
Что именно создаёт оператор — результирующий класс по умолчанию, ослабления обработки `NULL` и табличный режим — описано в абстракции [пользовательской формулы](../paradigm/Custom_formula_FORMULA.md); эта статья описывает только то, как эти варианты записываются.
