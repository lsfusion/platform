...
- `orderExpr1, ..., orderExprL`
    ...
    Список [выражений](Expression.md), по которым производится сортировка экспортируемых данных. Выражения произвольны: они не обязаны входить в список `propertyExpr1, ..., propertyExprN`. Выражение сортировки добавляется во внутренний запрос скрытой колонкой и в результат экспорта не попадает.
...
Значения классов даты и времени в текстовых форматах (**JSON**, **XML**, **CSV**) записываются строками: по умолчанию в форматах ISO 8601 (например, `2026-08-17`, `2026-08-17T12:34:56`; значения `ZDATETIME` — момент времени в UTC с суффиксом `Z`). Это поведение определяется [рабочим параметром](../paradigm/Working_parameters.md) `useISOTimeFormatsInIntegration`: если он сброшен, используются форматы текущей локали. В форматы **XLS** и **XLSX** такие значения записываются типизированными значениями даты/времени, а не строками. В формате **DBF** значения `DATE` и `DATETIME` записываются в поле даты (время у `DATETIME` при этом теряется), а `TIME` — строкой `HH:mm:ss`.
