...
- `SCHEDULE PERIOD intPeriod [FIXED] action`
    ...
    Cоздаёт планировщик, выполняющий действие `action` каждые `intPeriod` секунд. Ключевое слово `FIXED` указывает на то, что период до следующего действия отсчитывается от старта текущего действия. По умолчанию период отсчитывается от окончания текущего действия.
