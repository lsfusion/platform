...
Остальные опции (`manageSessionType`, `sessionScopeType`, `cancelType`) принимают значения в виде ключевых слов, перечисленные в их параметрах ниже.
...
- `NOCONSTRAINTFILTER`
    ...
    Ключевое слово (после `CHANGE`). Отключает фильтр соответствия ограничениям, который `CHANGE` добавляет по умолчанию.

- `NOCHANGE`

    Ключевое слово (после `CHANGE`). Подавляет запись обратно; возвращённое значение связывается с `alias`, но никакое свойство не обновляется. Используется, чтобы получить поведение фильтра ограничений от `CHANGE` без неявного присваивания.
