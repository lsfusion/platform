...
- `NULL` после начального значения
    ...
    Ключевое слово. Разрешает передаче начального значения быть `NULL`. По умолчанию, если какое-либо передаваемое значение равно `NULL`, действие пропускается, и управление передаётся следующему действию. Переданный `NULL` не становится значением объекта: он равносилен отсутствию значения, и текущим становится [объект по умолчанию](../paradigm/Interactive_view.md#defaultobject) группы. Автоматически включается, если для объекта указан маркер `INPUT` / `CHANGE`.
...
- `windowType`
    ...
    [Расположение формы](../paradigm/In_an_interactive_view_SHOW_DIALOG.md#location): `FLOAT` показывает форму как плавающее окно, `WINDOW` — как закладку в системном окне форм `System.forms` или в [окне `FORMS`](WINDOW_statement.md), которое называет `windowName`, `EMBEDDED` и `POPUP` — как встроенные редакторы, `IN` помещает форму внутрь `containerName` — [компонента дизайна](DESIGN_statement.md#selector), квалифицированного формой (имя формы, за которым следует путь к компоненту внутри дизайна этой формы), который должен быть контейнером. По умолчанию в синхронном режиме используется `FLOAT`, в асинхронном — `WINDOW`. В синхронном режиме закладка `WINDOW` до своего закрытия блокирует форму, из которой открыта, где бы она ни рисовалась, а из формы, показанной окном, показывается окном, как `FLOAT`. Мобильный веб-клиент и десктоп-клиент рисуют одно окно `System.forms`, поэтому там форма, названная в другое окно, открывается в `System.forms`.

    - `windowName`

        Имя окна, в которое помещается форма. [Составной идентификатор](IDs.md#cid) окна, объявленного как `WINDOW ... FORMS`; любое другое окно — ошибка.
