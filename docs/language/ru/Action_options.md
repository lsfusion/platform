...
При объявлении [действия](../paradigm/Actions.md) в [инструкции `ACTION`](ACTION_statement.md) в конце объявления может указываться некоторый набор *опций действия*. 
...
- `IN groupName`
    ...
    Указание [группы свойств и действий](../paradigm/Groups_of_properties_and_actions.md), к которой принадлежит действие. Если опция не указывается, то действие по умолчанию будет принадлежать группе `System.private`.
...
- `viewType`
    ...
    Указание типа [представления действия](../paradigm/Interactive_view.md#property) при добавлении на форму.

    - `GRID` - колонка таблицы
    - `TOOLBAR` - тулбар
    - `PANEL` - панель

  По действию аналогично указанию опции viewType в [блоке свойств](Properties_and_actions_block.md) [инструкции `FORM`](FORM_statement.md). Соответственно, если эта опция не указана ни в опциях действий, ни в блоке свойств непосредственно на форме, используется тип [представления по умолчанию](../paradigm/Interactive_view.md#property) для группы отображения, добавляемого на форму действия.

- `ON eventType { actionOperator }`

    Указание действия, которое будет обработкой по умолчанию некоторого [события](../paradigm/Form_events.md) всех интерактивных представлений этого действия. Могут быть переопределены в блоке свойств инструкции `FORM`.

    - `eventType`

        Тип [события формы](../paradigm/Form_events.md). Задается в одной из следующих форм:

        - `CHANGE [eventPhase]` - попытка изменения значения указанного свойства.

            - `eventPhase`

                Момент выполнения обработки относительно изменения значения свойства. Задается одним из ключевых слов:

                - `BEFORE` - обработка выполняется непосредственно до фактического изменения значения свойства.
                - `AFTER` - обработка выполняется непосредственно после фактического изменения значения свойства.

                Если не указан, обработка выполняется при попытке изменения значения и замещает обработку по умолчанию.

        - `CHANGEWYS` - попытка изменения значения указанного свойства с помощью специального механизма ввода. Подробнее можно прочитать в описании [событий формы](../paradigm/Form_events.md).
        ...
        - `CONTEXTMENU [caption]` - пользователь выбрал в контекстном меню свойства на форме заданный пункт.

            - `caption`

                [Строковый литерал](Literals.md#strliteral), задающий заголовок пункта контекстного меню. Если не указан, по умолчанию равен заголовку действия.

        - `KEYPRESS key` - пользователь нажал заданную кнопку на клавиатуре.

            - `key`

                Строковый литерал, задающий кнопку на клавиатуре.
...
- `ASON eventType [ACTION] propertyId`
    ...
    Указание того, что это действие будет являться обработкой по умолчанию некоторого [события](../paradigm/Form_events.md) всех интерактивных представлений заданного свойства или действия. Может быть переопределено в [блоке свойств и действий](Properties_and_actions_block.md) инструкции `FORM`. 
...
- `imageSetting`
    ...
    Настройка иконки действия. Один из вариантов:
    ...
    - `NOIMAGE`
        ...
        У действия не должно быть иконки.
...
- `annotationSetting`
  ...
  Аннотация действия. Начинается с @@. Поддерживаются следующие аннотации:
    ...
    - `@@noauth`
      ...
      Полностью обходит и проверку аутентификации, и настройку `enableAPI` при внешнем запросе для этого действия.

    - `@@api`

      При отключенном API (`enableAPI=0`) разрешает авторизованные внешние запросы для данного действия. Авторизованные пользователи, у которых есть доступ к форме навигатора `System.interpreter`, также могут использовать внешний API при `enableAPI=0` даже без этой аннотации.

    - `@@ui`

      Принудительно помечает действие как [интерактивное](../paradigm/Access_from_an_external_system.md#interactive) (требующее UI) при вызове через внешний API.

    - `@@noui`

      Отключает автоматическое определение [интерактивности](../paradigm/Access_from_an_external_system.md#interactive), так что действие выполняется синхронно при вызове через внешний API (если только клиент явно не запросил идентификатор уведомления через заголовок `Need-Notification-Id`).

    - `@@deprecated`
    - `@@deprecated(since)`
    - `@@deprecated(since,message)`

        Пометка действия как устаревшего и не рекомендованного для использования.
        Плагин показывает использование таких действий перечёркнутым.
...
- `EXTID extID`
    ...
    Указание имени, которое будет использоваться для поиска действия при [обращении из внешней системы](../paradigm/Access_from_an_external_system.md#http).

### Блок значений по умолчанию инструкции `DESIGN`

- `CHANGEKEY key [SHOW | HIDE]`

    Указание [комбинации клавиш](../paradigm/Form_events.md#keyboard), при нажатии которой будет выполнено это действие. Устанавливает значение для [дизайна по умолчанию](../paradigm/Form_design.md#defaultDesign), может быть переопределено в [инструкции `DESIGN`](DESIGN_statement.md).
...
- `STICKY` | `NOSTICKY`
    ...
    Ключевые слова. `STICKY` указывает на то, что действие в таблице будет прикреплено слева и при скроллинге вправо будет оставаться видимым. `NOSTICKY` снимает это закрепление. По умолчанию действие не закрепляется.
...
- `CONFIRM`
    ...
    Ключевое слово. Если указано, то при интерактивном запуске действия на форме пользователю будет задан вопрос о подтверждении запуска. Подтверждение запрашивается клиентом только при интерактивном запуске (например, по нажатию кнопки); при вызове действия из другого действия или через [внешний API](../paradigm/Access_from_an_external_system.md) подтверждение не запрашивается. Устанавливает значение для дизайна по умолчанию (атрибут [`askConfirm`](DESIGN_statement.md)), может быть переопределено в инструкции `DESIGN`.
