...
Инструкция `+=` относится только к свойствам. Реализация абстрактного действия добавляется [инструкцией `ACTION+`](ACTION_plus_statement.md): после знака `+` записывается тело действия, а не выражение.
...
Для абстрактного свойства во [взаимоисключающем](../paradigm/Property_extension.md#exclusive) режиме (по умолчанию для `MULTI`) платформа при финализации модулей на старте сервера проверяет, что условия реализаций не пересекаются; для формы `MULTI` это сигнатуры реализаций, и пересечение останавливает запуск с ошибкой `signature intersection of property ... for abstract property ...`. В частности, абстрактное свойство без параметров в этом режиме допускает не более одной реализации: второй `+=`, даже из другого модуля, — такое пересечение. Свойство, которое должно принимать несколько одновременно подходящих реализаций либо реализацию по умолчанию и переопределяющую её, объявляется в режиме `OVERRIDE`, который записывается после формы: у `ABSTRACT VALUE` он действует по умолчанию — применяется добавленная позже реализация, когда она возвращает значение, иначе более ранняя; `ABSTRACT MULTI OVERRIDE` применяет более конкретную сигнатуру, а при равных сигнатурах — добавленную позже.
