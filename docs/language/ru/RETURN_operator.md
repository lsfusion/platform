...
Оператор `RETURN` создает действие, которое выходит из наиболее вложенного [вызова действия](../paradigm/Call_EXEC.md). Если задано `resultExpr`, значение этого выражения становится результатом этого вызова. Иначе вызов просто завершается, не возвращая значения. В аспекте [инструкции `BEFORE`](BEFORE_statement.md) оператор `RETURN` завершает и вызов основного действия: его тело не выполняется. Реализация, добавленная к [абстрактному действию](../paradigm/Action_extension.md), не является отдельным вызовом: `RETURN` в ней выходит из вызова самого абстрактного действия, поэтому в форме `LIST` следующие за ней реализации не выполняются.
