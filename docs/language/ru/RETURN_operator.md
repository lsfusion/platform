...
Оператор `RETURN` создает действие, которое выходит из наиболее вложенного [вызова действия](../paradigm/Call_EXEC.md). Если задано `resultExpr`, значение этого выражения становится результатом этого вызова. Иначе вызов просто завершается, не возвращая значения. В аспекте [инструкции `BEFORE`](BEFORE_statement.md) оператор `RETURN` завершает и вызов основного действия: его тело не выполняется.
