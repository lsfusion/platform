...
Оператор `NESTEDSESSION` создаёт действие, которое выполняет другое действие во вложенной сессии. Применение изменений во вложенной сессии копирует их обратно в окружающую сессию, а не записывает в базу данных. Если оператор выполняется во время [транзакции применения](../paradigm/Apply_changes_APPLY.md) текущей сессии, [вложенная сессия не создаётся](../paradigm/New_session_NEWSESSION_NESTEDSESSION.md) — выполняемое действие откладывается и выполняется в текущей сессии внутри той же транзакции.
