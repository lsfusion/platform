...
- `type`
    ...
    - `LIST` - последовательная форма абстрактного действия. В этой форме все реализации выполняются по очереди, пока одна из них не завершит действие [оператором `RETURN`](RETURN_operator.md).
...
- `order`
    ...
    Порядок добавления между модулями — это порядок инициализации модулей: реализации из [требуемых модулей](Module_header.md) добавляются раньше реализаций из зависящих от них модулей, а внутри одного модуля — в порядке следования в исходном коде. В частности, в форме `LIST` (без `FIRST`) реализации из базового модуля выполняются раньше реализаций, добавленных к тому же действию зависимыми модулями.
