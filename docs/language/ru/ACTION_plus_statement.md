...
Для абстрактного действия типа `CASE` блок `WHEN conditionExpr THEN` обязателен. Для действий типов `MULTI` и `LIST` блок `WHEN ... THEN` не используется, и реализация записывается сразу после `+`.

Реализация всегда записывается как тело действия после знака `+`. Инструкция [`+=`](plus_equals_statement.md) к действиям не относится: она добавляет реализацию абстрактного свойства и принимает выражение, поэтому запись `abstractAction(...) += { ... }` не разбирается.
