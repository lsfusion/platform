...
Оператор `NEWSESSION` - создание [действия](../paradigm/Actions.md), которое выполняет другое действие в [новой сессии](../paradigm/New_session_NEWSESSION_NESTEDSESSION.md).
...
При указании `NESTED LOCAL` или `NESTED (propertyId1, ..., propertyIdN)` в новой сессии становятся видны изменения соответствующих [локальных свойств](../paradigm/Data_properties_DATA.md#local) текущей сессии. Также изменения этих локальных свойств в новой сессии попадут в текущую сессию при применении изменений в новой сессии.

### Параметры

- `NEWSQL`

    Ключевое слово. Если оператор выполняется не во время уже идущей транзакции применения, новая сессия получает [отдельное SQL-соединение](../paradigm/New_session_NEWSESSION_NESTEDSESSION.md#newsql), независимое от соединения текущей сессии, и весь блок `NESTED ... [CLASSES]` игнорируется — в новую сессию не переносятся ни значения локальных свойств, ни изменения классов. Если же оператор вызван внутри `APPLY`-транзакции текущей сессии, поведение определяется правилами рекурсивного применения и отдельное SQL-соединение в этом случае не открывается.
