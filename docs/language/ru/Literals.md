...
|---|---|---|---|---|
|<a className="lsdoc-anchor" id="strliteral"/>Строковый литерал|`'text'`<br/>`'{form.title}'`<br/>`'a${x}b'`|`STRING[N]`, `TEXT`|Последовательность символов в одинарных кавычках. Экранирование, многострочность, [локализацию](../paradigm/Internationalization.md), интерполяцию и встраиваемые последовательности см. в статье [Строковый литерал](String_literal.md).||
|<a className="lsdoc-anchor" id="rawstrliteral"/>Сырой строковый литерал|`r'text'`<br/>`r'C:\dir\file'`<br/>`r!'don't stop'!`|`STRING[N]`|Литерал с префиксом `r`/`R`, содержимое берётся как есть (без экранирования, локализации и обратного перевода). См. [Строковый литерал](String_literal.md).||
...
У класса `YEAR` нет собственного литерала: значение года записывается [оператором приведения типа](Type_conversion_operator.md), примененным к целочисленному литералу, `YEAR(2024)`.
