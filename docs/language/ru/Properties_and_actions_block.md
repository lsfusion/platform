...
Блок свойств и действий [инструкции FORM](FORM_statement.md) - добавление [свойств и действий](../paradigm/Form_structure.md#properties) в [структуру формы](../paradigm/Form_structure.md). 
...
Если были указаны общие параметры блока `(cparam1, ..., cparamN)` — скобки после `PROPERTIES` присутствуют, пусть даже пустые:
...
- `cparam1, ..., cparamN`
    ...
    Список общих параметров блока. Каждый параметр задается именем объекта формы. Если этот список задан, то при добавлении свойства (действия) нужно указывать только его идентификатор без параметров. Имя объекта задается [простым идентификатором](IDs.md#id). Список может быть и пустым: `PROPERTIES()` — это заголовок с нулём общих параметров, а не его отсутствие, поэтому и в таком блоке свойство (действие) указывается одним идентификатором, и добавить так можно только свойства и действия без параметров — `PROPERTIES() total, newCustomer` равносильно `PROPERTIES total(), newCustomer()`.
...
- `formPropertyId`
    ...
    Также в этом правиле, вместо идентификаторов свойств / действий, можно использовать [операторы работы с объектами](../paradigm/Interactive_view.md#objectoperators):

    - `VALUE` - показывает значение объекта: для объекта [встроенного класса](../paradigm/Built-in_classes.md) (например, `STRING` или `DATE`) это свойство, через которое значение такого объекта показывается на форме - в панели или колонкой таблицы - и вводится пользователем; для пользовательских классов показывает код объекта.
    - `INTERVAL` - показывает интервал между значениями пары объектов одного и того же класса даты или времени.
  ...
  Если такое свойство (действие) добавлено без явного имени, его имя на форме образуется из самого оператора и переданных ему объектов, а для `NEW`, `EDIT`, `NEWEDIT` с указанным классом — вместе с ним: например `VALUE(o)`, `DELETE(o)`, `NEW(o)`, `NEW[A](o)`, `INTERVAL(o1, o2)`. По этому имени на него можно ссылаться там, где требуется имя свойства на форме, не задавая ему отдельного имени.
...
#### Опции свойства или действия {#options}

- `changeType`

    Указание [стандартных обработок](../paradigm/Form_events.md#predefined) событий изменений свойства или действия. Задается одним из ключевых слов:
    ...
    - `READONLY` - при попытке редактирования свойства, будет либо включен механизм фильтрации, либо событие просто проигнорируется. На событие `EDIT` это не влияет.
    - `DISABLE` - свойство недоступно для редактирования.
...
- `SHOWIF propertyExpression`
    ...
    Указание свойства, определяющего видимость на форме добавляемого свойства или действия. Если значение этого свойства будет равно `NULL`, то добавляемое свойство (действие) не будет отображаться. Для свойства (действия) с опцией `COLUMNS` видимость определяется для каждой колонки отдельно: выражение по объектам колонки скрывает отдельные колонки. Опция может сочетаться с опцией `HEADER`.
...
- `DISABLEIF propertyExpression`
    ...
    Указание свойства, определяющего недоступность добавляемого свойства (действия). Если значение этого свойства не равно `NULL`, то добавляемое свойство (действие) будет недоступно. По поведению аналогично опции `DISABLE`, только с дополнительным условием, зависящим от данных.
...
- `CLASS propertyExpression`
    ...
    Указание свойства, значение которого определяет CSS-классы (через пробел) элемента значения добавляемого свойства. Аналогично свойству `valueClass` в [инструкции `DESIGN`](DESIGN_statement.md): явное значение полностью замещает классы значения по умолчанию, а в таблице для типовой текстовой ячейки элементом значения является сама ячейка (`td`), поэтому классы попадают на неё (подробнее — в описании `valueClass`).
...
- `HEADER propertyExpression`
    ...
    Указание свойства, определяющего заголовок колонки добавляемого свойства (действия). В качестве заголовка будет взято возвращаемое значение этого свойства. Если возвращаемое значение равно `NULL`, заголовок будет пустым; само свойство (действие) и его колонки остаются видимыми — скрываются они опцией `SHOWIF`.
...
- `FOOTER propertyExpression`
    ...
    Указание свойства, определяющего колонтитул добавляемого свойства (действия). Используется только в [печатном представлении](../paradigm/Print_view.md) формы.
...
- `COLUMNS [groupid] (groupName1, ..., groupNameM)`
    ...
    Указание верхних [групп объектов](../paradigm/Form_structure.md#objects), значения которых будут формировать набор [колонок](../paradigm/Form_structure.md#groupcolumns) для отображения добавляемого свойства (действия). Чаще всего, вместе с опцией `COLUMNS` используется опция `HEADER`, чтобы задать заголовки этим колонкам. Отдельные колонки скрываются опцией `SHOWIF`, выражение которой использует объекты колонки.
...
- `viewType`
    ...
    - `TOOLBAR` - тулбар
    - `PANEL` - панель
    - `POPUP` - всплывающая панель
    
  Если не указана, используется соответствующая опция из [опций свойств](Property_options.md). Если и она не указана, используется [тип представления по умолчанию](../paradigm/Interactive_view.md#property) для группы отображения этого свойства или действия.

- `customView`

    Указание пользовательского представления значения добавляемого свойства. Может иметь один из следующих видов:

    - `CUSTOM renderFunction [CHANGE [editFunction]]`
    - `CUSTOM CHANGE [editFunction]`

        Указание JavaScript функций, отвечающих за отображение (`renderFunction`) и редактирование (`editFunction`) значения свойства. Каждая функция задается [строковым литералом](Literals.md#strliteral) с ее именем. Если после ключевого слова `CHANGE` функция не указана, используется механизм редактирования по умолчанию.

    - `SELECT selectType`

        Указание того, что значение свойства должно отображаться и редактироваться в виде выбора из списка возможных значений. Тип элемента выбора `selectType` задается строковым литералом (например `'dropdown'`, `'buttonGroup'`).

    - `SELECT [AUTO]`

        Указание того, что тип элемента выбора определяется автоматически. Ключевое слово `AUTO` необязательно, может быть использовано для устранения неоднозначности.

    - `NOSELECT`

        Ключевое слово, запрещающее автоматический выбор представления в виде списка возможных значений.

- `OPTIONS propertyExpression`

    Указание свойства, значение которого должно являться объектом класса JSON. Значение передается в настройки представления значения добавляемого свойства.
...
- `sessionScopeType`
    ...
    - `THISSESSION` - текущая сессия формы (значение по умолчанию). Имеет смысл использовать явно, чтобы отменить модификатор, заданный для всего блока.

- `APPLY`

    Модификатор, указывающий, что изменения добавляемого свойства (или добавляемого действия) применяются сразу после изменения (выполнения), а не остаются в текущей сессии формы до явного применения. Модификатор дополняет [стандартную обработку](../paradigm/Form_events.md#predefined) события изменения: после записи значения (выполнения действия) изменения применяются так же, как [оператором `APPLY`](APPLY_operator.md). На явно заданную обработку (`ON CHANGE`) модификатор не влияет: такая обработка при необходимости применяет изменения сама.

    Какие изменения применяются, зависит от области сессии:

    - без `NEWSESSION` / `NESTEDSESSION` — применяется текущая сессия формы целиком, то есть вместе со всеми остальными неприменёнными изменениями формы, а не только изменение этого свойства;
    - с `NEWSESSION` — изменение (действие) выполняется и применяется в новой сессии, поэтому фиксируются только изменения, сделанные в этой сессии, а неприменённые изменения сессии формы остаются неприменёнными;
    - с `NESTEDSESSION` — применение вложенной сессии лишь копирует её изменения в сессию формы; в базу данных они попадают при применении сессии формы.

    Если применение отменяется, например из-за нарушения [ограничения](../paradigm/Constraints.md), транзакция откатывается, а сообщения, выданные при применении, показываются пользователю, как и для оператора `APPLY`. Без области сессии изменение при этом остаётся в сессии формы как несохранённое (свойство сохраняет введённое значение), а с `NEWSESSION` — отбрасывается вместе с новой сессией. На асинхронное выполнение изменения на клиенте модификатор не влияет и может комбинироваться с `OPTIMISTICASYNC`.

- `OPTIMISTICASYNC`

    Модификатор для добавляемого действия. Аналогичен такой же опции в [инструкции `ACTION`](ACTION_plus_statement.md).

- `insertPosition`

    Указание позиции вставки свойства или действия в список свойств или действий на форме. Чаще всего используется в механизме [расширения форм](../paradigm/Form_extension.md). Может задаваться одним из следующих способов:

    - `AFTER formPropertyName`
    - `BEFORE formPropertyName`

        Свойство или действие будет добавлено в структуру формы непосредственно перед (ключевое слово `BEFORE`) или после (ключевое слово `AFTER`) указанного свойства или действия. Если позиция указана для всего блока, его свойства и действия вставляются в указанное место в порядке перечисления.
    ...
    - `FIRST`
        ...
        Ключевое слово, указывающее на то, что свойство или действие будет добавлено в начало списка.

    - `LAST`

        Ключевое слово, указывающее на то, что свойство или действие должно будет добавлено в конец списка. В отличие от добавления по умолчанию, свойства или действия, вставленные с помощью `LAST`, всегда будут располагаться после всех свойств или действий, добавленных в порядке объявления.
...
- `ORDER [USER | FIXED] [DESC]`
    ...
    Указание опции порядка для добавляемого свойства. Аналогично указанию свойства в [блоке сортировок](Filters_and_sortings_block.md#sort).
...
- `FILTER [USER | FIXED]`
    ...
    Указание опции фильтра для добавляемого свойства. Аналогично указанию свойства в [блоке фиксированных фильтров](Filters_and_sortings_block.md#fixedfilters).

    - `USER`

        Свойство добавляется как [пользовательский фильтр](../paradigm/Interactive_view.md#userfilters): строка его условия показывается сразу при открытии формы. Это поведение по умолчанию (в отличие от блока фиксированных фильтров, где по умолчанию фильтр фиксированный).
...
- `QUICKFILTER formPropertyName`
    ...
    Указание свойства, которое будет использоваться в [обработках по умолчанию](../paradigm/Form_events.md#default) для быстрой фильтрации.
...
- `PIVOT calcType`
    ...
    Указание агрегирующей функции для значений добавляемого свойства при группировке строк (в том числе в [виде представления *сводная таблица*](Pivot_block.md)).

    - `calcType`

        Задаётся одним из ключевых слов:
...
- `PIVOT LAST [DESC] (propertyExpression1, ..., propertyExpressionZ)`
    ...
    Указание того, что при группировке строк значение добавляемого свойства берётся из строки, последней в порядке значений перечисленных выражений.

    - `DESC`

        Ключевое слово, при указании которого используется обратный порядок.

    - `propertyExpression1, ..., propertyExpressionZ`

        Непустой список выражений, задающих порядок строк.

- `PIVOT FORMULA formulaStr (operand1, ..., operandM)`

    Указание того, что при группировке строк значение добавляемого свойства вычисляется по формуле из агрегированных значений самого свойства и перечисленных свойств-операндов.

    - `formulaStr`

        Строковый литерал с арифметическим выражением (синтаксис [math.js](https://mathjs.org/)), в котором `$1` - агрегированное значение добавляемого свойства, а `$2, ..., $(M+1)` - агрегированные значения свойств `operand1, ..., operandM` соответственно.

    - `operand1, ..., operandM`

        Список имён свойств / действий на форме, агрегированные значения которых доступны в формуле как `$2, ..., $(M+1)`.

- `COLUMN`, `ROW`, `MEASURE`

    Ключевые слова, задающие начальное размещение добавляемого свойства в [виде представления *сводная таблица*](Pivot_block.md): в колонках (`COLUMN`), рядах (`ROW`) или измерениях (`MEASURE`). Поэлементный аналог списков `COLUMNS`, `ROWS` и `MEASURES` блока сводных таблиц.

- `ON eventType actionId(param1, ..., paramZ) | { actionOperator }`

    Указание действия, которое будет выполнено при наступлении указанного [события формы](../paradigm/Form_events.md) .
    ...
    - `eventType`
        ...
        - `EDIT` - редактирование объекта, стоящего за свойством: его значения или объекта, для которого свойство вычисляется, — как выберет [обработка по умолчанию](../paradigm/Form_events.md#default) этого события. Пустая обработка (`ON EDIT { }`) оставляет пункт `Перейти по ссылке` в контекстном меню свойства, но он ничего не делает.
        - `KEYPRESS key` - нажатие пользователем клавиши `key`, задаваемой строковым литералом.
...
- `EVENTID eventId`
    ...
    Указание идентификатора события для добавляемого свойства (действия). Аналогично такой же опции в [опциях свойств](Property_options.md).

    - `eventId`

        [Строковый литерал](Literals.md#strliteral).

- `ATTR`

    Ключевое слово. Используется только в [иерархическом](../paradigm/Structured_view.md#hierarchy) представлении. Указание того, что:  
...
- `EXTID extID`
    ...
    Указание имени, которое будет использоваться для [экспорта / импорта](../paradigm/Structured_view.md#extid) этого свойства. Используется только в [структурированном](../paradigm/Structured_view.md) представлении.
...
- `NOEXTID`
    ...
    Ключевое слово, исключающее свойство из экспорта / импорта в структурированном представлении.

- `EXTNULL`

  Ключевое слово. Используется только в [иерархическом](../paradigm/Structured_view.md#hierarchy) представлении. Указание того, что при экспорте в XML / JSON NULL значение не будет проигнорировано и будет выгружено:
    - при экспорте в JSON как 'null'.
    - при экспорте в XML как пустое значение.

- `IN propertyGroup`

    Указание [группы свойств и действий](../paradigm/Groups_of_properties_and_actions.md), которой принадлежит свойство (действие) на форме. Если эта опция не задана, то группой свойства (действия) на форме будет группа самого свойства. 
...
- `imageSetting`
    ...
    Настройка иконки, которая будет отображаться для свойства (действия) на форме. Эта опция, позволяет настроить отображение иконки вручную. Может иметь один из следующих видов:

    - `IMAGE imageLiteral`

        [Указание иконки](../paradigm/Icons.md#manual), которая будет отображаться в качестве иконки свойства (действия) на форме.
    ...
    - `IMAGE [AUTO]`
        ...
        Указание того, что будет использован режим [автоматической установки](../paradigm/Icons.md#auto) иконки. Ключевое слово `AUTO` необязательно, может быть использовано для устранения неоднозначности.

    - `IMAGE propertyExpression`

        Указание свойства, значение которого определяет иконку. Позволяет задавать иконку динамически, в зависимости от данных.

    - `NOIMAGE`

        Ключевое слово, указывающее на то, что иконка свойства (действия) должна отсутствовать.

- `STICKY` | `NOSTICKY`

    Ключевые слова, управляющие закреплением свойства (действия) в таблице. Аналогично такой же опции в [опциях свойств](Property_options.md).

- `syncType`

    Определяет, выполняется ли действие синхронно (`WAIT`) или асинхронно (`NOWAIT`). Если опция не указана, используется соответствующая опция из [опций свойств](Property_options.md).

- `HINTNOUPDATE`

    Ключевое слово. Помечает, что закэшированные значения добавляемого свойства не должны обновляться при изменениях в сессии этой формы. Аналогично [блоку `HINTNOUPDATE`](FORM_statement.md#blocks) инструкции `FORM`.

- `HINTTABLE`

    Ключевое слово. Помечает, что изменения добавляемого свойства при чтении данных этой формой должны материализовываться во временную таблицу. Аналогично [блоку `HINTTABLE`](FORM_statement.md#blocks) инструкции `FORM`.
...
// форма, строки которой - значения объекта встроенного класса: штрихкоды товаров
FORM barcodes 'Штрихкоды'
    OBJECTS b = STRING[15]
    // добавляем значение строкового объекта колонкой, со своим именем и заголовком на форме
    PROPERTIES barcode 'Штрихкод' = VALUE(b) READONLY
    // оставляем только штрихкоды, заданные хотя бы у одного товара
    FILTERS [GROUP SUM 1 BY barcode(Sku s)](b)
;
```

## Блок расширения свойств и действий {#extendproperties}
...
Где `formPropertyName` — [имя свойства (действия) на форме](#name), задаваемое одним из следующих способов:
