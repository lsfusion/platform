...
- `PIVOT calcType`
    ...
    Указание агрегирующей функции, которая будет использоваться для значений добавляемого свойства при группировке строк (в том числе в [виде представления *сводная таблица*](Pivot_block.md)). `calcType` задается одним из ключевых слов `SUM`, `MAX`, `MIN`.

- `PIVOT LAST [DESC] (propertyExpression1, ..., propertyExpressionZ)`

    Указание того, что при группировке строк значение добавляемого свойства берется из строки, последней в порядке значений перечисленных выражений (`DESC` - в обратном порядке).
