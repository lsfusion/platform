...
- `CUSTOM customExpr`
    ...
    Элементы навигатора этого окна рисует компонент React или HTML-шаблон, а не стандартная панель. Мобильный веб-клиент и десктоп-клиент оставляют своё стандартное меню.
    ...
    - `customExpr`
        ...
        Шаблон здесь тот же, который принимает атрибут `custom` контейнера [`DESIGN`](DESIGN_statement.md), с теми же правилами записи места; здесь `<Lsf:name>` — место, куда попадает стандартная кнопка элемента навигатора с этим именем. В литерале имя разрешается в [пространствах имён](../paradigm/Naming.md#namespace) модуля, поэтому элемент должен быть уже объявлен, а имя, которое ни во что не разрешается, — ошибка; шаблон, вычисляемый свойством, не разрешается, и там пишется [каноническое имя](../paradigm/Naming.md#canonicalname).

        Окну `NATIVE` компонент или шаблон задать нельзя, так как элементов навигатора оно не содержит.
