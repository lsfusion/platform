...
Блок только задает эти настройки — вид представления группы объектов он не меняет. Чтобы свойства группы сразу отображались сводной таблицей, вид представления *сводная таблица* должен быть выбран [опцией группы объектов](Object_blocks.md#objects) `PIVOT`. Если этот вид представления не является начальным, заданные настройки, кроме `settingsType`, применяются при первом переключении группы в него пользователем.

Указание `objectGroupId pivotOptions` задает сразу все опции сводной таблицы группы объектов: опции, не перечисленные в нем, сбрасываются к значениям по умолчанию, в том числе заданные ранее в опции группы объектов `PIVOT`. Из нескольких таких указаний — в опции группы объектов и в блоках настроек — действует последнее объявленное.
...
- `MEASURES(groupObject)`
  ...
  Ключевое слово `MEASURES` с идентификатором группы объектов в круглых скобках обозначает специальную колонку сводной таблицы `(колонка)`, в которой размещаются её измерения. По умолчанию эта колонка располагается первой; указание `MEASURES(groupObject)` в списке `COLUMNS` или `ROWS` позволяет разместить её в произвольной позиции среди колонок или перенести в ряды.
...
- `configFunction`
    ...
    [Строковый литерал](Literals.md#strliteral) с именем клиентской JavaScript-функции, которая переопределяет опции отображения сводной таблицы. По умолчанию функция не применяется.
