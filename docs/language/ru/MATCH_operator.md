...
Оператор `MATCH` создает [свойство](../paradigm/Properties.md), реализующее [полнотекстовый поиск](../paradigm/Comparison_operators_=_etc.md). Для сравнения строки с шаблоном используется отдельный [оператор `LIKE`](LIKE_operator.md).
...
Оператор `MATCH` сравнивает `searchExpr` с поисковой строкой или подготовленным поисковым запросом, в соответствии с правилом результата [операторов сравнения](../paradigm/Comparison_operators_=_etc.md) (`TRUE` или `NULL`, но никогда `FALSE`). Символы `%` и `_` в `compareExpr` не являются масками.
...
- Значения типов `TSVECTOR` и `TSQUERY` обычно получают свойствами `toTsVector[STRING]` и `toTsQuery[STRING]`.
