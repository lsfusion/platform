...
Оператор `LIKE` создает [свойство](../paradigm/Properties.md), реализующее [сравнение строки с шаблоном](../paradigm/Comparison_operators_=_etc.md). Для полнотекстового поиска используется отдельный [оператор `MATCH`](MATCH_operator.md).
...
Оператор `LIKE` создает свойство, которое возвращает `TRUE`, если указанная строка соответствует указанному шаблону, в соответствии с правилом результата [операторов сравнения](../paradigm/Comparison_operators_=_etc.md) (`TRUE` или `NULL`, но никогда `FALSE`). Шаблон может включать следующие специальные символы:
...
- `searchExpr`
    ...
    [Выражение](Expression.md), значение которого определяет сравниваемую строку.

    Значение выражения должно принадлежать одному из [строковых классов](../paradigm/Built-in_classes.md).
