---
slug: "/IMPORT_operator"
title: 'Оператор IMPORT'
---

Оператор `IMPORT` - создание [действия](../paradigm/Actions.md), импортирующего данные из заданного файла в [заданные свойства (параметры)](../paradigm/Data_import_IMPORT.md) или, в общем случае, в [заданную форму](../paradigm/In_a_structured_view_EXPORT_IMPORT.md#importForm).
...
- `rootExpr`
    ...
    [Выражение](Expression.md) строкового класса, задающее имя элемента, считаемого корнем при импорте. В исходном файле выполняется рекурсивный поиск первого элемента (для **JSON** — ключа) с таким именем; от найденного элемента (для **JSON** — значения найденного ключа) начинается обработка вложенных записей. Для **JSON**, если это значение — массив, он обрабатывается так же, как [массив на корневом уровне](../paradigm/Structured_view.md#value). Если элемент с заданным именем не найден, импорт завершается с ошибкой. Применяется только для импорта **JSON** и **XML**.
...
- `objClassId1, ..., objClassIdK`
    ...
    Классы [импортируемых](../paradigm/Data_import_IMPORT.md) объектов. Задаются [идентификаторами класса](IDs.md#classid). `K <= 1`. По умолчанию считается, что импортируется один объект класса `INTEGER`.
...
- `whereId`
    ...
    Идентификатор свойства, в которое будет записано [значение по умолчанию](../paradigm/Built-in_classes.md#defaultvalue) класса значения этого свойства для каждого импортируемого объекта. Параметры свойства и его классы должны соответствовать импортируемым объектам и их классам. Если это свойство не задано и количество импортируемых объектов больше нуля, по умолчанию осуществляется поиск свойства с именем `imported` и классами импортируемых объектов (например `System.imported[INTEGER]`). Это умолчание действует только для опции `TO`: при `FIELDS` свойство `System.imported[INTEGER]` не заполняется, а при импорте формы признак того, что объект прочитан из файла, записывают только [фильтры формы](../paradigm/In_a_structured_view_EXPORT_IMPORT.md#importForm), поэтому оно заполняется, только если на форме есть фильтр с ним.
