...
Значения классов даты и времени записываются в JSON строками: по умолчанию в форматах ISO 8601 (например, `2026-08-17`, `2026-08-17T12:34:56`; значения `ZDATETIME` — момент времени в UTC с суффиксом `Z`). Это поведение определяется [рабочим параметром](../paradigm/Working_parameters.md) `useISOTimeFormatsInIntegration`: если он сброшен, используются форматы текущей локали.
...
- `orderExpr1, ..., orderExprL`
    ...
    Список [выражений](Expression.md), по которым производится сортировка формируемых данных. Выражения произвольны: они не обязаны входить в список `propertyExpr1, ..., propertyExprN`. Выражение сортировки добавляется во внутренний запрос скрытой колонкой и в результирующий JSON не попадает.
