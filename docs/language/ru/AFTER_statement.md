...
Аспект получает те же значения параметров, что и основное действие, и выполняется после его тела, в том числе когда тело завершилось [оператором `RETURN`](RETURN_operator.md). Если вызов отменен аспектом [инструкции `BEFORE`](BEFORE_statement.md) или прерван ошибкой, аспект не выполняется. Выход из самого аспекта завершает только его. Несколько аспектов одного действия выполняются в порядке объявления.
