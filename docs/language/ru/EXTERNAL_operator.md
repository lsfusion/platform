---
slug: "/EXTERNAL_operator"
title: 'Оператор EXTERNAL'
---

Оператор `EXTERNAL` - создание [действия](../paradigm/Actions.md), выполняющего [обращение к внешней системе](../paradigm/Access_to_an_external_system_EXTERNAL.md). 
...
- `HTTP`, `TCP`, `UDP`, `SQL`, `LSF`, `DBF`
    ...
    Ключевые слова. Задают тип внешнего вызова; семантика каждого описана в статье [Обращение к внешней системе](../paradigm/Access_to_an_external_system_EXTERNAL.md).
...
- `NOENCODE`
    ...
    Ключевое слово. Отключает предварительное URL-кодирование самого текста строки подключения и строки `BODYURL` (значения параметров, подставляемых через `$N`, всё равно URL-кодируются отдельно). Без `NOENCODE` этот текст URL-кодируется перед отправкой.

- `lsfExecType`

    Ключевое слово. Определяет [способ задания](../paradigm/Access_from_an_external_system.md#actiontype) действия:
