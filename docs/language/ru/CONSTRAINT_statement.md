...
Также с помощью опции `CHECKED` можно использовать созданное ограничение при вызове диалогов изменения свойств, изменение значений которых может нарушить ограничение. В этом случае в диалоге будет устанавливаться дополнительный фильтр, таким образом, чтобы при изменении значения свойства не нарушить созданное ограничение. Фильтр вычисляется из условия в том виде, как оно записано (до автоматического заключения в `SET`): для каждого предлагаемого значения условие вычисляется так, как если бы изменяемое свойство уже было установлено в это значение для редактируемого объекта, поверх изменений, уже сделанных в сессии формы, и значения, при которых условие становится не `NULL`, скрываются. Остальные параметры условия связываются с редактируемым объектом; [оператор `PREV`](PREV_operator.md) внутри него читает значение до изменений сессии, а [оператор изменения](../paradigm/Change_operators_SET_CHANGED_etc.md) сравнивает предлагаемое значение с этим предыдущим значением - поэтому при таком условии хранимое значение свойства остаётся предложенным, а для объекта, у которого предыдущее значение `NULL`, условие на это предыдущее значение ничего не скрывает. Если необходимо ограничить набор свойств, для которых будет выполняться указанная выше фильтрация, то список свойств может быть указан после ключевого слова `BY`. Фильтр действует только в этих диалогах изменения; механизмы ввода, предлагающие значения иным способом, его не используют — там нарушающее значение отклоняется только при самой проверке ограничения.
...
CLASS Task;
CLASS Status;
allowed = DATA BOOLEAN (Status, Status); // из какого статуса в какой разрешён переход
status = DATA Status (Task);
// только разрешённые переходы из хранимого статуса: диалог выбора статуса предлагает хранимый статус
// и разрешённые из него статусы, а для задачи без хранимого статуса - все статусы
CONSTRAINT SETCHANGED(status(Task t)) AND PREV(status(t)) AND NOT allowed(PREV(status(t)), status(t))
    CHECKED BY status[Task]
    MESSAGE 'Переход в этот статус не разрешён';
```
