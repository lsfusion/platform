---
slug: "/lt-_WHEN_statement"
title: 'Инструкция <- WHEN'
---

Инструкция `<- WHEN` - создание [вычисляемого события](../paradigm/Calculated_events.md).
...
Инструкция `<- WHEN` создает вычисляемое событие для [свойства](../paradigm/Data_properties_DATA.md), указанного в левой части инструкции. Этот оператор может объявлять свои локальные параметры при задании свойства, значение которого будет [изменяться](../paradigm/Property_change_CHANGE.md). Затем эти параметры могут быть использованы в выражениях условия и значения, на которое будет изменяться свойство.

Для свойства можно задать только одно вычисляемое событие. Условие и значение не могут зависеть от изменяемого свойства: такая зависимость образует цикл, который при старте сервера приводит к ошибке `Property ... is recursive` (см. [вычисляемые события](../paradigm/Calculated_events.md)).
