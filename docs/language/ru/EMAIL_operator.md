...
Оператор `EMAIL` - создание [действия](../paradigm/Actions.md), [отправляющего почту](../paradigm/Send_mail_EMAIL.md).
...
- `recipientType1 ... recipientTypeN`
    ...
    - `CC` - вторичный получатель письма, которому направляется копия
...
- `attachNamePropertyID`
    ...
    Идентификатор свойства, которое определяет список имен вложений. Свойство должно иметь ровно один параметр класса `INTEGER` и возвращать значение строкового класса.
