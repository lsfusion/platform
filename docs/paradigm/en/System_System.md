...
- **Import**: `importFile[]` — the file a form is imported from by default. `imported[INTEGER]` and `importedString[STRING[10]]` — the mark that the corresponding "row" was read from the file: `imported[INTEGER]` is the default condition of [data import](Data_import_IMPORT.md), and [form import](In_a_structured_view_EXPORT_IMPORT.md#importForm) fills it only when it is used in a filter of the form. `inputList[INTEGER]` / `displayInputList[INTEGER]` — the buffer of list values and captions. `readFile[]`, `readDialogPath[]`, `showResult[]` — the results of file reading, path selection, and message display.
