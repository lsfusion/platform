...
To create a property implementing a general form of selection, the [`CASE`](../language/CASE_operator.md) operator is used. The polymorphic form of the selection operator is implemented using the [`MULTI`](../language/MULTI_operator.md), [`OVERRIDE`](../language/OVERRIDE_operator.md) and [`EXCLUSIVE`](../language/EXCLUSIVE_operator.md), operators; the single form using the [`IF`](../language/IF_operator.md) and [`IF ... THEN`](../language/IF_..._THEN_operator.md) operator (the only operator that allows the specification of an alternative result).
