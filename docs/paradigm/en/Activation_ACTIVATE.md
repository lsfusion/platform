...
Activation of these kinds only switches focus / selection in the user interface and does not modify data or open forms: if the target form is not yet opened for the user, it has to be [opened first](In_an_interactive_view_SHOW_DIALOG.md). Activating a form in a [window that draws one form at a time](Navigator_design.md#systemwindows) does displace the form drawn there, and that window then closes it the way it closes any displaced form.
...
To create the action, use the [`ACTIVATE` operator](../language/ACTIVATE_operator.md).
