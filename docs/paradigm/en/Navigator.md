...
-   *form* – [opens](In_an_interactive_view_SHOW_DIALOG.md) a specified [form](Forms.md) in the interactive view and the [asynchronous](In_an_interactive_view_SHOW_DIALOG.md#flow) mode. Each selection of the element opens a new instance of the form — one more tab, even if this form is already open. If duplicate forms are forbidden for the role or the user (the `forbidDuplicateForms[UserRole]` / `dataForbidDuplicateForms[CustomUser]` [setting](System_Security.md#formSettings)), then, when the same form is already open in the window the element opens it in, without a label (matched by the [canonical name](Naming.md#canonicalname) of the form, regardless of the objects it was opened with), the existing tab is activated instead of opening a new one; selecting the element with the `Ctrl` key held opens a new instance even then. A form [opened](In_an_interactive_view_SHOW_DIALOG.md) from an action behaves this way only if its opening is set to activate an instance that is already open.
...
To manage the navigator use the [`NAVIGATOR` statement](../language/NAVIGATOR_statement.md).
