...
-   *form* – [opens](In_an_interactive_view_SHOW_DIALOG.md) a specified [form](Forms.md) in the interactive view and the [asynchronous](In_an_interactive_view_SHOW_DIALOG.md#flow) mode. Each selection of the element opens a new instance of the form — one more tab, even if this form is already open. If duplicate forms are forbidden for the role or the user (the `forbidDuplicateForms[UserRole]` / `dataForbidDuplicateForms[CustomUser]` [setting](System_Security.md#formSettings)), then, when a tab of the same form is already open (matched by the [canonical name](Naming.md#canonicalname) of the form, regardless of how and with which objects it was opened), the existing tab is activated instead of opening a new one; selecting the element with the `Ctrl` key held opens a new instance even then. The restriction applies only to opening from the navigator: a form [opened](In_an_interactive_view_SHOW_DIALOG.md) from an action is always opened as a new instance.
