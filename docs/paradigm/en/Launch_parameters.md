...
|Custom (starting with `D`)|`-Dlsfusion.server.lightstart`|`boolean`|"Light" start mode (usually used during development). In this mode, the server does not perform most metadata synchronization operations or create [security policy](Security_policy.md) settings forms, etc., and the startup time and the amount of memory consumed at startup are therefore reduced. Since that synchronization is mostly skipped, metadata elements it maintains — in particular forms, navigator elements, properties and actions added after the last full start — are missing from the [reflection](System_Reflection.md) tables, and the start itself gives no warning about it: for example, looking up an action object by its canonical name (`Reflection.actionCanonicalName[STRING]`) returns `NULL` for a new action until a full start. From the logic the mode is detected by the `Service.isLightStart[]` property (see [Service](System_Service.md)).<br/>In the [IDE](IDE.md) it is set with a checkmark in [lsFusion server configuration](IDE.md#configuration) (enabled by default).|`false`|
