...
To create an action that executes another action in a new session, use the [`NEWSESSION` operator](../language/NEWSESSION_operator.md) (for nested sessions, use the [`NESTEDSESSION` operator](../language/NESTEDSESSION_operator.md)). Opening a new session on a separate SQL connection is enabled by the `NEWSQL` option of the [`NEWSESSION` operator](../language/NEWSESSION_operator.md).
