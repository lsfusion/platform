...
-   [`logics.topModule`](Launch_parameters.md#project) - the name of the top module, or a comma-separated list of top modules. If this parameter is specified (not empty), not all LSF files will be loaded, but only the specified module(s) and all their [dependencies](Modules.md#depends) (the union, if several are listed). By default, this parameter is considered not specified (empty). Combined with [`dryRun`](Working_parameters.md), this lets a handful of modules be validated without loading the whole project.
