...
:::info
Current implementation: in the `MESSAGE` mode, the form is always shown as a window (the "asynchronous window" option is supported in this case); in the asynchronous `PREVIEW` mode the form is always shown as a tab; in the synchronous `PREVIEW` mode - as a window. The `NOPREVIEW` mode is always asynchronous and the form location in it does not make sense (since no forms are shown to the user directly)
:::

### Language

To open the form in the print view, the [`PRINT` operator](../language/PRINT_operator.md) is used.
