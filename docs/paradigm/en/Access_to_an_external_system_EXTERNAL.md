---
slug: "/Access_to_an_external_system_EXTERNAL"
...
To declare an action that accesses an external system, use the [`EXTERNAL` operator](../language/EXTERNAL_operator.md). To reuse an external connection across several calls within one block, use the [`NEWCONNECTION` operator](../language/NEWCONNECTION_operator.md).
