---
slug: "/In_a_structured_view_EXPORT_IMPORT"
...
During import, filters change to the [default values](Built-in_classes.md#defaultvalue) of value classes of these filters: for each object read from the file, this value is written to every filter applied to its object group. Thus, a filter in which a property of the object is compared with an object of an upper group writes the upper object to this property, and a filter consisting of a single property of the object (for example, `System.imported[INTEGER]`) writes to this property the mark that the object was read from the file. Import writes no data other than the properties and filters of the form, so only an object group that such a filter is applied to has this mark.

When importing data into objects of numeric classes, 0-based numbering is used. In case of [hierarchical](Structured_view.md#hierarchy) formats, numbering is "end-to-end" (that is, when the object group being imported is encountered for the second and subsequent times, object numbering in it starts from the position that the previous one stopped at). Numbering is kept separately for each object group, so in a property used in the filters of several object groups the marks of these groups get mixed.
...
To open the form in the structured view, use the [`EXPORT` operator](../language/EXPORT_operator.md). To import a form, use the [`IMPORT` operator](../language/IMPORT_operator.md).
