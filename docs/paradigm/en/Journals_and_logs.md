...
|`explain`|`explainThreshold`|`100` ms|With `EXPLAIN ANALYZE` mode on for a user (`Service.explainAnalyzeMode[User]`), the plan of that user's statement is written when the planning plus execution time by that plan reaches the threshold; a read query is rerun under analysis for this, and only if its original execution also reached the threshold, while a data-modifying statement is executed under analysis right away. `0` turns the time filtering off. Only SQL statements that are actually executed reach the log, so an assignment to a property does not reach it by itself: the statement that writes the property's table is not executed at the assignment - the [change](Change_sessions.md) accumulates in the session, and the statement, with its plan, is executed when the changes are [applied](Apply_changes_APPLY.md), where the lsFusion stack of its record points at the apply (the saving of the property's changes), not at the assignment operator. The assignment itself executes - and can write into the log - only the queries needed to compute the changed rows and their values, and the queries to the session temporary table, to which the accumulated changes of the property move from memory as soon as more than one row is changed|
