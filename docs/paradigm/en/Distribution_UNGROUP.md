...
Since the simple form of the operator is semantically very similar to the operator [partition/sort](Partitioning_sorting_PARTITION_..._ORDER.md), to declare a property that implements a simple distribution the [`PARTITION` operator](../language/PARTITION_operator.md) is also used.

For the extended form, use the [`UNGROUP` operator](../language/UNGROUP_operator.md).
