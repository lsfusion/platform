...
You can also define an import *condition*: this is a property in which the [default value](Built-in_classes.md#defaultvalue) of the property value class is written for each row (as opposed to import destination in which column values are written). If the condition is not defined when importing into properties, the system property `imported` with the classes of the imported objects (for example, `System.imported[INTEGER]`) is used as the condition.
