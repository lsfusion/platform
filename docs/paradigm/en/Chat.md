...
The web client uses an extended JSON renderer for the message feed and the input field: links and text formatting are expanded automatically, a file is attached via base64 encoding. The desktop client works with the same data through the standard interactive-view facilities.
