...
If such a composition through a link can be changed, and its outer part — the view of the options, for example the caption of the linked object — is unique among the matching objects, the web client shows the value as a *selection element*: the list of all the objects the choice dialog would offer, displayed by that view; the user picks one of them, and the picked object is written into the link. A stored property whose own name reads, in camelCase, as `name`, `id`, `number` or `caption` is presumed unique outright, without consulting any statistics — this covers a typical composition like the caption example above. For any other property, whether the options are unique is trusted only above `minInterfaceStatForValueUnique` matching objects (`100` by default): at that count or below, the platform doesn't trust its own row-count statistics enough to answer either way, and no selection element is built. Above that count, the kind of element is determined by the [working parameters](Working_parameters.md): if the total length of the views of all options does not exceed `maxLengthForValueButton` (`40` by default), and in a table column `maxLengthForValueButtonGrid` (`10` by default), a button group is drawn — adjacent toggle buttons with the picked option highlighted; otherwise, outside a table column, with at most `maxInterfaceStatForValueList` (`3` by default) options — a list, a column of radio buttons (in a horizontal container, the same button group); otherwise, with at most `maxInterfaceStatForValueDropdown` (`20` by default) options — a dropdown, a standard drop-down list (a `select` element) that shows the picked option and opens the others on click, without text entry; with more options no selection element is built, and ordinary editing is used. If the property can be `NULL`, the element also allows clearing the value: clicking the picked option again (in a dropdown, an empty option) writes `NULL`. For a property shown read-only, no selection element is built automatically. The kind of element can also be set explicitly or disabled in the [form design](Form_design.md).

What these thresholds are compared against is not the actual number of options but its estimate from the statistics of the condition that selects the options, and the total length estimated as that number multiplied by the average length of a value of the view's class; if the estimated number of options exceeds the dropdown limit, no element is built. The actual number and length are read instead of the estimate only if the view of the options does not depend on the form's current objects and the condition either does not depend on them either, or the estimated number of objects it selects is at least the estimated number of objects in the whole class (then the whole class is read — except for an explicitly set input without an initial value, where the estimate remains in that case). The estimate is rough and can differ noticeably from the actual number of matching objects, so the element may be built even with many actual options (all of them are then shown in it), and not built with few. A selection element is also built for a property whose change action is set explicitly as a [value input](Primitive_input_INPUT.md) of a custom-class object with a [set of offered values](Primitive_input_INPUT.md#list): the options are the objects of that set, also narrowed by the fixed filters of the class's selection form and, for an input that changes the property, by default by the [constraints](Constraints.md); the view is the displayed value given for the set — its uniqueness is not checked here; the kind of element is determined by the same thresholds. If the input has no initial value, the picked option is determined by the value of the shown property itself, so the element is built only when the class of that value is a built-in one compatible with the class of the displayed values. When no element is built, the object is entered in the editor of the displayed value's class with [suggestions](#async) from the set, or picked in the selection form. Disabling the selection element restores the ordinary execution of the change action — for such an input, entry in the field with suggestions; for the selection form to open right away, the change action itself has to open it.
...
Typing into the list of values declared for a property, into a filter's value field, or into a "type to search" completion field, runs a query against the database on every keystroke rather than filtering an already-loaded list; a family of [working parameters](Working_parameters.md) tunes how that query is built and how many rows it may read.
...
While a custom-class object has no current value — the *undefined* default objects type is set, or no object collection matches the active filters (in particular, there are no objects of the class yet) — the [views](#property) of the properties and actions that take this object as input are not shown on the form, wherever they are placed. This applies to views that use the object's current value: if the object's values come from the table rows or from a [group-in-columns](Form_structure.md#groupcolumns), the absence of a current value neither hides the view by itself nor removes the matching rows. Properties and actions that do not take this object — for example, the create-object action — remain. Properties with a display condition and the [selector](Form_events.md#predefined) property, through which the user picks the object, are not hidden on this ground either; but by itself this does not make ordinary changes of the missing object's properties possible — the entered value is not saved. So, for all the fields of an object card to be available from the very opening, the object is created before the card is opened rather than the card being built on an object without a current value; if the object is created by the [create action](#objectoperators) on the already open form, its fields become available after the creation.
