...
The apply operation itself can be canceled by the [cancel changes](Cancel_changes_CANCEL.md) operator running during the apply transaction — for example, inside the applied action or inside an event handler. The result of the apply is reflected in the `System.canceled[]` property: `TRUE` if the apply was canceled, `NULL` otherwise. After a successful apply the session is cleared — its accumulated changes are discarded since they are now in the database; after a canceled apply the session is preserved. After the operation completes (whether successfully or not), all messages issued during the apply — including those produced by the applied action and by event handlers — are written to the special property `System.applyMessage[]`. They are written in addition to being shown: the user sees the messages as usual — for example, the [message](Constraints.md#message) about a violated constraint is shown as a dialog — and `System.applyMessage[]` only keeps their text.

Before the transaction starts, the platform checks the accumulated changes against the classes of the objects they refer to, and drops the rows whose objects have since been deleted or have changed class in the database; on a canceled apply it checks them again. The [working parameter](Working_parameters.md) `disableCheckDataClasses` (`false` by default) turns this check off - a diagnostic fallback, since without it an apply can carry changes for objects that no longer exist.

Every apply also creates an object of the `System.Session` class and writes the session properties the [journals](Journals_and_logs.md) read from it, `System.currentSession[]` among them. The working parameter `createSessionObjects` (`true` by default) is what does that; with it off those writes do not happen, and with them goes the session part of the journals - `System.currentSession[]` then stays `NULL`.
...
To declare an action that applies changes, use the [`APPLY` operator](../language/APPLY_operator.md).
