...
The requested value can also belong to a custom class. In that case the operator requests an *object* of that class, which the user selects from the class's [selection form](Interactive_view.md#edtClass), instead of entering a plain value.
...
The values offered to the user can be limited to a specific set of candidate values, and that set can be narrowed further by a condition.

This operator can only be used in property [change event](Form_events.md#property) handlers on a form.

### Language

The syntax of the primitive input operator is described by the [`INPUT` operator](../language/INPUT_operator.md).
