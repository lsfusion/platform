...
Поскольку `customizeForm` всегда правит код текущего пользователя, до чужой настройки добираются с формы метаданных `forms`: `hasExtendCodeAll[Form]` показывает, настроена ли выбранная форма для всех пользователей, `countExtendCode[Form]` считает пользователей, настроивших её для себя, а вкладка настройки содержит оба фрагмента кода и позволяет их удалить — для одного пользователя или для формы целиком.
