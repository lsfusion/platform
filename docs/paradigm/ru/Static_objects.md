...
Каждому статическому объекту пользовательского класса необходимо задать определенное имя и заголовок, а также можно задать иконку, к которым впоследствии можно обращаться при помощи [свойств](Properties.md) `name[StaticObject]`, `caption[StaticObject]` и `image[StaticObject]` соответственно. 
