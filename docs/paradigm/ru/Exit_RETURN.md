...
Оператор выхода может также задавать [результат](Actions.md) окружающего [вызова действия](Call_EXEC.md). Указанное значение становится результатом этого вызова: если вызывающий код захватывает результат, то захваченное значение приходит из этого оператора; если вызов используется как значение, то именно это значение дает вызов. Если оператор использован без значения, результат окружающего вызова — `NULL`.
