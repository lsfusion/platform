...
| `sumWeekFrom[DATE, INTEGER]` / `sumWeekTo[DATE, INTEGER]` | сдвигают `DATE` на `n` полных недель, **без привязки к началу или концу недели**: `sumWeekFrom` прибавляет `n*7` дней (`sum[DATE, INTEGER]` на `n*7`), `sumWeekTo` прибавляет `n*7 + 6` дней. Оба сохраняют исходный день недели — `sumWeekFrom(d, 0) = d`, `sumWeekTo(d, 0) = d + 6` — и всегда различаются ровно на 6 дней, поэтому ни одно не возвращает понедельник или воскресенье недели. Чтобы получить само начало или конец недели, см. рецепты ниже |
...
`Month` и `DOW` — встроенные [классы](User_classes.md#static), описывающие месяцы (12 объектов) и дни недели (7 объектов). Содержат следующие свойства:

| Свойство        | Что возвращает                                                                                              |
|-----------------|------------------------------------------------------------------------------------------------------------|
| `number[Month]` | номер месяца 1–12 (январь = 1 … декабрь = 12)                                                              |
| `name[Month]`   | название месяца как текст                                                                                  |
| `number[DOW]`   | номер дня недели 0–6 в стиле PostgreSQL, где воскресенье = 0 (воскресенье = 0, понедельник = 1, … суббота = 6) — та же нумерация, что у `extractDOWNumber[DATE]` |
| `numberM[DOW]`  | номер дня недели 0–6 с отсчётом от понедельника (понедельник = 0, вторник = 1, … воскресенье = 6)           |
| `name[DOW]`     | название дня недели как текст                                                                              |

Поиск по номеру идёт в обратную сторону: `month[INTEGER]`, `DOW[INTEGER]`. Формы `months` и `DOWs` показывают эти классы как справочники.
...
Самые частые в расписаниях вычисления дат — это композиции свойств выше. Счёт дней везде целый; `d` — любая `DATE`.

```lsf
// сегодня плюс N дней (N может быть отрицательным)
sum(currentDate(), N)

// понедельник — начало недели, содержащей d
sum(d, -numberM(extractDOW(d)))

// воскресенье — конец этой недели
sum(d, 6 - numberM(extractDOW(d)))

// день недели d: как объект DOW, как текст, как номер 0–6 от понедельника
extractDOW(d)
extractDOWName(d)
numberM(extractDOW(d))

// первый и последний день месяца d
firstDayOfMonth(d)
lastDayOfMonth(d)
```
