...
Также, при импорте можно задать *условие* импорта - свойство, в которое для каждой строки будет записано [значение по умолчанию](Built-in_classes.md#defaultvalue) класса значения этого свойства (в отличие от назначений импорта, в которые записываются значения колонок). Если при импорте в свойства условие не задано, условием считается системное свойство `imported` с классами импортируемых объектов (например, `System.imported[INTEGER]`).
...
[Рабочий параметр](Working_parameters.md) `minSizeForExcelStreamingReader` (`15000000` байт) определяет, как читается файл `XLSX`: файл не меньше этого размера читается построчно, потоком, а не загружается в память целиком; отрицательное значение отключает потоковое чтение при любом размере. Именно потоковое чтение позволяет прочитать большой файл, но формулы оно не вычисляет: ячейка с формулой читается как результат, который сохранило в файле создавшее его приложение, и пуста, если там ничего не сохранено. Старый двоичный формат `XLS` всегда читается целиком, независимо от параметра.

### Язык

Для объявления действия, импортирующего данные, используется [оператор `IMPORT`](../language/IMPORT_operator.md).
