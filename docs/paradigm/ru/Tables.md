...
| Политика именования                | Имя таблицы                   |
| ---------------------------------- | ----------------------------- |
...
Если для свойства система не может определить таблицу, в которую должно быть помещено это свойство, то автоматически создается таблица с именем из префикса `_auto` и идентификаторов классов свойства, отсортированных по алфавиту: `_auto_<идентификатор класса>_..._<идентификатор класса>`. Например, для свойства с аргументами классов `DATE`, `Item.Item`, `Country.Country`, `INTEGER` будет создана таблица `_auto_Country_Country_DATE_INTEGER_Item_Item`. Впрочем, ситуаций когда используется таблица по умолчанию рекомендуется не допускать, а задавать таблицы в которых будут храниться свойства явно.
...
По умолчанию, для каждой таблицы строится уникальный [индекс](Indexes.md) (первичный ключ) по всем ее ключевым полям `key0`, `key1`, ..., `keyN`, где `N` - количество ключевых полей в таблице минус `1`. Также автоматически добавляются индексы по суффиксам ключевых полей `keyK`, ..., `keyN`, для всех `K` от `1` до `N`.
...
Для создания таблиц необходимо использовать [инструкцию `TABLE`](../language/TABLE_statement.md). Для определения таблицы, в которую необходимо положить свойство, используется [опцию `TABLE`](../language/Property_options.md) в опциях свойств.
