...
-   *Плоские* (DBF, CSV, XLS, XLSX, TABLE) - по одному файлу-таблице для каждой группы объектов, при этом для каждой группы объектов с вложенностью больше единицы в ее таблице должна присутствовать колонка с именем `parent`, содержащая номер "верхней" строки в таблице группы-родителя.
...
-   human-readable (текстовые) и бинарные. Все иерархические форматы - human-readable, плоские могут быть как бинарными (DBF, TABLE, XLS, XLSX), так и human-readable (CSV). Для human-readable форматов можно / нужно задавать кодировку (по умолчанию UTF-8).
...
При построении иерархии групп объектов в структурированном представлении группы объектов, для которых при открытии формы были [переданы](Open_form.md#params) все объекты, игнорируются (как если бы этих групп объектов не было).
...
:::info
В отличие от имен свойств на форме, имена экспорта / импорта (`EXTID`) свойств могут совпадать, если они находятся в разных узлах иерархии (то есть разных группах объектов / свойств). То же самое касается имен экспорта / импорта групп объектов и групп свойств.
:::

У `EXTID` могут быть заданы специализированные постфиксы:
-   ':escapeInnerJSON'
    Данный постфикс отключает [встраивание JSON-значений](#innerjson): значение свойства экспортируется как строка, даже если само является JSON.
    Например:
    ```lsf
...
-   Строится иерархия свойств / групп объектов в соответствии с [иерархией](Static_view.md) групп объектов и группами отображения свойств: группа отображения свойства считается родителем этого свойства, иерархия групп объектов сохраняется.
...
:::info
В иерархическом представлении группы объектов, так же как и свойства, можно включать в группы свойств.
:::


:::info
Описанный алгоритм во многом аналогичен алгоритму построения контейнеров свойств в [дизайне по умолчанию](Form_design.md#defaultDesign) (с той лишь разницей, что там нет / не используется иерархия групп объектов). Так же как и в механизме построения контейнеров, одна и та же группа свойств может включаться в иерархию несколько раз для различных групп объектов.
:::
...
```json
JSON результат ::= 
    { JSON со свойствами, группами объектов / свойств без родителей }

JSON со свойствами, группами свойств / объектов ::= 
...
```xml
XML результат ::= 
    <имя формы> XML со свойствами, группами свойств / объектов без родителей </имя формы>

XML со свойствами, группами свойств / объектов ::= 
...
При экспорте в JSON строковое значение свойства, которое само является JSON (начинается и кончается на квадратную или фигурную скобку), например значение класса `JSON` или `JSONTEXT`, встраивается в результат как вложенный объект или массив, а не как строка. Если разобрать такое значение как JSON не удается, оно экспортируется как обычная строка. Разбор при этом нестрогий: токены без кавычек принимаются, а текст после первого разобранного значения отбрасывается, поэтому строковое значение, лишь похожее на JSON (например, `'[AA] [BB]'`), встраивается в измененном виде (`["AA"]`). Отключить встраивание для отдельного свойства можно постфиксом ':escapeInnerJSON' в [имени экспорта](#extid). Для строковых значений, которые могут начинаться со скобки, его следует отключать явно. При импорте из JSON, наоборот, если свойству в импортируемом документе соответствует вложенный объект или массив, в свойство записывается его текстовое представление.
...
При импорте файл сопоставляется с этой иерархией по именам: значение каждого свойства, группы свойств или группы объектов читается из элемента под его [именем экспорта / импорта](#extid). Элементы файла, не соответствующие ни одному имени иерархии, игнорируются. Обратная ситуация также не приводит ни к ошибке, ни к предупреждению: группа объектов или группа свойств, которой не соответствует ни один элемент, не импортирует ничего (все ее поддерево пропускается), а свойство, собственный элемент которого отсутствует в импортируемой записи — при присутствующем родительском элементе — импортируется как `NULL`. В частности, при импорте из JSON записи группы объектов читаются только из массива под ключом, равным имени экспорта / импорта этой группы. Массив на корневом уровне преобразуется в `{ "value" : [ ... ] }` (см. [предопределенное значение](#value)) и поэтому читается только группой объектов с именем экспорта / импорта `value`.

Пустая строка в файле импортируется в строковое свойство по-разному в зависимости от формата: из JSON она записывается как пустая, но не `NULL`, строка — то есть как значение, отличное и от отсутствующего поля, и от явного `null`, которые оба импортируются как `NULL`, — а из CSV и XML — как `NULL`.
...
### Ключ группы объектов {#extkey}

По умолчанию наборы объектов группы объектов определяются в файле своей позицией: в иерархическом представлении - порядком записей, в [плоском](#flat) - порядком рядов в таблице. Значения самих объектов при этом в файл не попадают, а при импорте платформа определяет их сама: для объекта [конкретного пользовательского класса](User_classes.md) создается новый объект, для объекта числового класса используется порядковый номер набора объектов, начиная с нуля, причем для вложенной группы объектов нумерация сквозная - она не начинается заново для каждой записи родительской группы. Соответственно, группа объектов, импортируемая таким образом, должна состоять ровно из одного объекта конкретного пользовательского или числового класса.

Если для группы объектов задана соответствующая опция (`EXTKEY`), ее наборы объектов определяются в файле не позицией, а значениями своих объектов: при экспорте эти значения записываются в файл, при импорте читаются из него. В иерархическом представлении такая группа объектов представляется не списком, а парами ключ-значение, где ключом является значение объекта:

*JSON*:
```json
JSON группы объектов ::=
    "имя группы объектов" : {
        "значение объекта 1" : { JSON с дочерними свойствами, группами свойств / объектов 1 },
        "значение объекта 2" : { JSON с дочерними свойствами, группами свойств / объектов 2 },
        ...
        "значение объекта N" : { JSON с дочерними свойствами, группами свойств / объектов N },
    }
```
...
XML группы объектов ::=
    <имя группы объектов>
        <значение объекта 1> XML с дочерними свойствами, группами свойств / объектов 1 </значение объекта 1>
        <значение объекта 2> XML с дочерними свойствами, группами свойств / объектов 2 </значение объекта 2>
        ...
        <значение объекта N> XML с дочерними свойствами, группами свойств / объектов N </значение объекта N>
    </имя группы объектов>
```

В иерархическом представлении группа объектов с ключом должна состоять ровно из одного объекта [встроенного класса](Built-in_classes.md), значение которого и записывается как ключ (в XML - как имя тега, поэтому оно должно быть допустимым именем тега). В [плоском представлении](#flat) ограничений на состав группы объектов нет, а значения объектов записываются в отдельные колонки таблицы.

При импорте наборы объектов с одинаковым ключом считаются одним набором объектов, и значения свойств для него берутся из последней такой записи в файле.
...
импортируемый файл будет выглядеть следующим образом:
...
Если в теге необходимо объявить пространство имен, но при этом сам тег не должен ему принадлежать, в экспорт необходимо добавить свойство с пометкой `ATTR` и именем `xmlns:namespace`. При этом предполагается, что в значении такого свойства будет uri объявляемого пространства имен.
...
-   Колонками - свойства, группы отображения которых равны этой группе объектов.

Если для группы объектов задан [ключ](#extkey), то к этим колонкам добавляются колонки со значениями объектов этой группы объектов - по одной на каждый объект, перед колонками свойств (то есть после колонки `parent`, если она есть). Именуются они, как и остальные элементы файла, по [имени экспорта / импорта](#extid) объекта.

В формате CSV (при отсутствии первой строки-заголовка) колонки именуются по аналогии с XLS (то есть `A` - первая, `B` - вторая и т.п.). Сколько таких имен порождается, задает [рабочий параметр](Working_parameters.md) `maxColumnsInPlainImportExport` (`256`, то есть до `IV`), а значит, и сколько колонок вообще можно адресовать: колонки, которые идут в файле дальше, имен не получают и просто не читаются, а импорт или экспорт, которому нужно больше свойств, чем есть имен, завершается ошибкой с именем первого свойства, которому колонки не хватило. Имена строятся один раз при старте сервера, поэтому изменение параметра требует перезапуска.
