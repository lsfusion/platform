...
-   [Ввод значения (`INPUT`)](Primitive_input_INPUT.md)
...
### Выполнение ввода {#execution}

Действие ввода значения выполняется в рамках обработки события на открытой форме. При вызове действия оно приостанавливает текущий поток и обращается к [интерактивному представлению](Interactive_view.md) этой формы — пользовательской сессии, в которой было инициировано событие, — за получением значения от пользователя. Платформа показывает пользователю элемент ввода, соответствующий запрашиваемому значению (например, текстовое поле для строкового значения, выбор даты для даты или диалог выбора файла для файла); пользователь вводит или выбирает значение либо отменяет ввод. Результат всегда является типизированным значением запрашиваемого класса — платформа проверяет введённые пользователем данные как значение этого класса и никогда не выполняет их как код — и далее этот типизированный результат используется для вызова основного или альтернативного действия.
...
Ввод значения часто подразумевает наличие некоторого *начального* (предыдущего) значения, с которого начинается ввод (то есть текущему значению присваивается начальное) и которое пользователь должен / может впоследствии изменить. Как и большинство остальных значений, начальное значение задается как некоторое свойство.
...
:::info
В текущей реализации платформы начальные значения и автоматическое изменение поддерживаются только для оператора ввода значения (`INPUT`) и диалогового открытия формы (`DIALOG`).
:::
