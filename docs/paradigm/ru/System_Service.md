...
Через это API проходят два [рабочих параметра](Working_parameters.md). `sessionConfigTimeout` (по умолчанию `0`, то есть без изменений) входит в настройки, которые возвращает `getServerSettings[]`, но применяется только после того, как клиент аутентифицируется и для него создаётся навигатор: если он положителен, тогда он переопределяет время простоя (в секундах), после которого контейнер сервлетов по умолчанию завершает HTTP-сессию веб-клиента; это HTTP-сессия, не имеющая отношения к тому, сколько остаётся открытой сессия приложения lsFusion. `hideDesktopClientLink` (по умолчанию `true`) определяет, станет ли сервер вообще генерировать JNLP-ссылки запуска для десктоп-клиента на Java Web Start; пока он включён, `getServerSettings[]` не сообщает таких ссылок, и страница входа не предлагает ссылку на загрузку десктоп-клиента.
