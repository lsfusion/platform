...
Настройка планировщика осуществляется на форме `Администрирование > Планировщик > Задания`. Планировщик запускается автоматически вместе с сервером приложений; на той же форме его можно остановить (`Остановить планировщик`) и запустить снова (`Запустить планировщик`). Если в настройках системы указан сервер, то планировщика можно будет запустить только с этого сервера (рис. 1.).
...
Также могут быть заданы и необязательные параметры:
