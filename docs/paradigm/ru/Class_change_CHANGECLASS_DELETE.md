...
Для объявления действия, реализующего изменение классов объектов, используется [оператор `CHANGECLASS`](../language/CHANGECLASS_operator.md) или [оператор `DELETE`](../language/DELETE_operator.md).
