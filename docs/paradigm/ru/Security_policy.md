...
`Редактирование объекта` - может ли пользователь вызвать [событие редактирования объекта](Form_events.md#property) этого свойства; при запрете пункт `Перейти по ссылке` скрыт из контекстного меню свойства, и событие вызвать нельзя.
...
### Создание пользователей и ролей из кода

Пользователи, роли и назначения ролей — обычные объекты и свойства системных модулей [`Authentication`](System_Authentication.md) и [`Security`](System_Security.md), поэтому всё, что настраивается на описанных выше формах, можно задать и в коде приложения — например, при заполнении начальных или демонстрационных данных. Такую инициализацию обычно подключают к обработчику [события запуска](Launch_events.md), например `onStarted[]`:
