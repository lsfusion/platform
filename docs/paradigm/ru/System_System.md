...
| `forceUpdate[STRING]`                             | применить накопленное обновление группы объектов (по её имени на форме) в режиме ручного обновления |
...
Модуль фиксирует основные [окна](Form_views.md): `logo`, `root`, `system` (верхняя горизонтальная панель), `toolbar` (левая вертикальная), `forms` (центральная область), `log` (правая полоса для уведомлений). CSS-классы этих окон — абстрактные свойства `logoWindowClass[]`, `rootWindowClass[]`, `systemWindowClass[]`, `formsWindowClass[]`, `toolbarWindowClass[]`, `logsWindowClass[]`; их реализует модуль [SystemEvents](System_SystemEvents.md) по настройкам оформления (класс фона темы, например `bg-dark-subtle`, плюс вспомогательные классы панели навигации). Они объявлены в [форме выбора по значению](Property_extension.md), допускающей несколько реализаций, поэтому проект заменяет классы окна, добавляя собственную реализацию: реализация модуля, который требует `SystemEvents` напрямую или через требуемые им модули, добавляется позже и потому проверяется первой (без такой зависимости порядок двух модулей не гарантирован); она действует, когда возвращает значение, иначе остаются классы модуля. Значения не склеиваются: реализация, сохраняющая классы темы, повторяет их. Вид системных окон настраивается также через эти настройки и стили проекта (регистрируются через `onWebClientInit[STRING]`); окно со своими классами проект объявляет сам и размещает в нём элементы навигатора.
...
- [Инструкция `+=`](../language/plus_equals_statement.md) — добавляет реализацию проекта для оконного свойства-класса.
- [Инструкция `WINDOW`](../language/WINDOW_statement.md) — объявляет собственное окно проекта с его CSS-классом.
