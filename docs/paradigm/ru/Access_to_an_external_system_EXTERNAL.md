...
### HTTP - выполнение http-запроса веб-сервера {#http}
...
Таймаут вызова и строгость проверки SSL читаются из свойств `System.timeoutHttp[]` (в миллисекундах, по умолчанию 30 минут, если значение не задано) и `System.insecureSSL[]` (при истинном значении отключает проверку TLS-сертификата).


:::info
При выполнении на клиенте десктоп-клиент полностью выполняет вызов локально, а обычный веб-клиент (браузер) делегирует вызов в `XMLHttpRequest` и поддерживает только то, что предоставляет сам этот API: cookies управляются собственным cookie jar браузера, а таймауты и проверка TLS - самим браузером.
:::
...
Расширение файла при этом определяется автоматически по аналогии с [оператором `WRITE`](Write_file_WRITE.md).
...
Собственные [заголовки](https://en.wikipedia.org/wiki/List_of_HTTP_header_fields) и [cookies](https://en.wikipedia.org/wiki/HTTP_cookie) запроса можно передать вместе с вызовом. Для исходящих заголовков объявляйте отдельное локальное свойство, а не системное свойство `System.headers[TEXT]`: в действии, вызванном [из внешней системы](Access_from_an_external_system.md#request), оно уже заполнено заголовками входящего запроса, и все они, включая служебные (например `Content-Length`), попадут в исходящий запрос — как правило, с ошибкой выполнения в результате. Если всё же используется `System.headers[TEXT]`, очищайте его перед вызовом.
...
:::info
При выполнении на клиенте прямая работа с сокетами доступна локально в десктоп-клиенте и через Flutter-мост во Flutter-клиенте (веб/мобильном); обычный веб-клиент (браузер) прямого доступа к сокетам не имеет и падает с `UnsupportedOperationException`.
:::
...
## Сохранение подключений между вызовами

При серии обращений к одной и той же конечной точке открытое подключение можно [переиспользовать](New_connection_NEWCONNECTION.md) между вызовами вместо открытия нового на каждый. Это снижает накладные расходы на установление подключения и позволяет передавать между вызовами состояние, которое подключение хранит у себя — временные таблицы и переменные SQL-сессии, открытый TCP-сокет и буфер чтения, текущее положение в DBF-файле и т.п.

## Язык

Для объявления действия, обращающегося к внешней системе, используется [оператор `EXTERNAL`](../language/EXTERNAL_operator.md). Для переиспользования внешнего подключения между несколькими вызовами в одном блоке используется [оператор `NEWCONNECTION`](../language/NEWCONNECTION_operator.md).
