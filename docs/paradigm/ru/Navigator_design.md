...
-   `System.forms` - окно, в котором будут открываться пользовательские формы; оно рисует их закладками. Приложение может объявить свои окна, держащие формы: форма открывается в такое окно, когда его называет её [открытие](In_an_interactive_view_SHOW_DIALOG.md#location), и так форма рисуется шапкой или боковой панелью приложения. Такое окно рисует одну форму за раз, если оно не объявлено рисующим свои формы закладками и не рисуется React-компонентом, который размещает их так, как решает сам.
-   `System.log` - окно, в которое будут выводиться все сообщения пользователю. Если это окно будет невидимо, его сообщения выводятся в лог и не показываются нигде.
...
Для управления окнами необходимо использовать [инструкцию `WINDOW`](../language/WINDOW_statement.md).
