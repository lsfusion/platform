...
Некоторые клиенты **не** откатываются с корневого URL по RFC 8414 на URL внутри контекста при `404`. Это проверено на коннекторе `claude.ai` (2026-05): обнаружение прекращается, а пользователь видит общую ошибку «не удалось подключиться к серверу».
