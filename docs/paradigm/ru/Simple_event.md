...
По сравнению с обычным событием, простое событие лишь выполняет свою обработку для каждого набора объектов, удовлетворяющего условию события. Обработка при этом подчиняется правилам [цикла](Loop_FOR.md): множество таких наборов объектов фиксируется до начала обработки, а действие выполняется по одному разу для каждого набора последовательно, и каждое выполнение видит изменения, сделанные предыдущими. Порядок перебора наборов по умолчанию произвольный и может быть задан явно, как в цикле. Однако при этом простые события имеют ряд важных дополнительных возможностей:
...
Условие простого события проверяется и на объектах, [удаляемых](Class_change_CHANGECLASS_DELETE.md) в сессии: при удалении объекта его первичные свойства сбрасываются в `NULL`, поэтому условие, учитывающее переход значения свойства объекта в `NULL` (в частности, обёрнутое в оператор изменений), выполняется для каждого удаляемого объекта, у которого прежнее значение свойства было не `NULL`, и обработка выполняется для уже удалённого объекта. Если обработка не должна выполняться при удалении, в условие добавляется проверка принадлежности объекта классу: для удалённого объекта она даёт `NULL`, и событие не наступает. Смена класса, выводящая объект из прежнего класса, действует так же, как удаление.
