...
Запрашиваемое значение может относиться и к пользовательскому классу. В этом случае оператор запрашивает *объект* этого класса, который пользователь выбирает из [формы выбора](Interactive_view.md#edtClass) класса, а не вводит значение напрямую.
...
Значения, предлагаемые пользователю для выбора, можно ограничить заданным набором, а этот набор дополнительно сузить условием.
