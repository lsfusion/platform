...
Оператор *пользовательской формулы* позволяет создавать [свойство](Properties.md), вычисляющее некоторую формулу на языке SQL. При этом можно задать одну реализацию, используемую для любого SQL-сервера, или различные реализации для разных SQL серверов.
...
Пользовательские формулы укладываются в стандартное для платформы `NULL`-распространение: по умолчанию при `NULL`-аргументе формула не вычисляется и свойство сразу возвращает `NULL`, а на не-`NULL` аргументах формула обязана вернуть не-`NULL`-результат.
...
Для объявления свойства с помощью пользовательской формулы используется [оператор `FORMULA`](../language/FORMULA_operator.md).
