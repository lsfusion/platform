...
|Пользовательские (начинаются на `D`)|`-Dlsfusion.server.lightstart`|`boolean`|Режим "облегченного" запуска (как правило используется при разработке). В этом режиме сервер не выполняет большинство операций синхронизации метаданных, создания форм настройки [политики безопасности](Security_policy.md) и т.п., соответственно уменьшается время запуска и объем потребляемой памяти при запуске. Так как эта синхронизация по большей части пропускается, поддерживаемые ею элементы метаданных — в частности, формы, элементы навигатора, свойства и действия, добавленные после последнего полного запуска, — отсутствуют в таблицах [рефлексии](System_Reflection.md), а сам запуск проходит без предупреждений об этом: например, поиск объекта действия по каноническому имени (`Reflection.actionCanonicalName[STRING]`) для нового действия возвращает `NULL` до полного запуска. Из логики режим распознаётся свойством `Service.isLightStart[]` (см. [Service](System_Service.md)).<br/>В [IDE](IDE.md) регулируется галочкой в [lsFusion server конфигурации](IDE.md#configuration) (по умолчанию включена).|`false`|
