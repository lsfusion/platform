...
Возможность выполнения запроса также зависит от настройки [`enableAPI`](Working_parameters.md), которая позволяет по умолчанию отключить API, ограничить его авторизованными запросами или дополнительно разрешить анонимные запросы. Отдельное действие можно пометить [`@@noauth`](../language/Action_options.md), чтобы обойти и проверку аутентификации, и `enableAPI`, или [`@@api`](../language/Action_options.md), чтобы разрешить его при `enableAPI=0` (всё ещё требуя авторизованного пользователя). При `enableAPI=0` внешний API также разрешается авторизованному пользователю, у которого есть доступ к форме навигатора `System.interpreter`, поскольку такой пользователь уже может выполнять произвольный lsFusion-код через браузерный UI.
...
-   [`@@ui`](../language/Action_options.md) - всегда интерактивно.
-   [`@@noui`](../language/Action_options.md) - никогда не определяется как интерактивное, и действие выполняется синхронно (если только клиент явно не запросил идентификатор уведомления через заголовок `Need-Notification-Id` - он обрабатывается всегда).
