...
1.  Рекурсивно строит промежуточное свойство (result) с дополнительным первым параметром (номером операции) следующим образом:
    ...
    2.  `result(i+1, o1, o2, ..., oN) = step(o1, ..., oN, $o1, $o2, ..., $oN) IF result(i, $o1, $o2, ..., $oN)`, где `step` - свойство *шага*, а `$o1, ..., $oN` обозначают значения параметров на предыдущей итерации.
...
Агрегирующая функция выбирается автоматически по классу значений `initial`/`step`: если они принадлежат числовому классу, используется `SUM`, и класс результата — тот же числовой класс; в остальных случаях (как правило, при классе `BOOLEAN`) используется `OR`, и класс результата — тот же не-числовой класс.
...
1.  `CYCLES YES` - циклы допускаются. В этом случае при обнаружении цикла значение свойства будет равно максимально допустимому значению для класса значений этого свойства; для рекурсии со значениями класса `BOOLEAN` такого максимума нет, и маркер цикла не влияет на результат.
