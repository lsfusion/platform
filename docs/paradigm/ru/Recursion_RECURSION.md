...
1.  Рекурсивно строит промежуточное свойство (result) с дополнительным первым параметром (номером операции) следующим образом:
    ...
    2.  `result(i+1, o1, o2, ..., oN) = step(o1, ..., oN, $o1, $o2, ..., $oN) IF result(i, $o1, $o2, ..., $oN)`, где `step` - свойство *шага*, а `$o1, ..., $oN` обозначают значения параметров на предыдущей итерации (в исходном коде они записываются как `$name` — см. [оператор `RECURSION`](../language/RECURSION_operator.md)).
...
Агрегирующая функция выбирается автоматически по классу значений `initial`/`step`: если они принадлежат классу `BOOLEAN`, используется `OR`; в остальных случаях — `SUM`.
