...
Пока клиент ждёт ответа на синхронный запрос к серверу — применение изменений, выполнение действия и так далее — ничто не сообщает пользователю о выполняющемся запросе, пока не пройдёт примерно задержка, заданная рабочим параметром `busyDialogTimeout` (по умолчанию `1000` мс), после чего показывается индикатор ожидания; меньшее заданное значение соблюдается не точно — desktop-клиент никогда не ждёт меньше `1000` мс, а веб-клиент вычитает из него около `500` мс собственной задержки, но не меньше `500` мс. Веб-клиент всегда показывает модальный диалог; в desktop-клиенте `busyDialog` выбирает между тем же диалогом — с кнопкой отмены и периодически обновляемым сообщением о ходе выполнения (по умолчанию) — и, если отключён, обычным курсором ожидания без возможности отмены. В desktop-клиенте `maxRequestQueueSize` (по умолчанию `0`, то есть без ограничения) дополнительно ограничивает, сколько собственных асинхронных запросов клиента без ожидания ответа может быть поставлено в очередь одновременно; когда очередь достигает этого размера, дальнейшие запросы, вызванные пользователем, отправляются синхронно (и тем самым попадают под действие индикатора ожидания выше) вместо накопления в очереди, защищая соединение от переполнения при быстрых повторных действиях.

В веб-клиенте сообщение сервера в лог, пришедшее пока панель логов не в фокусе, по умолчанию сопровождается всплывающим уведомлением; `enableShowingRecentlyLogMessages` заставляет вместо этого саму панель ненадолго раскрыться. В desktop-клиенте по умолчанию новое сообщение никак не отмечается; тот же параметр заставляет его собственную панель логов точно так же ненадолго раскрыться.
