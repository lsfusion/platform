...
При отображении группы объектов в виде таблицы количество отображаемых рядов (наборов объектов) может как определяться автоматически на основе высоты видимой части таблицы, так и задаваться разработчиком явно при создании формы. В автоматическом случае размер первой подгружаемой порции рядов задается [рабочим параметром](Working_parameters.md) `pageSizeDefaultValue` (по умолчанию `50`); размер следующих порций клиент подбирает сам, под высоту таблицы.
...
В веб-клиенте ведущие колонки таблицы остаются закреплёнными (*sticky*) при её горизонтальной прокрутке, пока суммарная ширина колонок вплоть до данной включительно не превышает заданной рабочим параметром `maxStickyLeft` (по умолчанию `0.33`) доли от видимой ширины таблицы; первая колонка, правый край которой пересёк бы эту долю, и все колонки после неё, прокручиваются как обычно.
...
Когда фокус переходит в текстовое поле ввода на форме, платформа может сразу начать изменение этого свойства, не дожидаясь ввода. Разрешает это `suppressOnFocusChange` (по умолчанию `false`): при выключенном параметре изменение по одному только фокусу не начинается никогда, а при включенном - начинается, кроме случаев, когда фокус перевела сама платформа - щелчок мышью, прокрутка, переход стрелками, возврат фокуса, применение фильтра и тому подобное, - там оно подавляется всегда.
...
В desktop-клиенте `showNotDefinedStrings` (по умолчанию `false`) помечает по-настоящему отсутствующее (`NULL`) строковое значение явной пометкой вместо пустой ячейки: "Not defined", либо "Required", если свойство обязано быть заполнено. С пустой строкой всё наоборот: при выключенном параметре она помечается как "Empty string", чтобы её не спутали с пустой ячейкой; при включённом параметре она, наоборот, отображается как настоящая пустая ячейка.
...
В любой момент все действующие на группу объектов фильтры — постоянные, добавленные пользователем и фильтры представления — объединяются в одно закэшированное условие перед применением, чтобы изменение одного из них не заставляло платформу пересчитывать остальные заново; рабочий параметр `disableCombineFilters` отключает это и вычисляет / кэширует каждый фильтр независимо, как запасной вариант для совместимости. Результат в обоих случаях одинаковый.

### Список асинхронного ввода значений {#async}

Ввод текста в список значений свойства (объявленный оператором `INPUT ... LIST`), поле фильтра или поле поиска с автодополнением запускает запрос к базе данных на каждое нажатие клавиши, а не фильтрует уже загруженный список; семейство [рабочих параметров](Working_parameters.md) настраивает построение этого запроса и то, сколько строк он может прочитать.
...
При наведении на заголовок колонки таблицы в любом клиенте, либо на любой другой элемент интерфейса в веб-клиенте, всплывающая подсказка с подробной информацией показывается после времени наведения, заданного рабочим параметром `tooltipDelay` (по умолчанию `1500` мс; `0` отключает её). В десктоп-клиенте элемент, не являющийся заголовком колонки, всё равно ждёт фиксированные `1500` мс независимо от заданного значения — `tooltipDelay` там определяет только, появится ли подсказка вообще, а не то, сколько ждать. В веб-клиенте область просмотра по её площади в пикселях (ширина × высота) распределяется по одному из трёх размерных уровней, определяющих компактность / адаптивность интерфейса, а границы между уровнями задают два рабочих параметра: *tiny* при площади не больше `maxPixelTinySize` (`748800`, например 1280×585), *mini* при площади не больше `maxPixelMiniSize` (`1121280`, например 1536×730), и *normal* при большей площади.
