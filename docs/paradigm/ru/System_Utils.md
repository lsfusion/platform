...
Почти все эти свойства объявлены через `FORMULA` и на PostgreSQL транслируются в одноимённую SQL-функцию или оператор. В строках, где свойство напрямую соответствует выражению PostgreSQL, оно приведено в конце описания после `; PG` (`$1`, `$2`, … — параметры по порядку). Свойства, результат которых — строка, возвращают её в неограниченном строковом классе независимо от классов аргументов: у большинства это `TEXT`, у `substring[STRING, STRING]`, `splitPart[STRING, STRING, INTEGER]`, `regexpReplace[STRING, STRING, STRING, STRING]` и `array[STRING, STRING, INTEGER]` — `STRING` без ограничения длины.
