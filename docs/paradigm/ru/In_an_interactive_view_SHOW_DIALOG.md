...
Если режим не задан явно, платформа выбирает его автоматически: оператор работает синхронно, если открываемая форма показывается в модальном расположении, если форма, из которой она открывается, сама модальна, или если текущая сессия ещё может использоваться после этого действия; в противном случае - асинхронно.

### Расположение формы {#location}

Открываемая форма может быть показана одним из следующих способов:

-   Как *окно* (`FLOAT`) - форма показывается в виде плавающего окна.
-   Как *закладка* (`WINDOW`) - форма открывается в виде закладки в системном [окне](Navigator_design.md) `System.forms` или в другом окне `FORMS`, которое называет оператор, - объявленном приложением для формы, которую оно рисует отдельно от остальных, например шапки над областью форм.
-   Как *встроенная форма* (`EMBEDDED`) - форма показывается встроенной в форму, из которой она открывается.
-   Как *всплывающая форма* (`POPUP`) - форма показывается в плавающем всплывающем окне, привязанном к элементу, из которого она открывается.
-   В *контейнере* (`IN`) - форма встраивается в заданный контейнер формы, из которой она открывается.

По умолчанию, в синхронном режиме работы форма показывается как окно, а в асинхронном - как закладка.

Закладка, открытая в синхронном режиме, до своего закрытия блокирует вызвавшую форму. Если вызвавшая форма сама показана как окно, открываемая форма вместо закладки показывается как окно.

Каждое выполнение действия открывает новый экземпляр формы, даже если та же форма уже открыта, если только открытие не задано так, чтобы активировать уже открытый экземпляр: тогда при уже открытой в том же окне той же форме с той же меткой вместо новой [активируется](Activation_ACTIVATE.md) она. Такое открытие делает это либо всегда, либо только пока для пользователя запрещено повторное открытие форм; открытие формы из [навигатора](Navigator.md) делает второе.


:::info
В текущей реализации платформы плавающее окно, встроенная форма и всплывающая форма модальны; закладка и форма в контейнере - нет. Именно эту модальность платформа использует, когда выбирает режим автоматически (см. выше).
:::
...
Кроме того, при вызове формы можно указать, что при её закрытии через `System.formOk` сначала должны проверяться все [ограничения](Constraints.md), и при нарушении любого из них форма не закрывается.
...
Для открытия формы в интерактивном представлении используется [оператор `SHOW`](../language/SHOW_operator.md). Для показа формы в режиме диалога используется [оператор `DIALOG`](../language/DIALOG_operator.md).
