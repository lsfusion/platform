...
-   [logics.topModule](Launch_parameters.md#project) - имя верхнего модуля, или список верхних модулей через запятую. Если этот параметр задан (не пустой), будут подключаться не все найденные lsf-файлы, а только заданный модуль(и) и все их [зависимости](Modules.md#depends) (объединение, если указано несколько). По умолчанию этот параметр считается не заданным (пустым). В сочетании с [dryRun](Working_parameters.md) позволяет быстро проверить несколько модулей, не загружая весь проект.
