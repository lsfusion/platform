...
Для иконки, заданной путем к файлу, платформа дополнительно ищет по имени этого файла подходящую иконку шрифта — тем же механизмом, что и при [автоматической установке](#auto), и так же требуя подключенного модуля `Icon.lsf`. Если совпадение найдено, к элементу вместе с путем прикрепляются CSS-классы найденной иконки, и там, где используются иконки шрифта, показывается она, а не растровое изображение. Минимальный ранг такого совпадения задает [рабочий параметр](Working_parameters.md) `defaultImagePathRankingThreshold`. По умолчанию он равен `0.0`, то есть подходит любое найденное совпадение; чтобы указанный путь использовался как есть, порог нужно поднять.
...
|--------------------------------------------|---------------------------------------|--------------------|
| `defaultImagePathRankingThreshold`         | Иконки, [заданные путем к файлу](#manual) | `0.0`           |
