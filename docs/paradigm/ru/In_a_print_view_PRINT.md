...
Также как и в [интерактивном представлении](In_an_interactive_view_SHOW_DIALOG.md), в интерактивных режимах печатного представления поддерживается [управление потоком](In_an_interactive_view_SHOW_DIALOG.md#flow) — синхронное или асинхронное выполнение (с тем же поведением, что и в операторе открытия формы в интерактивном представлении). Итоговое расположение формы (окно или закладка) определяется автоматически режимом и типом выполнения, как описано ниже.
...
Синтаксис оператора открытия формы в печатном представлении описывается [оператором `PRINT`](../language/PRINT_operator.md).
