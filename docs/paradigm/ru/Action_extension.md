...
-   В последовательной форме все реализации выполняются в порядке этого списка внутри одного вызова абстрактного действия: [выход](Exit_RETURN.md) из реализации завершает этот вызов, и оставшиеся реализации пропускаются.
...
Так же как и для [оператора ветвления](Branching_CASE_IF_MULTI.md#poly), для абстрактного действия существует *полиморфная форма*, когда можно не задавать условие явно, а использовать в качестве условия принадлежность сигнатуре соответствующего этому условию действия.
