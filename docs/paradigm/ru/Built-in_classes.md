...
Класс `YEAR` отличается от `INTEGER` значением по умолчанию (текущий год) и интерактивным представлением, где его значение показывается без группировки разрядов (шаблон `####`), а в веб-клиенте вводится через выбор года.
...
|------------|-------------|
|`RAWFILE`   |Пустая строка (при явном преобразовании типа — dat)|
|`JSONFILE`  |json         |
|`XMLFILE`   |xml          |
|`CSVFILE`   |csv          |
|`TEXTFILE`  |txt          |
...
|`XML`       |xml          |
|`JSON`, `JSONTEXT`|json   |
|Строковые классы, `LINK`, ссылки на файлы конкретного типа|dat|
...
Встроенный класс записывается в коде как [идентификатор класса](../language/IDs.md#classid) - ключевое слово, обозначающее этот класс. Значения встроенных классов записываются в виде [литералов](../language/Literals.md), каждый из которых имеет свою форму и ограничения; значение `YEAR`, у которого литерала нет, записывается [оператором приведения типа](../language/Type_conversion_operator.md): `YEAR(2024)`.
