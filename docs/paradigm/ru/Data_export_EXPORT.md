...
На формат `DBF` влияют два [рабочих параметра](Working_parameters.md). `exportDBFNumericMandatoryZeroes` (по умолчанию `false`) записывает числовое поле со всеми знаками после запятой и ведущим нулем (`0.50`), а не опускает незначащие знаки (`.5`); действует только на поля, у которых знаки после запятой вообще есть. `exportDBFLanguageDriverName` (по умолчанию `false`) записывает кодовую страницу файла в байт языкового драйвера его заголовка, чтобы читающая сторона могла взять кодировку из самого файла; платформа знает только идентификатор `cp866`, поэтому для экспорта в любой другой кодировке, включая кодировку по умолчанию, параметр ничего не делает.

### Язык

Для объявления действия, экспортирующего данные, используется [оператор `EXPORT`](../language/EXPORT_operator.md).
