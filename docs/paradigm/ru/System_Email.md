...
Само соединение приёма настраивают два [рабочих параметра](Working_parameters.md). `mailReceiveTimeout` (по умолчанию `5000` мс) задаёт тайм-аут ожидания ответа почтового сервера: чтение, не уложившееся в это время, прерывает приём ошибкой. Значение `0` тайм-аут снимает совсем, а не ужесточает. Тайм-аут установления самого соединения этим параметром не задаётся, и на отправку он не распространяется. `ignoreBodyStructureSizeFix` (по умолчанию `false`) велит не доверять размеру части письма, который сообщает IMAP-сервер, и читать вложение до фактического конца данных; это обходной путь для серверов, сообщающих неверный размер, из-за чего вложения приходят обрезанными. У вложения с расширением `.dbf` при этом отсекается завершающий перевод строки, если он там оказался. Для учётных записей POP3 параметр ничего не меняет.
