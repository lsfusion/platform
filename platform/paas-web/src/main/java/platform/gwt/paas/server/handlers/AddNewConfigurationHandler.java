...
public class AddNewConfigurationHandler extends SecuredActionHandler<AddNewConfigurationAction, GetConfigurationsResult, PaasRemoteInterface> {

    public AddNewConfigurationHandler(PaasDispatchServlet servlet) {
        super(servlet);
    }

    @Override
    public GetConfigurationsResult executeEx(final AddNewConfigurationAction action, final ExecutionContext context) throws DispatchException, RemoteException {
        return new GetConfigurationsResult(
                servlet.getLogics().addNewConfiguration(
