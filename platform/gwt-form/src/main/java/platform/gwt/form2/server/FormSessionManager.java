...
import platform.gwt.form2.server.convert.ClientComponentToGwtConverter;
...
public class FormSessionManager {
    ...
    public GForm createFormAndPutInSession(RemoteFormInterface remoteForm) throws IOException {
        ...
        ClientComponentToGwtConverter converter = new ClientComponentToGwtConverter();
        GForm gForm = converter.convertOrCast(clientForm);

        gForm.sessionID = formSessionID;

        session.setAttribute(formSessionID, new FormSessionObject(clientForm, remoteForm, converter));
