CodeMirror.defineMode("fusion", function(config, parserConfig) {
    ...
    var keywords = words("ABSTRACT ACTION ADD ADDFORM ADDOBJ ADDSESSIONFORM AFTER " + 
                           "ALL AND APPLY AS ASC BEFORE BOTTOM BY CASCADE CENTER CHECKED " + 
                           "CLASS COLUMNS CONSTRAINT DATA DEFAULT DESC DESIGN DIALOG " + 
                           "DRAWROOT EDIT EDITABLE EDITFORM EDITSESSIONFORM EXCEPTLAST " + 
                           "EXCLUSIVE FALSE FILTER FILTERGROUP FILTERS FIXED FOOTER " + 
                           "FORCE FORM FORMULA FROM GRID GROUP HALIGN HEADER HIDE " + 
                           "HIDESCROLLBARS HIDETITLE HIGHLIGHTIF HINT HORIZONTAL IF " + 
                           "IMPORT IN INCREMENT INIT INTERSECT IS LEFT LIST MAX MIN " + 
                           "MODAL MODULE MSG NAVIGATOR NEW NEWSESSION NO NOT NOTHING " + 
                           "NULL OBJECT OBJECTS OBJVALUE ORDER OVERRIDE PANEL PARENT " + 
