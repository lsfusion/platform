CodeMirror.defineMode("fusion", function(config, parserConfig) {
    ...
    var keywords = words("ABSTRACT ACTION ADD ADDFORM ADDOBJ ADDSESSIONFORM AFTER " + 
                           ...
                           "AUTOSET BACKGROUND BCC BEFORE BOTTOM BREAK BY CANCEL " +
                           "CASCADE CASE CC CENTER CHANGE CHANGECLASS CHANGED CHANGEWYS CHECK CHECKED " +
                           "CLASS CLOSE COLOR COLUMNS CONCAT CONFIRM CONSTRAINT CONTAINER " +
                           "CUSTOM CUSTOMFILE CYCLES DATA DEFAULT DELETE DESC DESIGN " + 
                           "DIALOG DO DOCKING DOCX DRAWROOT DROP ECHO EDIT EDITABLE EDITFORM " +
                           "EDITKEY EDITSESSIONFORM ELSE EMAIL END EQUAL EVENTID EVENTS EXCELFILE " +
                           ...
                           "LIST LOADFILE LOCAL LOGGABLE MAX MAXCHARWIDTH MESSAGE " + 
                           "META MIN MINCHARWIDTH MODAL MODULE NAME NAMESPACE NAVIGATOR " + 
                           "NEW NEWSESSION NO NOT NOTHING NULL NUMERIC OBJECT OBJECTS " + 
                           "OBJVALUE OK ON OPENFILE OR ORDER OVERRIDE PAGESIZE PANEL PARENT " +
                           "PARTITION PDF PDFFILE PERSISTENT POSITION PREFCHARWIDTH " + 
                           "PREV PRINT PRIORITY PROPERTIES PROPERTY PROPORTION READONLY " + 
                           "READONLYIF RECURSION REGEXP REMOVE REPORTFILE REQUEST " + 
                           "REQUIRE RESOLVE RETURN RGB RIGHT RIGHTBOTTOM ROUND RTF " + 
