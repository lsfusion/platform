...
$BODY$
	...
	BEGIN
		LOOP
			SELECT CASE WHEN forward THEN array_agg(key1 ORDER BY key1 ASC) ELSE array_agg(key1 ORDER BY key1 DESC) END INTO daysOff FROM countrydate
				WHERE key0 = $1 AND isdayoffcd = 1 AND CASE WHEN forward THEN key1 > lastOff ELSE key1 < lastOff END LIMIT bufferSize;
