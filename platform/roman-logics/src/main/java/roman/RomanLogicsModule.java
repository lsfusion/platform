...
//        addConstraint(addJProp("Для товара должен быть задан артикул", baseLM.andNot1, is(sku), 1, articleSku, 1), false);
       ...
       /*addConstraint(addJProp("Выбранный должен быть среди связанных кодов", andNot1, addCProp(LogicalClass.instance, true, article), 1,
        ...
        typeFabricArticleSku = addJProp(idGroup, true, "typeFabricArticleSku", "Тип одежды (ИД)", typeFabricArticle, articleSku, 1);
...
//                routeToFillShipmentBarcode, 1, 2);
        ...
        quantityImporterFreightArticle = addSGProp(baseGroup, "quantityImporterFreightArticle", true, "Кол-во позиций", quantityImporterFreightSku, 1, 2, articleSku, 3);
...
//        priceFreightImporterFreightSku = addJProp(baseGroup, "priceFreightImporterFreightSku", "Цена за фрахт", divideDouble2, sumFreightImporterFreightSku, 1, 2, 3, quantityImporterFreightSku, 1, 2, 3);
        ...
        sumInOutImporterFreightArticle = addSGProp(baseGroup, "sumInOutImporterFreightArticle", true, "Сумма по артикулу", sumInOutImporterFreightSku, 1, 2, articleSku, 3);
        sumInOutImporterFreightBrandSupplier = addSGProp(baseGroup, "sumInOutImporterFreightBrandSupplier", true, "Сумма по бренду", sumInOutImporterFreightSku, 1, 2, brandSupplierArticleSku, 3);

        sumInOutFreightArticle = addSGProp(baseGroup, "sumInOutFreightArticle", true, "Сумма по артикулу", sumInOutImporterFreightSku, 2, articleSku, 3);
...
//            addActionsOnObjectChange(objBarcode, addPropertyObject(barcodeNotFoundMessage, objBarcode));
    ...
    private class FreightShipmentFormEntity extends FormEntity<RomanBusinessLogics> {
        ...
        private GroupObjectEntity gobjShipmentRoute;
...
//            addObjectActions(this, objFreight);
            ...
            gobjShipmentRoute = new GroupObjectEntity(genID());
            objShipment = new ObjectEntity(genID(), shipment, "Поставка");
            objRoute = new ObjectEntity(genID(), route, "Маршрут");
            gobjShipmentRoute.add(objShipment);
            gobjShipmentRoute.add(objRoute);
            addGroup(gobjShipmentRoute);

            addPropertyDraw(objShipment, baseLM.date, sidDocument, nameSupplierDocument);
            addPropertyDraw(objShipment, objRoute, nameImporterShipmentRoute);
            ...
            addFixedFilter(new CompareFilterEntity(addPropertyObject(routeFreight, objFreight), Compare.EQUALS, objRoute));
        ...
        public FormView createDefaultRichDesign() {
            ...
            design.get(gobjShipmentRoute).grid.constraints.fillVertical = 1;
            design.get(objPallet.groupTo).grid.constraints.fillVertical = 1;
            design.get(objSku.groupTo).grid.constraints.fillVertical = 2;

            design.addIntersection(design.getGroupObjectContainer(gobjShipmentRoute),
    ...
    private class InvoiceShipmentFormEntity extends FormEntity<RomanBusinessLogics> {
        ...
        private InvoiceShipmentFormEntity(NavigatorElement parent, String sID, String caption) {
            ...
            addFixedFilter(new OrFilterEntity(new NotNullFilterEntity(addPropertyObject(quantityDocumentSku, objInvoice, objSku)),
                    new CompareFilterEntity(addPropertyObject(quantityInvoiceSku, objInvoice, objSku), Compare.GREATER, addPropertyObject(baseLM.vzero))));
...
//            addHintsNoUpdate(NDSImporterFreightSku);
    ...
    private class FreightContentFormEntity extends FormEntity<RomanBusinessLogics> {
        ...
        private FreightContentFormEntity(NavigatorElement<RomanBusinessLogics> parent, String sID, String caption) {
            ...
            addPropertyDraw(objFreight, objBrand, quantityFreightBrandSupplier, sumInOutFreightBrandSupplier);
            addPropertyDraw(objImporter, objFreight, objBrand, quantityImporterFreightBrandSupplier, sumInOutImporterFreightBrandSupplier);
            ...
            addPropertyDraw(objFreight, objArticle, quantityFreightArticle, sumInOutFreightArticle);
            addPropertyDraw(objImporter, objFreight, objArticle, quantityImporterFreightArticle, sumInOutImporterFreightArticle);
