            ...
            if(QueryJoins.size()==1) { // "простой" запрос
                ...
                for(AndJoinQuery And : AndProps) {
                    ...
                    if(From.length()==0) {
                        ...
                        From = From + " FULL JOIN " + AndSelect + " ON " + (AndJoin.length()==0?Where.TRUE:AndJoin);
                        Second = false;
                    }
                }
...
class CompiledJoinQuery<K,V> { //class CompiledJoinQuery<K,V>{ //
