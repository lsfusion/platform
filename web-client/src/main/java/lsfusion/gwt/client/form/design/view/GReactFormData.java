...
// skipped the WHOLE group controller on it, taking the group's CHROME (its toolbar, and with it the target of
// the server-side FILTER / ORDER actions) down with the table and saying nothing. Now the two are asked
// separately: React gets a controller when any part of the group is projected, asked of the projection so the
// enumeration is not written twice; the platform builds the table when it still draws the rows, and the
// toolbar either way (GGridController.chromeOnly) - a FILTER / ORDER on rows it does not draw says so once.
...
public class GReactFormData {
    ...
    // rows too, or the membership/order of the rows. The levels are MONOTONE - ORDER implies ROWS implies ENTRIES -
    ...
    private boolean isProjectedContainer(GComponent component) {
        ...
        if (component.isLsfView()) {
            ...
            // for a renderer that is not there. It is keyed by the component SID, so it needs no integration name -
            // that is the MEMBER's business (GFormController.getControllerStructure), as the server's claim says too
            return !draw.isList && isPropertyShown(draw, GGroupObjectValue.EMPTY);
        }
    ...
    private void fillContainers(JavaScriptObject data, GContainer scope, GComponent component) {
        if (getProjectedContainerScope(component) == scope)
            setField(data, component.sID, buildDescriptorEntry(component));
    ...
    // to nothing at all on a missing node - so it is said once, here, where both halves are known. The relation is
    // static - a drawing component that is an lsf child of this scope (its descriptor lands here, and an lsf
    // component has no part) - so it is asked once per scope, at build time.
    private void reportPlacedNotDrawn(GContainer scope) {
        if (saidNoRowsHere.get(scope) != null)
            return;
        for (GGroupObject group : form.groupObjects) {
            GComponent draw = group.getDrawComponent();
            if (draw == null || getProjectedContainerScope(draw) != scope)
                continue;
            saidNoRowsHere.put(scope, Boolean.TRUE);
            GwtClientUtils.consoleError("'" + scope.sID + "' places '" + draw.sID + "' and the platform draws it, so"
                    + " data." + group.getSID() + " has no rows here (nor a controller for them); drop `lsf = TRUE` from '"
                    + draw.sID + "' if React should draw them instead");
            return;
        }
    }
    ...
    private JavaScriptObject buildDescriptorEntry(GComponent component) {
        ...
        for (GPropertyReader reader : component.getDescriptorReaders())
            if (isProjectedDescriptorAttribute(reader, component))
                emitAttribute(entry, reader, GGroupObjectValue.EMPTY, component);
        return entry;
    }
    ...
    public GContainer partScope(GComponent component) {
        ...
        // A PANEL draw is a component of its own, standing where the design put it, and its part goes there - even
        // when its group's rows are drawn by GWT in another container: that mixed group gets a react controller beside
        // its GWT one (GFormController.initializeControllers), so the value has somewhere to be routed, and the
        // projection carries the entry and nothing else of the group. GFormController.isReactOwned asks this too.
    ...
    // ... and a cell in each row, unless the platform draws it: an LSF list property has its column and no cell. Not
    // asked per row: a SHOWIF is keyed by the column group objects, and a projected list draw has none (the server
    // refuses a COLUMNS draw wherever React draws the rows), so the column's answer is every cell's answer
    private boolean hasCellEntry(GPropertyDraw draw) {
        return hasColumnEntry(draw) && !draw.isLsfView();
    }
    ...
    private JavaScriptObject buildColumnEntry(GPropertyDraw draw) {
        ...
        if (draw.isLsfView())
            entry = buildDescriptorEntry(draw);
        else {
            entry = newObject();
