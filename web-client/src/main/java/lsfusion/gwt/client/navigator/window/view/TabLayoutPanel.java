...
public class TabLayoutPanel extends ResizeComposite implements HasWidgets,
     ...
     * @param widget widget to be added to top bar
     */
    public TabLayoutPanel(double barHeight, Unit barUnit, Widget widget) {
        ...
        if (widget != null){
            panel.add(widget);
            panel.setWidgetHorizontalPosition(widget, Alignment.END);
            width = getWidthFromStyle(widget.getElement().getStyle().getWidth());
        }

        // Add the tab bar to the panel.
        panel.add(tabBar);
        panel.setWidgetLeftRight(tabBar, 0, Unit.PX, width, Unit.PX);
    ...
    private int getWidthFromStyle(String width){
        if (width.length()!=0){
            return Integer.parseInt(width.replaceAll("px", "")) + 2; // +2 for offset
        }
        return 0;
    }
