...
/*
* taken from https://groups.google.com/forum/#!msg/google-web-toolkit/BgfklltCv2A/ZclR11R0BgAJ
* implemented touch actions
* */
...
public class CustomSplitLayoutPanel extends DockLayoutPanel {
    ...
    class HSplitter extends Splitter {
        ...
        @Override
        protected int getEventPosition(Event event, boolean touch) {
            if (touch) return event.getTouches().get(0).getClientX();
            else return event.getClientX();
        }
    ...
    abstract class Splitter extends Widget {
        ...
        public Splitter(Widget target, boolean reverse) {
            ...
            sinkEvents(m_touch ? Event.TOUCHEVENTS : Event.MOUSEEVENTS);
         }
        ...
        public void onBrowserEvent(Event event) {
            ...
            switch (type) {
                ...
                case Event.ONTOUCHSTART:
                    if (type == Event.ONTOUCHSTART || event.getButton() == NativeEvent.BUTTON_LEFT) {
                        mouseDown = true;
                        /*
                         * Resize glassElem to take up the entire scrollable
                         * window area, which is the greater of the scroll size
                         * and the client size.
                         */
                        ...
                        boolean touch = type == Event.ONTOUCHSTART;
                        offset = getEventPosition(event, touch) - getAbsolutePosition();
                        if (offset < 0) offset = -(offset - splitterSize);
                    ...
                    if (this.toggleDisplayAllowed) {
                        ...
                        if (now - this.lastClick < DOUBLE_CLICK_TIMEOUT) {
                            ...
                            if (layout.size == 0) {
                                ...
                                /*
                                 * Collapse to size 0. We change the size
                                 * instead of hiding the widget because hiding
                                 * the widget can cause issues if the widget
                ...
                case Event.ONTOUCHMOVE:
                    if (mouseDown) {
                        boolean touch = type == Event.ONTOUCHMOVE;
                        int size;
                        if (reverse) {
                            size = getTargetPosition() + getTargetSize() - getEventPosition(event, touch) - offset;
                        } else {
                            size = getEventPosition(event, touch) - getTargetPosition() - offset;
                        }
        ...
        protected abstract int getEventPosition(Event event, boolean touch);
    ...
    class VSplitter extends Splitter {
        ...
        @Override
        protected int getEventPosition(Event event, boolean touch) {
            if (touch) return event.getTouches().get(0).getClientY();
            else return event.getClientY();
        }
    ...
    /**
     * Construct a new {@link CustomSplitLayoutPanel} with the default splitter size
     * of 8px.
     */
    ...
    /**
     * Construct a new {@link CustomSplitLayoutPanel} with the specified splitter size
     * in pixels.
     *
     ...
     *
     * <p>
     * Its associated splitter cannot be dragged to a position that would
     * make it smaller than this size. This method has no effect for the
     ...
     * speed-bump effect where the slider will stick to a preferred minimum
     * size before closing completely.
     *
     * <p>
