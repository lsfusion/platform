...
**Блоки file-focused:** суть блока — какие файлы дописывать (каждое имя = пара `docs/<type>/en/<имя>.md` + `docs/<type>/ru/<имя>.md`, type ∈ paradigm/language). Если конструкции блока ещё нет статьи — исполнитель создаёт оба файла в нужной type-папке, а оркестратор подключает их в `docs/sidebars.js` (вне блочного коммита).
...
Создаёт исполнитель блока (language-статья в `docs/language/{en,ru}/`), paradigm-аспект — в указанной существующей статье; оркестратор подключает новый файл в `docs/sidebars.js`.
