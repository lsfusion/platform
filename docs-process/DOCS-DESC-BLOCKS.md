...
**Блоки file-focused:** суть блока — какие файлы дописывать (каждое имя = пара `docs/en/<имя>.md` + `docs/ru/<имя>.md`). Если конструкции блока ещё нет статьи — исполнитель создаёт оба файла, а оркестратор добавляет их в `manifest.json` и `.rag/bootstrap/sidebars-snapshot.js` (вне блочного коммита).
Процесс — `DOCS-DESC-PLAN.md`; промпты — `DOCS-DESC-PROMPTS.md`; инструкция людям — `DOCS-DESC-EXECUTOR.md`. Граф из `manifest.json` (paradigm+language); how-to — позже. Отметки ведёт **только оркестратор**.
...
|------|:----:|-------|------|-----------|:----:|:---------:|--------|-------|
| T1 | 1 | B02 | Логическая модель | Классы | P+L | 6/4 | in-progress | Alex (оркестратор) |
| T1 | 1 | B05 | Логическая модель | Арифметика/сравнение/логика | P+L | 4/3 | todo | Alex (оркестратор) |
| T1 | 1 | B06 | Логическая модель | Строки/округление/экстремум/конверсия/структуры | P+L | 6/11 | todo | Alex (оркестратор) |
| T1 | 1 | B07 | Логическая модель | Композиция (JOIN) | P+L | 1/1 | todo | Alex (оркестратор) |
| T1 | 1 | B08 | Логическая модель | Выборка (CASE/IF/MULTI/OVERRIDE/EXCLUSIVE) | P+L | 1/6 | todo | Alex (оркестратор) |
| T1 | 1 | B09 | Логическая модель | Фильтрация и порядок | P | 2/0 | todo | Alex (оркестратор) |
| T1 | 2 | B03 | Логическая модель | Свойства и данные | P+L | 2/2 | todo | Alex (оркестратор) |
| T1 | 2 | B04 | Логическая модель | Операторы-выражения: основа | P+L | 2/4 | todo | Alex (оркестратор) |
| T1 | 3 | B01 | Логическая модель | Логическая модель — корень | P | 2/0 | todo | Alex (оркестратор) |
...
> Примечание: `MCP_server` (B45) есть в `manifest.json`, но отсутствует в `.rag/bootstrap/sidebars-snapshot.js` — оркестратору определить его место в дереве.
...
Создаёт исполнитель блока (language-статья), paradigm-аспект — в указанной существующей статье; оркестратор вписывает новый файл в `manifest.json` и `.rag/bootstrap/sidebars-snapshot.js`.
