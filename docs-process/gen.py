...
import os
BASE=os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # корень platform
MANIFEST=os.path.join(BASE,'docs/manifest.json')
SIDEBAR=os.path.join(BASE,'.rag/bootstrap/sidebars-snapshot.js')
import json, sys
m=json.load(open(MANIFEST))
PARA={k for k,v in m.items() if v.get('sourceType')=='paradigm'}
LANG={k for k,v in m.items() if v.get('sourceType')=='language'}
B=[]
def b(*a): B.append(a)
...
# sidebar depth (для порядка внутри слоя — не используется в выводе, но оставим)
txt=open(SIDEBAR).read()
depth=0; pst=[]; node_depth={}; pend=None
for ln in txt.splitlines():
    s=ln.strip()
    mh=re.search(r"id:\s*'([^']+)'", s)
    if "link:" in s and mh: node_depth[mh.group(1)]=depth; pend=mh.group(1)
    else:
        bm=re.fullmatch(r"'([^']+)',?", s)
        if bm: node_depth[bm.group(1)]=depth
    op=s.count('['); cl=s.count(']')
    if 'items:' in s and op>cl: depth+=1; pst.append((depth,pend)); pend=None; op-=1
    net=op-cl
    while net<0:
        depth-=1
        if pst and pst[-1][0]>depth: pst.pop()
        net+=1
...
PROMPTDIR=os.path.join(BASE,"prompts")   # plain-text промпт на блок (для вложений)
...
o.append("**Блоки file-focused:** суть блока — какие файлы дописывать (каждое имя = пара `docs/en/<имя>.md` + `docs/ru/<имя>.md`). Если конструкции блока ещё нет статьи — исполнитель создаёт оба файла, а оркестратор добавляет их в `manifest.json` и `.rag/bootstrap/sidebars-snapshot.js` (вне блочного коммита).")
o.append("Процесс — `DOCS-DESC-PLAN.md`; промпты — `DOCS-DESC-PROMPTS.md`; инструкция людям — `DOCS-DESC-EXECUTOR.md`. Граф из `manifest.json` (paradigm+language); how-to — позже. Отметки ведёт **только оркестратор**.\n")
...
o.append("Создаёт исполнитель блока (language-статья), paradigm-аспект — в указанной существующей статье; оркестратор вписывает новый файл в `manifest.json` и `.rag/bootstrap/sidebars-snapshot.js`.\n")
...
open(os.path.join(BASE,"DOCS-DESC-BLOCKS.md"),"w").write("\n".join(o))
...
def files_lines(label, arts):
    ...
    for a in arts: lines.append(f"- `docs/en/{a}.md` + `docs/ru/{a}.md`")
    return lines
p=[]
...
for tid,tname,_ in TRACKS:
    ...
    for bid in track_blocks(tid):
        ...
        p.append("")
        p.append("Перед работой прочитай: docs/AGENTS.md (релевантные разделы целиком) и DOCS-DESC-PLAN.md.")
        ...
        if LAY[bid]>=2:
            ...
            p.append("и термины) и дерево .rag/bootstrap/sidebars-snapshot.js, в большей степени чем код. Обзорная статья вводит и связывает")
        ...
        if CREATE.get(bid):
            p.append("СОЗДАТЬ новые статьи (нет в доке вообще; создать файлы en+ru, сообщить оркестратору для manifest/sidebars):")
            for art,what,phome in CREATE[bid]:
                p.append(f"- `docs/en/{art}.md` + `docs/ru/{art}.md` — {what}; {phome}.")
            p.append("")
        p.append("Подсмотреть уже описанный функционал (ТОЛЬКО для охвата; качество/формулировки оттуда НЕ брать — это")
        p.append("черновой ИИ-результат неоднозначного качества):  git show docs-desc:docs/en/<имя>.md")
        p.append("")
        p.append("ЦИКЛ СДАЧИ на КАЖДЫЙ под-цикл (commit-first):")
        if both:
        ...
        p.append("")
        p.append("Если конструкции блока ещё нет статьи — создать файлы en+ru и сообщить оркестратору (он добавит их в")
        p.append("manifest.json и .rag/bootstrap/sidebars-snapshot.js). Нашёл обобщаемое правило/проблему — допиши его в AGENTS.md")
        p.append("ОТДЕЛЬНЫМ коммитом (AGENTS.md дописывать может любой; это файл для ИИ). CLAUDE.md/manifest.json/sidebars.js")
        p.append("в коммиты блока НЕ включать. push и слияние (squash) отревьюенных коммитов делает оркестратор.")
        os.makedirs(PROMPTDIR, exist_ok=True)
        open(os.path.join(PROMPTDIR, f"{bid}.txt"),"w").write("\n".join(p[bstart:])+"\n")
        p.append("```")
        p.append("")
open(os.path.join(BASE,"DOCS-DESC-PROMPTS.md"),"w").write("\n".join(p))
