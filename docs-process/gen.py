...
# ВНИМАНИЕ: колонки Status/Owner и ручные заметки в DOCS-DESC-BLOCKS.md НЕ сохраняются —
# здесь зашит устаревший снимок статусов. Регенерация ПЕРЕЗАТИРАЕТ живой реестр (done→todo и т.п.).
# Реестр статусов ведёт оркестратор вручную в DOCS-DESC-BLOCKS.md. После любого `python3 gen.py`
# восстанови статусы (`git checkout HEAD -- DOCS-DESC-BLOCKS.md`, затем перенеси нужные правки текста).
import os
...
o.append("Создаёт исполнитель блока (language-статья в `docs/language/{en,ru}/`), paradigm-аспект — в указанной существующей статье; исполнитель подключает новый файл в `docs/sidebars.js` тем же блочным коммитом.\n")
...
for tid,tname,_ in TRACKS:
    ...
    for bid in track_blocks(tid):
        ...
        if CREATE.get(bid):
            p.append("СОЗДАТЬ новые статьи (нет в доке вообще; создать файлы en+ru И подключить их в docs/sidebars.js тем же блочным коммитом):")
        ...
        p.append("docs/language/) и сам подключить их в docs/sidebars.js тем же блочным коммитом. Нашёл обобщаемое правило/проблему —")
        p.append("допиши его в AGENTS.md ОТДЕЛЬНЫМ коммитом (AGENTS.md дописывать может любой; это файл для ИИ).")
        p.append("CLAUDE.md в коммиты блока НЕ включать; docs/sidebars.js — НАОБОРОТ, включать в блочный коммит (это часть документации). Ветку блока пушишь сам (для ревью); merge ветки")
